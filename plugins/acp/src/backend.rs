...
use crate::events::enqueue_line;
use crate::handler::PluginMessageHandler;
use crate::state::{HostHandle, SharedState};
use crate::utils::strip_ansi_and_controls;
...
impl AcpBackend {
	pub fn new(host: HostHandle, cmd_rx: Receiver<AgentCommand>, state: SharedState) -> Self {
		Self {
			host,
			cmd_rx,
			state,
		...
		if let Some(stderr) = stderr {
			...
			tokio::task::spawn_local(async move {
				...
				while let Ok(Some(line)) = lines.next_line().await {
					...
					enqueue_line(&state.events, &state.panel_id, format!("[acp] {}", line));
				}
			});
		}

		let handler = PluginMessageHandler {
			host: HostHandle::new(self.host.host.0),
	...
	fn enqueue_message(&self, msg: String) {
		enqueue_line(&self.state.events, &self.state.panel_id, msg);
	}
}
