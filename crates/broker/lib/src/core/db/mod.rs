//! Broker-owned helix-db storage initialization and schema configuration.
...
use crate::core::knowledge::KnowledgeError;

const SCHEMA_HQL: &str = include_str!("../knowledge/schema.hql");
...
#[cfg(test)]
pub(crate) fn schema_config() -> &'static Config {
	&SCHEMA_CONFIG
}

/// Broker-owned helix-db storage.
pub struct BrokerDb {
...
impl BrokerDb {
	/// Opens (or creates) the broker database at the given path.
	pub fn open(db_path: PathBuf) -> Result<Self, KnowledgeError> {
		std::fs::create_dir_all(&db_path)?;

		let path_str = db_path.to_str().unwrap_or("broker_db");
		let storage =
	...
	/// Returns the on-disk path for this database.
	pub fn db_path(&self) -> &PathBuf {
		&self.db_path
	}
}

/// Returns the default broker DB path under the user state directory.
///
/// # Errors
///
