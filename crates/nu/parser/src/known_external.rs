...
use xeno_nu_protocol::CustomExample;
use xeno_nu_protocol::ast::{self, Expr, Expression};
use xeno_nu_protocol::engine::{self, CallImpl, CommandType, UNKNOWN_SPAN_ID};
use xeno_nu_protocol::ir::{self, DataSlice};
...
fn ast_call_to_extern_call(engine_state: &EngineState, call: &ast::Call, extern_name: &[&str]) -> Result<ast::Call, ShellError> {
	let head_span = call.head;
	...
	let arg_extern_name = Expression::new_existing(Expr::String(extern_name[0].to_string()), call.head, call_head_id, Type::String);
...
fn ir_call_to_extern_call(stack: &mut Stack, call: &ir::Call, extern_name: &[&str]) -> Result<ir::Call, ShellError> {
	...
	for index in 0..call.args_len {
		match &call.arguments(stack)[index] {
			engine::Argument::Flag { data, name, short, span } => {
			...
			a @ (engine::Argument::Positional { .. } | engine::Argument::Spread { .. } | engine::Argument::ParserInfo { .. }) => {
...
fn known_external_option_name(data: &[u8], name: DataSlice, short: DataSlice) -> String {
	if !data[name].is_empty() {
		format!("--{}", std::str::from_utf8(&data[name]).expect("invalid utf-8 in flag name"))
	} else {
		format!("-{}", std::str::from_utf8(&data[short]).expect("invalid utf-8 in flag short name"))
	}
}
