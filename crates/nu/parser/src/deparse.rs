use xeno_nu_utils::escape_quote_string;

fn string_should_be_quoted(input: &str) -> bool {
	input.starts_with('$')
		|| input
			.chars()
			.any(|c| c == ' ' || c == '(' || c == '\'' || c == '`' || c == '"' || c == '\\' || c == ';' || c == '|')
}
...
mod test {
	...
	fn test_arg_with_flag() {
		...
		assert_eq!(escape_for_script_arg("--version=v5.2"), "--version=v5.2".to_string());
	...
	fn test_flag_arg_with_values_contains_special() {
		...
		assert_eq!(escape_for_script_arg("--version='xx yy'"), r#"--version="'xx yy'""#.to_string());
		assert_eq!(escape_for_script_arg("--separator=`"), r#"--separator="`""#.to_string());
	}
	...
	fn test_escape() {
		...
		assert_eq!(escape_for_script_arg(r#"--arg=""#), r#"--arg="\"""#.to_string());
	}
}
