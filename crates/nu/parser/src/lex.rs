use xeno_nu_protocol::{ParseError, Span};
...
// whitespace.
fn is_item_terminator(block_level: &[BlockKind], c: u8, additional_whitespace: &[u8], special_tokens: &[u8]) -> bool {
	block_level.is_empty()
		&& (c == b' ' || c == b'\t' || c == b'\n' || c == b'\r' || c == b'|' || c == b';' || additional_whitespace.contains(&c) || special_tokens.contains(&c))
}
...
pub fn lex_item(
	...
	while let Some(c) = input.get(*curr_offset) {
		...
		if let Some(start) = quote_start {
				...
				if input.get(*curr_offset + 1).is_some() {
					...
					*curr_offset += 2;
					...
					return (
						...
						Some(ParseError::UnexpectedEof((start as char).to_string(), Span::new(span.end - 1, span.end))),
					);
				}
			}
			...
			in_comment = previous_char.map(char::from).map(char::is_whitespace).unwrap_or(true);
		} else if c == b'\n' || c == b'\r' {
			in_comment = false;
			...
			if let Some(BlockKind::CurlyBracket) = block_level.last() {
				...
				*curr_offset += 1;
				...
				return (
					...
					Some(ParseError::Unbalanced("{".to_string(), "}".to_string(), Span::new(span.end - 1, span.end))),
				);
			}
		} else if c == b'(' {
			// We encountered an opening `(` delimiter.
	...
	if let Some(delim) = quote_start {
		...
		return (
			...
			Some(ParseError::UnexpectedEof((delim as char).to_string(), Span::new(span.end - 1, span.end))),
		);
	}
	...
	if let Some(block) = block_level.last() {
		...
		let cause = ParseError::UnexpectedEof((delim as char).to_string(), Span::new(span.end - 1, span.end));
	...
	let output = match &input[(span.start - span_offset)..(span.end - span_offset)] {
		...
		b"out>|" | b"o>|" => {
			err = Some(ParseError::Expected("`|`.  Redirecting stdout to a pipe is the same as normal piping.", span));
			Token {
				contents: TokenContents::Item,
				span,
			}
...
fn lex_raw_string(input: &[u8], curr_offset: &mut usize, span_offset: usize) -> Result<(), ParseError> {
	...
	*curr_offset += prefix_sharp_cnt + 1;
	...
	if input.get(*curr_offset) != Some(&b'\'') {
		return Err(ParseError::Expected("'", Span::new(span_offset + *curr_offset, span_offset + *curr_offset + 1)));
	}
...
pub fn lex_signature(
	...
	lex_internal(&mut state, additional_whitespace, special_tokens, skip_comment, true, None);
...
pub fn lex_n_tokens(state: &mut LexState, additional_whitespace: &[u8], special_tokens: &[u8], skip_comment: bool, max_tokens: usize) -> isize {
	let n_tokens = state.output.len();
	lex_internal(state, additional_whitespace, special_tokens, skip_comment, false, Some(max_tokens));
...
pub fn lex(input: &[u8], span_offset: usize, additional_whitespace: &[u8], special_tokens: &[u8], skip_comment: bool) -> (Vec<Token>, Option<ParseError>) {
	let mut state = LexState {
		input,
		output: Vec::new(),
		error: None,
	...
	lex_internal(&mut state, additional_whitespace, special_tokens, skip_comment, false, None);
	(state.output, state.error)
}
...
fn lex_internal(
	...
	while let Some(c) = state.input.get(curr_offset) {
		if max_tokens.is_some_and(|max_tokens| state.output.len() >= initial_output_len + max_tokens) {
			break;
		}
		let c = *c;
		if c == b'|' {
			...
			if let Some(prev) = state.output.last_mut() {
				match prev.contents {
					TokenContents::Eol => {
						*prev = Token::new(TokenContents::Pipe, Span::new(state.span_offset + idx, state.span_offset + idx + 1));
						...
						while state.output.len() > offset {
							...
							if state.output[index].contents == TokenContents::Comment && state.output[index - 1].contents == TokenContents::Eol {
					...
					_ => {
						state
							.output
							.push(Token::new(TokenContents::Pipe, Span::new(state.span_offset + idx, state.span_offset + idx + 1)));
					}
				}
			} else {
				state
			...
			if !is_complete && state.error.is_none() {
				state.error = Some(ParseError::ExtraTokens(Span::new(curr_offset, curr_offset + 1)));
			}
			let idx = curr_offset;
			curr_offset += 1;
			state.output.push(Token::new(
			...
			if !additional_whitespace.contains(&c) {
				state
					.output
					.push(Token::new(TokenContents::Eol, Span::new(state.span_offset + idx, state.span_offset + idx + 1)));
			}
		} else if c == b'#' {
...
fn is_redirection(token: &[u8]) -> bool {
	matches!(token, b"o>" | b"out>" | b"e>" | b"err>" | b"o+e>" | b"e+o>" | b"out+err>" | b"err+out>")
}
