...
pub fn parse_plugin_use(working_set: &mut StateWorkingSet, call: Box<Call>) -> Pipeline {
	use xeno_nu_protocol::{FromValue, PluginRegistryFile};
	...
	if let Err(err) = (|| {
		...
		let plugin_config = call
			...
			.map(|(_, _, expr)| {
				let expr = expr.as_ref().expect("--plugin-config arg should have been checked already");
				eval_constant(working_set, expr)
					.and_then(Spanned::<String>::from_value)
					.map_err(|err| err.wrap(working_set, call.head))
			})
		...
		let filename_query = {
			let path = xeno_nu_path::expand_path_with(&name.item, &cwd, true);
			path.to_str()
				.and_then(|path_str| find_in_dirs(path_str, working_set, &cwd, Some("NU_PLUGIN_DIRS")))
		...
		let plugin_config_path = if let Some(custom_path) = &plugin_config {
			find_in_dirs(&custom_path.item, working_set, &cwd, None).ok_or_else(|| ParseError::FileNotFound(custom_path.item.clone(), custom_path.span))?
		} else {
			ParserPath::RealPath(
				working_set
					.permanent_state
					...
					.ok_or_else(|| ParseError::LabeledErrorWithHelp {
						...
						span: call.head,
						help: "pass --plugin-config to `plugin use` when $nu.plugin-path is not set".into(),
					})?
					.to_owned(),
			)
		};
		...
		let contents = PluginRegistryFile::read_from(file, Some(call.head)).map_err(|err| err.wrap(working_set, call.head))?;
		...
		nu_plugin_engine::load_plugin_registry_item(working_set, plugin_item, Some(call.head)).map_err(|err| err.wrap(working_set, call.head))?;
	...
	Pipeline::from_vec(vec![Expression::new(working_set, Expr::Call(call), call_span, Type::Nothing)])
}
