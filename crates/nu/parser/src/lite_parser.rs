...
use xeno_nu_protocol::ast::RedirectionSource;
...
pub enum LiteRedirectionTarget {
	File { connector: Span, file: Span, append: bool },
	Pipe { connector: Span },
}
...
impl LiteRedirectionTarget {
	pub fn connector(&self) -> Span {
		match self {
			LiteRedirectionTarget::File { connector, .. } | LiteRedirectionTarget::Pipe { connector } => *connector,
		}
	}

	pub fn spans(&self) -> impl Iterator<Item = Span> {
		match *self {
			LiteRedirectionTarget::File { connector, file, .. } => Either::Left([connector, file].into_iter()),
...
pub enum LiteRedirection {
	Single { source: RedirectionSource, target: LiteRedirectionTarget },
	Separate { out: LiteRedirectionTarget, err: LiteRedirectionTarget },
}
...
impl LiteRedirection {
	pub fn spans(&self) -> impl Iterator<Item = Span> {
		match self {
			...
			LiteRedirection::Separate { out, err } => Either::Right(out.spans().chain(err.spans()).sorted()),
		}
	}
}
...
impl LiteCommand {
	...
	fn check_accepts_redirection(&self, span: Span) -> Option<ParseError> {
		self.parts.is_empty().then_some(ParseError::UnexpectedRedirection { span })
	}

	fn try_add_redirection(&mut self, source: RedirectionSource, target: LiteRedirectionTarget) -> Result<(), ParseError> {
		let redirection = match (self.redirection.take(), source) {
			(None, _) if self.parts.is_empty() => Err(ParseError::UnexpectedRedirection { span: target.connector() }),
			...
			(Some(LiteRedirection::Single { source, target: first }), _) => {
				Err(ParseError::MultipleRedirections(source, first.connector(), target.connector()))
			}
			(Some(LiteRedirection::Separate { out, .. }), RedirectionSource::Stdout | RedirectionSource::StdoutAndStderr) => {
				Err(ParseError::MultipleRedirections(RedirectionSource::Stdout, out.connector(), target.connector()))
			}
			(Some(LiteRedirection::Separate { err, .. }), RedirectionSource::Stderr) => {
				Err(ParseError::MultipleRedirections(RedirectionSource::Stderr, err.connector(), target.connector()))
			}
		}?;
	...
	pub fn parts_including_redirection(&self) -> impl Iterator<Item = Span> + '_ {
		self.parts
			.iter()
			.copied()
			.chain(self.redirection.iter().flat_map(|redirection| redirection.spans()))
...
fn last_non_comment_token(tokens: &[Token], cur_idx: usize) -> Option<TokenContents> {
	...
	for token in tokens.iter().take(cur_idx).rev() {
		...
		match (token.contents, expect) {
			(TokenContents::Comment, TokenContents::Comment) | (TokenContents::Comment, TokenContents::Eol) => expect = TokenContents::Eol,
...
pub fn lite_parse(tokens: &[Token], working_set: &StateWorkingSet) -> (LiteBlock, Option<ParseError>) {
			...
			Mode::Normal => {
				if let Some((source, append, span)) = file_redirection.take() {
					match &token.contents {
						...
						TokenContents::Pipe | TokenContents::ErrGreaterPipe | TokenContents::OutErrGreaterPipe => {
							error = error.or(Some(ParseError::Expected("redirection target", token.span)));
							command.push(span);
							pipeline.push(&mut command);
					...
					match &token.contents {
						...
						TokenContents::OutErrGreaterThan => {
							error = error.or(command.check_accepts_redirection(token.span));
							file_redirection = Some((RedirectionSource::StdoutAndStderr, false, token.span));
						}
						TokenContents::OutErrGreaterGreaterThan => {
							error = error.or(command.check_accepts_redirection(token.span));
						...
						TokenContents::ErrGreaterPipe => {
							let target = LiteRedirectionTarget::Pipe { connector: token.span };
						...
						TokenContents::OutErrGreaterPipe => {
							let target = LiteRedirectionTarget::Pipe { connector: token.span };
							if let Err(err) = command.try_add_redirection(RedirectionSource::StdoutAndStderr, target) {
								error = error.or(Some(err));
							}
							pipeline.push(&mut command);
							command.pipe = Some(token.span);
	...
	if last_non_comment_token(tokens, tokens.len()) == Some(TokenContents::Pipe) {
		(
			block,
			Some(ParseError::UnexpectedEof("pipeline missing end".into(), tokens[tokens.len() - 1].span)),
		)
	} else {
		(block, error)
	}
