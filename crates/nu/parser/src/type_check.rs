use xeno_nu_protocol::ast::{Assignment, Block, Comparison, Expr, Expression, Math, Operator, Pipeline, Range};
use xeno_nu_protocol::engine::StateWorkingSet;
use xeno_nu_protocol::{ParseError, Span, Type, combined_type_string};

fn type_error(op: Operator, op_span: Span, lhs: &Expression, rhs: &Expression, is_supported: fn(&Type) -> bool) -> (Type, Option<ParseError>) {
...
pub fn type_compatible(lhs: &Type, rhs: &Type) -> bool {
	...
	match (lhs, rhs) {
		...
		(Type::Any, _) => true,
		(_, Type::Any) => true,
		(Type::Record(lhs), Type::Record(rhs)) | (Type::Table(lhs), Type::Table(rhs)) => is_compatible(lhs, rhs),
		(Type::Glob, Type::String) => true,
		(Type::CellPath, other) => other.is_subtype_of(&Type::CellPath) || Type::CellPath.is_subtype_of(other),
		(Type::OneOf(types), u) | (u, Type::OneOf(types)) => types.iter().any(|t| type_compatible(t, u)),
		(lhs, rhs) => lhs == rhs,
	}
}

// TODO: rework type checking for Custom values
pub fn math_result_type(working_set: &mut StateWorkingSet, lhs: &mut Expression, op: &mut Expression, rhs: &mut Expression) -> (Type, Option<ParseError>) {
	let &Expr::Operator(operator) = &op.expr else {
		*op = Expression::garbage(working_set, op.span);
		return (Type::Any, Some(ParseError::IncompleteMathExpression(op.span)));
	};
	match operator {
			...
			_ => {
				*op = Expression::garbage(working_set, op.span);
				type_error(operator, op.span, lhs, rhs, |ty| {
					matches!(
						ty,
						Type::Int | Type::Float | Type::Number | Type::String | Type::Date | Type::Duration | Type::Filesize,
					)
				})
			}
		},
		Operator::Math(Math::Subtract) => match (&lhs.ty, &rhs.ty) {
			...
			_ => {
				*op = Expression::garbage(working_set, op.span);
				type_error(operator, op.span, lhs, rhs, |ty| {
					matches!(ty, Type::Int | Type::Float | Type::Number | Type::Date | Type::Duration | Type::Filesize)
				})
			}
		},
		Operator::Math(Math::Multiply) => match (&lhs.ty, &rhs.ty) {
			...
			_ => {
				*op = Expression::garbage(working_set, op.span);
				type_error(operator, op.span, lhs, rhs, |ty| {
					matches!(ty, Type::Int | Type::Float | Type::Number | Type::Duration | Type::Filesize,)
				})
			}
		},
		Operator::Math(Math::Divide) => match (&lhs.ty, &rhs.ty) {
			...
			_ => {
				*op = Expression::garbage(working_set, op.span);
				type_error(operator, op.span, lhs, rhs, |ty| {
					matches!(ty, Type::Int | Type::Float | Type::Number | Type::Filesize | Type::Duration)
				})
			}
		},
		...
		Operator::Math(Math::Pow) => match (&lhs.ty, &rhs.ty) {
			...
			_ => {
				*op = Expression::garbage(working_set, op.span);
				type_error(operator, op.span, lhs, rhs, |ty| matches!(ty, Type::Int | Type::Float | Type::Number))
			}
		},
		Operator::Math(Math::Concatenate) => match (&lhs.ty, &rhs.ty) {
			...
			_ => {
				*op = Expression::garbage(working_set, op.span);
				let is_supported = |ty: &Type| matches!(ty, Type::List(_) | Type::Table(_) | Type::String | Type::Binary | Type::Any | Type::Custom(_));
				let help = if matches!(lhs.ty, Type::List(_) | Type::Table(_)) || matches!(rhs.ty, Type::List(_) | Type::Table(_)) {
					Some(
			...
			_ => {
				*op = Expression::garbage(working_set, op.span);
				type_error(operator, op.span, lhs, rhs, |ty| {
					matches!(
						ty,
						Type::Int | Type::Float | Type::Number | Type::String | Type::Filesize | Type::Duration | Type::Date | Type::Bool | Type::Nothing
					)
				})
			}
		},
		...
		Operator::Comparison(Comparison::Equal | Comparison::NotEqual) => match (&lhs.ty, &rhs.ty) {
			(Type::Custom(a), Type::Custom(b)) if a == b => (Type::Custom(a.clone()), None),
			(Type::Custom(a), _) => (Type::Custom(a.clone()), None),
			_ => (Type::Bool, None),
		},
		...
		Operator::Comparison(Comparison::StartsWith | Comparison::NotStartsWith | Comparison::EndsWith | Comparison::NotEndsWith) => {
			match (&lhs.ty, &rhs.ty) {
				(Type::String | Type::Any, Type::String | Type::Any) => (Type::Bool, None),
				// TODO: should this include glob?
				(Type::Custom(a), Type::Custom(b)) if a == b => (Type::Custom(a.clone()), None),
		...
		Operator::Comparison(Comparison::In | Comparison::NotIn) => match (&lhs.ty, &rhs.ty) {
			...
			_ => {
				let err = if matches!(
					&rhs.ty,
					Type::List(_) | Type::Range | Type::String | Type::Record(_) | Type::Custom(_) | Type::Any
				) {
		...
		Operator::Assignment(Assignment::AddAssign) => compound_assignment_result_type(working_set, lhs, op, rhs, operator, Math::Add),
		Operator::Assignment(Assignment::ConcatenateAssign) => compound_assignment_result_type(working_set, lhs, op, rhs, operator, Math::Concatenate),
		Operator::Assignment(Assignment::DivideAssign) => compound_assignment_result_type(working_set, lhs, op, rhs, operator, Math::Divide),
		Operator::Assignment(Assignment::MultiplyAssign) => compound_assignment_result_type(working_set, lhs, op, rhs, operator, Math::Multiply),
		Operator::Assignment(Assignment::SubtractAssign) => compound_assignment_result_type(working_set, lhs, op, rhs, operator, Math::Subtract),
...
pub fn check_pipeline_type(working_set: &StateWorkingSet, pipeline: &Pipeline, input_type: Type) -> (Vec<Type>, Option<Vec<ParseError>>) {
	...
	for elem in &pipeline.elements {
		...
		if let Expr::Call(call) = &elem.expr.expr {
			...
			if new_types.contains(&Type::Any) {
				...
				new_types = io_types
					.into_iter()
					.filter(|(in_type, _)| current_types.iter().any(|ty| type_compatible(in_type, ty)))
			...
			let Some(types_string) = combined_type_string(&current_types, "or") else {
				output_errors
					...
					.push(ParseError::InternalError("Pipeline has no type at this point".to_string(), elem.expr.span));
				continue;
			};

			output_errors.get_or_insert_default().push(ParseError::InputMismatch(types_string, call.head));
		} else {
...
pub fn check_block_input_output(working_set: &StateWorkingSet, block: &Block) -> Vec<ParseError> {
	...
	for (input_type, output_type) in &block.signature.input_output_types {
		...
		for pipeline in &block.pipelines {
			let (checked_output_types, err) = check_pipeline_type(working_set, pipeline, current_type);
		...
		if !current_output_types.iter().any(|ty| type_compatible(output_type, ty)) {
			...
			let Some(current_ty_string) = combined_type_string(&current_output_types, "or") else {
				output_errors.push(ParseError::InternalError("Block has no type at this point".to_string(), span));
				continue;
			};

			output_errors.push(ParseError::OutputMismatch(output_type.clone(), current_ty_string, span))
		}
	}
...
pub fn check_range_types(working_set: &mut StateWorkingSet, range: &mut Range) {
	...
	match (&mut range.from, &mut range.next, &mut range.to) {
		(Some(expr), _, _) | (None, Some(expr), Some(_)) | (None, None, Some(expr)) if !type_compatible(&Type::Number, &expr.ty) => {
		...
		(Some(_), Some(rhs), _) | (Some(_), None, Some(rhs)) | (None, Some(_), Some(rhs)) if !type_compatible(&Type::Number, &rhs.ty) => {
			working_set.error(ParseError::OperatorUnsupportedType {
				op: "..",
				unsupported: rhs.ty.clone(),
				op_span: next_op_span,
		...
		(_, None) => {
			*op = Expression::garbage(working_set, op.span);
			let err = ParseError::OperatorIncompatibleTypes {
				...
				lhs_span: lhs.span,
				rhs_span: rhs.span,
				help: Some("The result type of this operation is not compatible with the type of the variable."),
			};
