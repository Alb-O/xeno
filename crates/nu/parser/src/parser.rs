		...
		match bytes.as_slice() {
			...
			b"plugin" => {
				if spans.len() > 1 && working_set.get_span_contents(spans[1]) == b"use" {
					...
					working_set.error(ParseError::BuiltinCommandInPipeline("plugin use".into(), spans[0]));
				}
...
pub fn parse_builtin_commands(working_set: &mut StateWorkingSet, lite_command: &LiteCommand) -> Pipeline {
	...
	match name {
		...
		#[cfg(feature = "plugin")]
		b"plugin" if lite_command.parts.get(1).is_some_and(|span| working_set.get_span_contents(*span) == b"use") => {
			if let Some(redirection) = lite_command.redirection.as_ref() {
