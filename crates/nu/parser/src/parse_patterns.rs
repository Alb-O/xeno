...
use xeno_nu_protocol::ast::{MatchPattern, Pattern};
use xeno_nu_protocol::engine::StateWorkingSet;
use xeno_nu_protocol::{ParseError, Span, SyntaxShape, Type, VarId};

use crate::parser::{is_variable, parse_value};
use crate::{lex, lite_parse};
		...
		for command in &output.block[0].commands {
			...
			while spans_idx < command.parts.len() {
				...
				if contents == b".." {
					...
					if let Some(var_id) =
						parse_variable_pattern_helper(working_set, Span::new(command.parts[spans_idx].start + 2, command.parts[spans_idx].end))
					{
						args.push(MatchPattern {
						...
						working_set.error(ParseError::Expected("valid variable name", command.parts[spans_idx]));
					}
				} else {
