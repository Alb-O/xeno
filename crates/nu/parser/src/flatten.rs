...
use xeno_nu_protocol::ast::{
	Argument, Block, Expr, Expression, ExternalArgument, ImportPatternMember, ListItem, MatchPattern, PathMember, Pattern, Pipeline, PipelineElement,
	PipelineRedirection, RecordItem,
};
use xeno_nu_protocol::engine::StateWorkingSet;
use xeno_nu_protocol::{DeclId, GetSpan, Span, SyntaxShape, VarId};
...
fn flatten_block_into(working_set: &StateWorkingSet, block: &Block, output: &mut Vec<(Span, FlatShape)>) {
	for pipeline in &block.pipelines {
		flatten_pipeline_into(working_set, pipeline, output);
	}
}

fn flatten_pipeline_into(working_set: &StateWorkingSet, pipeline: &Pipeline, output: &mut Vec<(Span, FlatShape)>) {
	for expr in &pipeline.elements {
		flatten_pipeline_element_into(working_set, expr, output)
	}
}

fn flatten_pipeline_element_into(working_set: &StateWorkingSet, pipeline_element: &PipelineElement, output: &mut Vec<(Span, FlatShape)>) {
	...
	if let Some(redirection) = pipeline_element.redirection.as_ref() {
		match redirection {
			...
			PipelineRedirection::Separate { out, err } => {
				let (out, err) = if out.span() <= err.span() { (out, err) } else { (err, out) };
...
fn flatten_positional_arg_into(working_set: &StateWorkingSet, positional: &Expression, shape: &SyntaxShape, output: &mut Vec<(Span, FlatShape)>) {
	if matches!(shape, SyntaxShape::ExternalArgument) && matches!(positional.expr, Expr::String(..) | Expr::GlobPattern(..)) {
...
fn flatten_expression_into(working_set: &StateWorkingSet, expr: &Expression, output: &mut Vec<(Span, FlatShape)>) {
	match &expr.expr {
		...
		Expr::UnaryNot(not) => {
			output.push((Span::new(expr.span.start, expr.span.start + 3), FlatShape::Operator));
		...
		Expr::Closure(block_id) => {
			...
			{
				output.push((Span::new(outer_span.start, first.0.start), FlatShape::Closure));
			}
		...
		Expr::Call(call) => {
			...
			let mut positional_args = signature.required_positional.iter().chain(&signature.optional_positional);
			...
			for arg in &call.arguments {
				match arg {
					...
					Argument::Unknown(positional) => {
						let shape = signature.rest_positional.as_ref().map(|arg| &arg.shape).unwrap_or(&SyntaxShape::Any);
					...
					Argument::Spread(expr) => {
						output.push((Span::new(expr.span.start - 3, expr.span.start), FlatShape::Operator));
						flatten_expression_into(working_set, expr, output);
					}
				}
			}
		...
		Expr::ImportPattern(import_pattern) => {
			...
			for member in &import_pattern.members {
				match member {
					...
					ImportPatternMember::Name { span, .. } => output.push((*span, FlatShape::String)),
					ImportPatternMember::List { names } => output.extend(names.iter().map(|&(_, span)| (span, FlatShape::String))),
				}
			}
		}
		...
		Expr::StringInterpolation(exprs) => {
				...
				&& first.0.start != expr.span.start
			{
				...
				output.push((Span::new(expr.span.start, expr.span.start + 2), FlatShape::StringInterpolation));
				flattened.push((Span::new(expr.span.end - 1, expr.span.end), FlatShape::StringInterpolation));
			}
			output.extend(flattened);
		}
		Expr::GlobInterpolation(exprs, quoted) => {
			...
			if *quoted {
				// If we aren't a bare word interpolation, also highlight the outer quotes
				output.push((Span::new(expr.span.start, expr.span.start + 2), FlatShape::GlobInterpolation));
				flattened.push((Span::new(expr.span.end - 1, expr.span.end), FlatShape::GlobInterpolation));
			}
			output.extend(flattened);
		}
		Expr::Record(list) => {
...
fn flatten_pattern_into(match_pattern: &MatchPattern, output: &mut Vec<(Span, FlatShape)>) {
	match &match_pattern.pattern {
		...
		Pattern::List(items) => {
			if let Some(first) = items.first() {
				if let Some(last) = items.last() {
					output.push((Span::new(match_pattern.span.start, first.span.start), FlatShape::MatchPattern));
					for item in items {
						flatten_pattern_into(item, output);
					}
					output.push((Span::new(last.span.end, match_pattern.span.end), FlatShape::MatchPattern))
				}
			} else {
		...
		Pattern::Record(items) => {
			if let Some(first) = items.first() {
				if let Some(last) = items.last() {
					output.push((Span::new(match_pattern.span.start, first.1.span.start), FlatShape::MatchPattern));
					for (_, pattern) in items {
						flatten_pattern_into(pattern, output);
					}
					output.push((Span::new(last.1.span.end, match_pattern.span.end), FlatShape::MatchPattern))
				}
			} else {
				output.push((match_pattern.span, FlatShape::MatchPattern));
			}
...
pub fn flatten_pipeline(working_set: &StateWorkingSet, pipeline: &Pipeline) -> Vec<(Span, FlatShape)> {
	let mut output = Vec::new();
	flatten_pipeline_into(working_set, pipeline, &mut output);
	output
}

pub fn flatten_pipeline_element(working_set: &StateWorkingSet, pipeline_element: &PipelineElement) -> Vec<(Span, FlatShape)> {
	let mut output = Vec::new();
	flatten_pipeline_element_into(working_set, pipeline_element, &mut output);
	output
}

pub fn flatten_expression(working_set: &StateWorkingSet, expr: &Expression) -> Vec<(Span, FlatShape)> {
	let mut output = Vec::new();
