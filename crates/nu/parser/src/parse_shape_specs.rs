...
use xeno_nu_protocol::{Completion, IntoSpanned, ParseError, ShellError, Span, Spanned, SyntaxShape, Type, Value};
use xeno_nu_utils::NuCow;

use crate::TokenContents;
use crate::lex::lex_signature;
...
pub fn parse_shape_name(working_set: &mut StateWorkingSet, bytes: &[u8], span: Span) -> SyntaxShape {
	match bytes {
		...
		_ if bytes.starts_with(b"oneof") || bytes.starts_with(b"list") || bytes.starts_with(b"record") || bytes.starts_with(b"table") => {
...
pub fn parse_completer(working_set: &mut StateWorkingSet, _bytes: &[u8], span: Span) -> Option<Completion> {
	let error_count = working_set.parse_errors.len();
	let expr = parse_value(
		working_set,
		span,
		&SyntaxShape::OneOf(vec![SyntaxShape::List(Box::new(SyntaxShape::String)), SyntaxShape::String]),
	);
...
fn parse_generic_shape(working_set: &mut StateWorkingSet<'_>, bytes: &[u8], span: Span) -> SyntaxShape {
...
fn split_generic_params<'a>(working_set: &mut StateWorkingSet, bytes: &'a [u8], span: Span) -> (&'a [u8], Option<Spanned<&'a [u8]>>) {
	...
	if params.ends_with(&[close_delim]) {
		let end = span.end - 1;
		(type_name, Some((&params[..(params.len() - 1)]).into_spanned(Span::new(start, end))))
...
fn parse_named_type_params(working_set: &mut StateWorkingSet, Spanned { item: source, span }: Spanned<&[u8]>) -> Vec<(String, SyntaxShape)> {
	...
	while idx < tokens.len() {
		...
		if working_set.get_span_contents(tokens[idx].span).starts_with(b",") {
			idx += 1;
			continue;
		}
...
fn parse_type_params(working_set: &mut StateWorkingSet, Spanned { item: source, span }: Spanned<&[u8]>) -> Vec<SyntaxShape> {
	let (tokens, err) = lex_signature(source, span.start, &[b'\n', b'\r'], &[b':', b','], true);
