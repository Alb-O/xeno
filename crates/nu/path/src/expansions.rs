...
/// the original error.
pub fn locate_in_dirs<I, P>(filename: impl AsRef<Path>, cwd: impl AsRef<Path>, dirs: impl FnOnce() -> I) -> std::io::Result<PathBuf>
where
	...
	P: AsRef<Path>,
{
	...
	match canonicalize_with(filename, cwd) {
		...
		Err(err) => {
			...
			let mut found = None;
			for dir in dirs() {
				if let Ok(path) = canonicalize_with(dir, cwd).and_then(|dir| canonicalize_with(filename, dir)) {
					found = Some(path);
					break;
				}
			}
