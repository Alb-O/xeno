...
///
/// It performs the same normalization as `xeno_nu_path::components()`, except it also expands n-dots,
...
///
/// It performs the same normalization as `xeno_nu_path::components()`, except it also expands ".."
...
mod test_expand_ndots {
	...
	fn three_dots_with_trailing_slash() {
		...
		let expected = if cfg!(windows) { r"\tmp\..\..\" } else { "/tmp/../../" };
		assert_path_eq!(expand_ndots(path), expected);
	}
	...
	fn filenames_with_dots() {
		...
		let expected = if cfg!(windows) { r"...foo...\" } else { "...foo.../" };
		assert_path_eq!(expand_ndots(path), expected);
	}
	...
	fn trailing_dots() {
		...
		let expected = if cfg!(windows) { r"\foo\bar\.." } else { "/foo/bar/.." };
		assert_path_eq!(expand_ndots(path), expected);
	}
...
mod test_expand_dots {
	...
	fn backtrack_once() {
		...
		let expected = if cfg!(windows) { r"\foo\baz\" } else { "/foo/baz/" };
		assert_path_eq!(expand_dots(path), expected);
	}
