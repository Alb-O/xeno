...
///
/// ```
/// use xeno_nu_path::{PathBuf, RelativePath, RelativePathBuf};
///
...
///
/// ```
/// use xeno_nu_path::{Path, RelativePathBuf};
///
...
#[cfg_attr(not(windows), doc = "```")]
#[cfg_attr(windows, doc = "```no_run")]
/// use xeno_nu_path::{AbsolutePath, AbsolutePathBuf, PathBuf};
///
...
#[cfg_attr(not(windows), doc = "```")]
#[cfg_attr(windows, doc = "```no_run")]
/// use xeno_nu_path::{AbsolutePathBuf, Path};
///
...
impl<Form: PathForm> PathBuf<Form> {
	...
	///
	/// ```
	/// use xeno_nu_path::{PathBuf, RelativePathBuf};
	///
...
impl_try_from_borrowed_to_borrowed!([std::path::Component<'_>], |component| { Path::new(component).try_into() });
impl_try_from_borrowed_to_borrowed!([std::path::Components<'_>], |components| { Path::new(components).try_into() });
...
impl TryFrom<Box<Path>> for Box<RelativePath> {
	...
	#[inline]
	fn try_from(path: Box<Path>) -> Result<Self, Self::Error> {
		if path.is_relative() { Ok(box_to_box_unchecked(path)) } else { Err(path) }
	}
}
...
impl TryFrom<Box<Path>> for Box<AbsolutePath> {
	...
	#[inline]
	fn try_from(path: Box<Path>) -> Result<Self, Self::Error> {
		if path.is_absolute() { Ok(box_to_box_unchecked(path)) } else { Err(path) }
	}
}
...
impl TryFrom<Box<std::path::Path>> for Box<RelativePath> {
	...
	#[inline]
	fn try_from(path: Box<std::path::Path>) -> Result<Self, Self::Error> {
		if path.is_relative() { Ok(std_box_to_box(path)) } else { Err(path) }
	}
}
...
impl TryFrom<Box<std::path::Path>> for Box<AbsolutePath> {
	...
	#[inline]
	fn try_from(path: Box<std::path::Path>) -> Result<Self, Self::Error> {
		if path.is_absolute() { Ok(std_box_to_box(path)) } else { Err(path) }
	}
}
