...
use xeno_nu_path::absolute_with;
use xeno_nu_protocol::ast::Expr;
use xeno_nu_protocol::engine::{Call, EngineState, Stack};
use xeno_nu_protocol::{ShellError, Span, Type, Value, VarId};
...
pub fn convert_env_vars(stack: &mut Stack, engine_state: &EngineState, conversions: &Value) -> Result<(), ShellError> {
	let conversions = conversions.as_record()?;
	for (key, conversion) in conversions.into_iter() {
		if let Some((case_preserve_env_name, val)) = stack.get_env_var_insensitive(engine_state, key) {
			match val.get_type() {
				Type::String => {}
				_ => continue,
			}
...
pub fn convert_env_values(engine_state: &mut EngineState, stack: &mut Stack) -> Result<(), ShellError> {
	...
	if let Some(err) = error { Err(err) } else { Ok(()) }
}
					...
					match value {
						Value::List { vals, .. } => {
							let paths: Vec<String> = vals
								...
								.map(|s| xeno_nu_path::expand_tilde(&*s).to_string_lossy().into_owned())
								.collect();
...
pub fn path_str(engine_state: &EngineState, stack: &Stack, span: Span) -> Result<String, ShellError> {
	let (pathname, pathval) = match stack.get_env_var_insensitive(engine_state, "path") {
		...
		None => Err(ShellError::EnvVarNotFoundAtRuntime {
			envvar_name: if cfg!(windows) { "Path".to_string() } else { "PATH".to_string() },
			span,
		}),
	}?;
...
pub fn get_dirs_var_from_call(stack: &Stack, call: &Call) -> Option<VarId> {
	call.get_parser_info(stack, DIR_VAR_PARSER_INFO)
		.and_then(|x| if let Expr::Var(id) = x.expr { Some(id) } else { None })
}
...
pub fn find_in_dirs_env(filename: &str, engine_state: &EngineState, stack: &Stack, dirs_var: Option<VarId>) -> Result<Option<PathBuf>, ShellError> {
...
fn get_converted_value(engine_state: &EngineState, stack: &Stack, name: &str, orig_val: &Value, direction: &str) -> Result<Value, ConversionError> {
	...
	Ok(ClosureEvalOnce::new(engine_state, stack, conversion.clone())
		.debug(false)
