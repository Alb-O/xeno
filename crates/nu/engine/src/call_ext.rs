...
use xeno_nu_protocol::engine::{self, EngineState, Stack, StateWorkingSet};
use xeno_nu_protocol::eval_const::eval_constant;
use xeno_nu_protocol::{FromValue, ShellError, Span, Value, ast, ir};
...
pub trait CallExt {
	...
	fn opt_const<T: FromValue>(&self, working_set: &StateWorkingSet, pos: usize) -> Result<Option<T>, ShellError>;
...
impl CallExt for ast::Call {
	...
	fn rest<T: FromValue>(&self, engine_state: &EngineState, stack: &mut Stack, starting_pos: usize) -> Result<Vec<T>, ShellError> {
		let stack = &mut stack.use_call_arg_out_dest();
		self.rest_iter_flattened(starting_pos, |expr| eval_expression::<WithoutDebug>(engine_state, stack, expr))?
			.into_iter()
...
impl CallExt for ir::Call {
	fn has_flag(&self, _engine_state: &EngineState, stack: &mut Stack, flag_name: &str) -> Result<bool, ShellError> {
		Ok(self.named_iter(stack).find(|(name, _)| name.item == flag_name).is_some_and(|(_, value)| {
			// Handle --flag=false
	...
	fn get_flag_span(&self, stack: &Stack, name: &str) -> Option<Span> {
		self.named_iter(stack).find_map(|(i_name, _)| (i_name.item == name).then_some(i_name.span))
	}

	fn rest<T: FromValue>(&self, _engine_state: &EngineState, stack: &mut Stack, starting_pos: usize) -> Result<Vec<T>, ShellError> {
		self.rest_iter_flattened(stack, starting_pos)?.into_iter().map(T::from_value).collect()
	}

	fn opt<T: FromValue>(&self, _engine_state: &EngineState, stack: &mut Stack, pos: usize) -> Result<Option<T>, ShellError> {
		self.positional_iter(stack).nth(pos).cloned().map(T::from_value).transpose()
	}

	fn opt_const<T: FromValue>(&self, _working_set: &StateWorkingSet, _pos: usize) -> Result<Option<T>, ShellError> {
