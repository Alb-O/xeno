...
use xeno_nu_protocol::engine::{Closure, EngineState, EnvVars, Stack};
use xeno_nu_protocol::{IntoPipelineData, PipelineData, ShellError, Value};

use crate::{EvalBlockWithEarlyReturnFn, eval_block_with_early_return, get_eval_block_with_early_return};
...
///
/// ```no_run
/// # use xeno_nu_protocol::{PipelineData, Value};
...
impl ClosureEval {
	...
	pub fn new_preserve_out_dest(engine_state: &EngineState, stack: &Stack, closure: Closure) -> Self {
		let engine_state = engine_state.clone();
	...
	fn try_add_arg(&mut self, value: Cow<Value>) {
		if let Some(var_id) = self.block.signature.get_positional(self.arg_index).and_then(|var| var.var_id) {
...
///
/// ```no_run
/// # use xeno_nu_protocol::{ListStream, PipelineData, PipelineIterator};
/// # use xeno_nu_engine::ClosureEvalOnce;
/// # let engine_state = unimplemented!();
/// # let stack = unimplemented!();
/// # let closure = unimplemented!();
...
///
/// ```no_run
/// # use xeno_nu_protocol::{PipelineData, PipelineIterator};
/// # use xeno_nu_engine::ClosureEvalOnce;
/// # let engine_state = unimplemented!();
/// # let stack = unimplemented!();
/// # let closure = unimplemented!();
...
impl<'a> ClosureEvalOnce<'a> {
	...
	pub fn new_preserve_out_dest(engine_state: &'a EngineState, stack: &Stack, closure: Closure) -> Self {
		let block = engine_state.get_block(closure.block_id);
		let eval = get_eval_block_with_early_return(engine_state);
		Self {
			engine_state,
