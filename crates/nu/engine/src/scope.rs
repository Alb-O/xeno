...
use xeno_nu_protocol::ast::Expr;
use xeno_nu_protocol::engine::{Command, EngineState, Stack, Visibility};
use xeno_nu_protocol::{CommandWideCompleter, DeclId, ModuleId, Signature, Span, Type, Value, VarId, record};
...
impl<'e, 's> ScopeData<'e, 's> {
	...
	pub fn collect_vars(&self, span: Span) -> Vec<Value> {
		...
		for (var_name, var_id) in &self.vars_map {
			...
			let var_value = var_value_result.ok().or(var.const_val.clone()).unwrap_or(Value::nothing(span));
	...
	pub fn collect_commands(&self, span: Span) -> Vec<Value> {
		...
		for (command_name, decl_id) in &self.decls_map {
			if self.visibility.is_decl_id_visible(decl_id) && !self.engine_state.get_decl(**decl_id).is_alias() {
	...
	fn collect_signatures(&self, signature: &Signature, span: Span) -> Value {
		let mut sigs = signature
			...
			.map(|(input_type, output_type)| {
				(
					...
					Value::list(self.collect_signature_entries(input_type, output_type, signature, span), span),
				)
			})
		...
		if sigs.is_empty() {
			...
			sigs.push((
				...
				Value::list(self.collect_signature_entries(any_type, any_type, signature, span), span),
			));
		}
	...
	fn collect_signature_entries(&self, input_type: &Type, output_type: &Type, signature: &Signature, span: Span) -> Vec<Value> {
		...
		for (decl_name, decl_id) in self.engine_state.get_decls_sorted(false) {
			if self.visibility.is_decl_id_visible(&decl_id) {
				...
				if let Some(alias) = decl.as_alias() {
					...
					let expansion = String::from_utf8_lossy(self.engine_state.get_span_contents(alias.wrapped_call.span));
	...
	fn collect_module(&self, module_name: &[u8], module_id: &ModuleId, span: Span) -> Value {
		...
		let (module_desc, module_extra_desc) = self.engine_state.build_module_desc(*module_id).unwrap_or_default();
	...
	pub fn collect_engine_state(&self, span: Span) -> Value {
		let num_env_vars = self.engine_state.env_vars.values().map(|overlay| overlay.len() as i64).sum();
		...
		(Value::Record { val: rec_a, .. }, Value::Record { val: rec_b, .. }) => {
			...
			match (rec_a.values().next(), rec_b.values().next()) {
				(Some(val_a), Some(val_b)) => match (val_a, val_b) {
					(Value::String { val: str_a, .. }, Value::String { val: str_b, .. }) => str_a.cmp(str_b),
					_ => Ordering::Equal,
				},
				_ => Ordering::Equal,
			}
