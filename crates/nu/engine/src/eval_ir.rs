...
#[cfg(feature = "os")]
use xeno_nu_protocol::process::check_exit_status_future;
...
impl<'a> EvalContext<'a> {
	...
	fn collect_reg(&mut self, reg_id: RegId, fallback_span: Span) -> Result<Value, ShellError> {
		...
		#[cfg(feature = "os")]
		if xeno_nu_experimental::PIPE_FAIL.get() {
...
fn eval_instruction<D: DebugContext>(
	...
	match instruction {
		...
		Instruction::CheckErrRedirected { src } => match ctx.borrow_reg(*src) {
			#[cfg(feature = "os")]
			PipelineData::ByteStream(stream, _) if matches!(stream.source(), xeno_nu_protocol::ByteStreamSource::Child(_)) => Ok(Continue),
		...
		Instruction::Call { decl_id, src_dst } => {
			...
			#[cfg(feature = "os")]
			{
				...
				let result_exit_status_future = result.clone_exit_status_future().map(|f| f.with_span(*span));
...
fn drain(ctx: &mut EvalContext<'_>, data: PipelineExecutionData) -> Result<InstructionResult, ShellError> {
	...
	let pipefail = xeno_nu_experimental::PIPE_FAIL.get();
	if !pipefail {
		return Ok(Continue);
	}
	#[cfg(feature = "os")]
