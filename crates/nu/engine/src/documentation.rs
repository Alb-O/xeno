...
use xeno_nu_protocol::ast::{Argument, Call, Expr, Expression, RecordItem};
use xeno_nu_protocol::debugger::WithoutDebug;
use xeno_nu_protocol::engine::{Command, CommandType, EngineState, Stack, UNKNOWN_SPAN_ID};
use xeno_nu_protocol::{Category, Config, IntoPipelineData, PipelineData, PositionalArg, Signature, Span, SpanId, Spanned, SyntaxShape, Type, Value, record};
use xeno_nu_utils::terminal_size;
...
pub fn get_full_help(command: &dyn Command, engine_state: &EngineState, stack: &mut Stack) -> String {
	...
	let sig = engine_state.get_signature(command).update_from_command(command);
	...
	match command.command_type() {
		CommandType::Alias => get_alias_documentation(&mut long_desc, command, &sig, &help_style, engine_state, stack),
		_ => get_command_documentation(&mut long_desc, command, &sig, &nu_config, &help_style, engine_state, stack),
	};

	let mut final_help = if !nu_config.use_ansi_coloring.get(engine_state) {
		xeno_nu_utils::strip_ansi_string_likely(long_desc)
	} else {
		long_desc
	};
...
fn try_nu_highlight(code_string: &str, reject_garbage: bool, engine_state: &EngineState, stack: &mut Stack) -> Option<String> {
...
fn nu_highlight_string(code_string: &str, engine_state: &EngineState, stack: &mut Stack) -> String {
	try_nu_highlight(code_string, false, engine_state, stack).unwrap_or_else(|| code_string.to_string())
}

/// Apply code highlighting to code in a capture group
fn highlight_capture_group(captures: &Captures, engine_state: &EngineState, stack: &mut Stack) -> String {
...
fn highlight_code<'a>(text: &'a str, engine_state: &EngineState, stack: &mut Stack) -> Cow<'a, str> {
	...
	let do_try_highlight = |captures: &Captures| highlight_capture_group(captures, engine_state, stack);
...
fn get_alias_documentation(
	...
	long_desc.push_str(&format!("{help_section_name}Alias{RESET}: {help_subcolor_one}{alias_name}{RESET}"));
	long_desc.push_str("\n\n");
	...
	let alias_expansion = String::from_utf8_lossy(engine_state.get_span_contents(alias.wrapped_call.span));
...
fn get_command_documentation(
	...
	let _ = write!(long_desc, "{help_section_name}Usage{RESET}:\n  > {}\n", sig.call_signature());
	...
	for (sig, decl_id) in signatures {
		...
		if sig.name.starts_with(&format!("{cmd_name} ")) && !matches!(sig.category, Category::Removed) {
			// If it's a plugin, alias, or custom command, display that information in the help
			if command_type == CommandType::Plugin || command_type == CommandType::Alias || command_type == CommandType::Custom {
	...
	if !sig.named.is_empty() {
		long_desc.push_str(&get_flags_section(sig, help_style, |v| match v {
			FormatterValue::DefaultValue(value) => nu_highlight_string(&value.to_parsable_string(", ", nu_config), engine_state, stack),
			FormatterValue::CodeString(text) => highlight_code(text, engine_state, stack).to_string(),
		}))
	}

	if !sig.required_positional.is_empty() || !sig.optional_positional.is_empty() || sig.rest_positional.is_some() {
		let _ = write!(long_desc, "\n{help_section_name}Parameters{RESET}:\n");
		for positional in &sig.required_positional {
			write_positional(long_desc, positional, PositionalKind::Required, help_style, nu_config, engine_state, stack);
		}
		...
		if let Some(rest_positional) = &sig.rest_positional {
			write_positional(long_desc, rest_positional, PositionalKind::Rest, help_style, nu_config, engine_state, stack);
		}
	}

	fn get_term_width() -> usize {
		if let Ok((w, _h)) = terminal_size() { w as usize } else { 80 }
	}
	...
	for example in examples {
		...
		if let Some(result) = &example.result {
			...
			table_call.add_named((
				...
				None,
				Some(Expression::new_unknown(Expr::Int(get_term_width() as i64 - 2), Span::unknown(), Type::Int)),
			));

			let table = engine_state.find_decl("table".as_bytes(), &[]).and_then(|decl_id| {
				engine_state
					.get_decl(decl_id)
					.run(engine_state, stack, &(&table_call).into(), PipelineData::value(result.clone(), None))
					.ok()
			});
...
fn update_ansi_from_config(ansi_code: &mut String, engine_state: &EngineState, nu_config: &Config, theme_component: &str) {
...
fn get_argument_for_color_value(nu_config: &Config, color: &Value, span: Span, span_id: SpanId) -> Option<Argument> {
	match color {
		Value::Record { val, .. } => {
			let record_exp: Vec<RecordItem> = (**val)
				...
				.map(|(k, v)| {
					RecordItem::Pair(
						Expression::new_existing(Expr::String(k.clone()), span, span_id, Type::String),
						Expression::new_existing(Expr::String(v.clone().to_expanded_string("", nu_config)), span, span_id, Type::String),
					)
				})
				.collect();
			...
			Some(Argument::Positional(Expression::new_existing(
				...
				Span::unknown(),
				UNKNOWN_SPAN_ID,
				Type::Record([("fg".to_string(), Type::String), ("attr".to_string(), Type::String)].into()),
			)))
		}
...
impl HelpStyle {
	...
	pub fn update_from_config(&mut self, engine_state: &EngineState, nu_config: &Config) {
		update_ansi_from_config(&mut self.section_name, engine_state, nu_config, "shape_string");
		update_ansi_from_config(&mut self.subcolor_one, engine_state, nu_config, "shape_external");
		update_ansi_from_config(&mut self.subcolor_two, engine_state, nu_config, "shape_block");
	}
}
...
fn write_positional(
	...
	if !positional.desc.is_empty() || arg_kind == PositionalKind::Optional {
		let _ = write!(long_desc, ": {}", highlight_code(&positional.desc, engine_state, stack));
	}
	if arg_kind == PositionalKind::Optional {
		if let Some(value) = &positional.default_value {
			let _ = write!(
				long_desc,
				" (optional, default: {})",
				nu_highlight_string(&value.to_parsable_string(", ", nu_config), engine_state, stack)
			);
		} else {
...
fn write_flag_to_long_desc<F>(flag: &xeno_nu_protocol::Flag, long_desc: &mut String, help_subcolor_one: &str, help_subcolor_two: &str, formatter: &mut F)
where
	F: FnMut(FormatterValue) -> String,
{
	...
	if !flag.desc.is_empty() {
		let _ = write!(long_desc, ": {}", &formatter(FormatterValue::CodeString(&flag.desc)));
	}
	if let Some(value) = &flag.default_value {
		let _ = write!(long_desc, " (default: {})", &formatter(FormatterValue::DefaultValue(value)));
	}
	long_desc.push('\n');
}
	...
	F: FnMut(FormatterValue) -> String,
{
	...
	for flag in flags {
		write_flag_to_long_desc(flag, &mut long_desc, help_subcolor_one, help_subcolor_two, &mut formatter);
	}
...
#[cfg(test)]
mod tests {
	use xeno_nu_protocol::UseAnsiColoring;
	...
	fn test_code_formatting() {
		...
		assert!(matches!(highlight_code(haystack, &engine_state, &mut stack), Cow::Owned(_)));
