use xeno_nu_protocol::ast::{Block, Expression};
use xeno_nu_protocol::debugger::{WithDebug, WithoutDebug};
use xeno_nu_protocol::engine::{EngineState, Stack};
use xeno_nu_protocol::{PipelineData, PipelineExecutionData, ShellError, Value};

use crate::{eval_block, eval_block_with_early_return, eval_expression, eval_expression_with_input, eval_ir_block, eval_subexpression};

/// Type of eval_block() function
pub type EvalBlockFn = fn(&EngineState, &mut Stack, &Block, PipelineData) -> Result<PipelineExecutionData, ShellError>;

/// Type of eval_ir_block() function
pub type EvalIrBlockFn = fn(&EngineState, &mut Stack, &Block, PipelineData) -> Result<PipelineExecutionData, ShellError>;

/// Type of eval_block_with_early_return() function
pub type EvalBlockWithEarlyReturnFn = fn(&EngineState, &mut Stack, &Block, PipelineData) -> Result<PipelineExecutionData, ShellError>;
...
pub type EvalExpressionWithInputFn = fn(&EngineState, &mut Stack, &Expression, PipelineData) -> Result<PipelineData, ShellError>;

/// Type of eval_subexpression() function
pub type EvalSubexpressionFn = fn(&EngineState, &mut Stack, &Block, PipelineData) -> Result<PipelineData, ShellError>;
