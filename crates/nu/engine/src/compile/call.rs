...
use xeno_nu_protocol::ast::{Argument, Call, Expression, ExternalArgument};
use xeno_nu_protocol::engine::{ENV_VARIABLE_ID, IN_VARIABLE_ID, NU_VARIABLE_ID, StateWorkingSet};
use xeno_nu_protocol::ir::{Instruction, IrAstRef, Literal};
use xeno_nu_protocol::{IntoSpanned, RegId, Span, Spanned};
...
pub(crate) fn compile_call(
	...
	if decl.is_keyword() {
		match decl.name() {
			"if" => {
				return compile_if(working_set, builder, call, redirect_modes, input_reg, io_reg);
			}
			"match" => {
				return compile_match(working_set, builder, call, redirect_modes, input_reg, io_reg);
			}
			"const" | "export const" => {
			...
			"let" | "mut" => {
				return compile_let(working_set, builder, call, redirect_modes, input_reg, io_reg);
			}
			"try" => {
				return compile_try(working_set, builder, call, redirect_modes, input_reg, io_reg);
			}
			"loop" => {
				return compile_loop(working_set, builder, call, redirect_modes, input_reg, io_reg);
			}
			"while" => {
				return compile_while(working_set, builder, call, redirect_modes, input_reg, io_reg);
			}
			"for" => {
				return compile_for(working_set, builder, call, redirect_modes, input_reg, io_reg);
			}
			"break" => {
				return compile_break(working_set, builder, call, redirect_modes, input_reg, io_reg);
			}
			"continue" => {
				return compile_continue(working_set, builder, call, redirect_modes, input_reg, io_reg);
			}
			"return" => {
				return compile_return(working_set, builder, call, redirect_modes, input_reg, io_reg);
			}
			"def" | "export def" => {
				return builder.load_empty(io_reg);
			}
	...
	enum CompiledArg<'a> {
		...
		Named(&'a str, Option<&'a str>, Option<RegId>, Span, Option<IrAstRef>),
	...
	for arg in &call.arguments {
		let arg_reg = arg
			...
			.map(|expr| {
				...
				compile_expression(working_set, builder, expr, RedirectModes::value(arg.span()), None, arg_reg)?;

				Ok(arg_reg)
			})
			.transpose()?;

		let ast_ref = arg.expr().filter(|_| requires_ast).map(|expr| IrAstRef(Arc::new(expr.clone())));

		match arg {
			Argument::Positional(_) | Argument::Unknown(_) => {
				compiled_args.push(CompiledArg::Positional(arg_reg.expect("expr() None in non-Named"), arg.span(), ast_ref))
			}
			...
			Argument::Spread(_) => compiled_args.push(CompiledArg::Spread(arg_reg.expect("expr() None in non-Named"), arg.span(), ast_ref)),
		}
	}
		...
		match arg {
			...
			CompiledArg::Named(name, short, Some(reg), span, ast_ref) => {
				if !name.is_empty() {
					...
					builder.push(Instruction::PushShortNamed { short, src: reg }.into_spanned(span))?;
				}
				builder.set_last_ast(ast_ref);
			}
...
pub(crate) fn compile_help(working_set: &StateWorkingSet<'_>, builder: &mut BlockBuilder, decl_name: Spanned<&str>, io_reg: RegId) -> Result<(), CompileError> {
	let help_command_id = working_set.find_decl(b"help").ok_or_else(|| CompileError::MissingRequiredDeclaration {
		decl_name: "help".into(),
		span: decl_name.span,
	})?;
...
pub(crate) fn compile_external_call(
	...
	compile_call(working_set, builder, &call, redirect_modes, input_reg, io_reg)
}

pub(crate) fn compile_unlet(_working_set: &StateWorkingSet, builder: &mut BlockBuilder, call: &Call, io_reg: RegId) -> Result<(), CompileError> {
	...
	for i in 0..call.positional_len() {
		...
		let var_id = match &arg.expr {
			xeno_nu_protocol::ast::Expr::Var(var_id) => Some(*var_id),
			xeno_nu_protocol::ast::Expr::FullCellPath(cell_path) => {
		...
		match var_id {
			Some(var_id) => {
				...
				if var_id == NU_VARIABLE_ID || var_id == ENV_VARIABLE_ID || var_id == IN_VARIABLE_ID {
					...
					return Err(CompileError::InvalidLiteral {
						msg: format!("'${}' is a built-in variable and cannot be deleted", var_name),
						span: arg.span,
					});
				}
