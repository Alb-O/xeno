use xeno_nu_protocol::ast::Pattern;
use xeno_nu_protocol::ir::{DataSlice, Instruction, IrAstRef, IrBlock, Literal};
use xeno_nu_protocol::{CompileError, IntoSpanned, RegId, Span, Spanned};
...
impl BlockBuilder {
	...
	pub(crate) fn next_register(&mut self) -> Result<RegId, CompileError> {
		if let Some(index) = self.register_allocation_state.iter_mut().position(|is_allocated| {
			if !*is_allocated {
				*is_allocated = true;
				true
			} else {
			...
			Err(CompileError::RegisterOverflow { block_span: self.block_span })
		}
	}

	/// Check if a register is initialized with a value.
	pub(crate) fn is_allocated(&self, reg_id: RegId) -> bool {
		self.register_allocation_state.get(reg_id.get() as usize).is_some_and(|state| *state)
	}

	/// Mark a register as initialized.
	pub(crate) fn mark_register(&mut self, reg_id: RegId) -> Result<(), CompileError> {
		if let Some(is_allocated) = self.register_allocation_state.get_mut(reg_id.get() as usize) {
			*is_allocated = true;
			Ok(())
		} else {
			Err(CompileError::RegisterOverflow { block_span: self.block_span })
	...
	pub(crate) fn free_register(&mut self, reg_id: RegId) -> Result<(), CompileError> {
		...
		if self.register_allocation_state.get(index).is_some_and(|is_allocated| *is_allocated) {
	...
	pub(crate) fn set_label(&mut self, label_id: LabelId, target_index: usize) -> Result<(), CompileError> {
		*self.labels.get_mut(label_id.0).ok_or(CompileError::UndefinedLabel {
			label_id: label_id.0,
			span: None,
		})? = Some(target_index);
		Ok(())
	...
	pub(crate) fn push(&mut self, instruction: Spanned<Instruction>) -> Result<(), CompileError> {
		...
		let allocate_result = match &instruction.item {
			...
			Instruction::LoadLiteral { dst, lit } => {
				allocate(&[], &[*dst]).and(
					...
					match lit {
						...
						| Literal::Filepath { val: _, no_expand: _ }
						| Literal::Directory { val: _, no_expand: _ }
						| Literal::GlobPattern { val: _, no_expand: _ }
						| Literal::String(_)
						| Literal::RawString(_)
						| Literal::CellPath(_)
						| Literal::Date(_)
			...
			Instruction::OpenFile { file_num: _, path, append: _ } => allocate(&[*path], &[]),
			Instruction::WriteFile { file_num: _, src } => allocate(&[*src], &[]),
			Instruction::CloseFile { file_num: _ } => Ok(()),
			Instruction::Call { decl_id: _, src_dst } => allocate(&[*src_dst], &[*src_dst]),
			Instruction::StringAppend { src_dst, val } => allocate(&[*src_dst, *val], &[*src_dst]),
			Instruction::GlobFrom { src_dst, no_expand: _ } => allocate(&[*src_dst], &[*src_dst]),
			Instruction::ListPush { src_dst, item } => allocate(&[*src_dst, *item], &[*src_dst]),
			Instruction::ListSpread { src_dst, items } => allocate(&[*src_dst, *items], &[*src_dst]),
			Instruction::RecordInsert { src_dst, key, val } => allocate(&[*src_dst, *key, *val], &[*src_dst]),
			Instruction::RecordSpread { src_dst, items } => allocate(&[*src_dst, *items], &[*src_dst]),
			Instruction::Not { src_dst } => allocate(&[*src_dst], &[*src_dst]),
			Instruction::BinaryOp { lhs_dst, op: _, rhs } => allocate(&[*lhs_dst, *rhs], &[*lhs_dst]),
			Instruction::FollowCellPath { src_dst, path } => allocate(&[*src_dst, *path], &[*src_dst]),
			Instruction::CloneCellPath { dst, src, path } => allocate(&[*src, *path], &[*src, *dst]),
			Instruction::UpsertCellPath { src_dst, path, new_value } => allocate(&[*src_dst, *path, *new_value], &[*src_dst]),
			...
			Instruction::Match { pattern: _, src, index: _ } => allocate(&[*src], &[*src]),
			Instruction::CheckMatchGuard { src } => allocate(&[*src], &[*src]),
			Instruction::Iterate { dst, stream, end_index: _ } => allocate(&[*stream], &[*dst, *stream]),
	...
	pub(crate) fn add_comment(&mut self, comment: impl std::fmt::Display) {
		add_comment(self.comments.last_mut().expect("no last instruction"), comment)
	}

	/// Load a register with a literal.
	pub(crate) fn load_literal(&mut self, reg_id: RegId, literal: Spanned<Literal>) -> Result<(), CompileError> {
		self.push(
	...
	pub(crate) fn data(&mut self, data: impl AsRef<[u8]>) -> Result<DataSlice, CompileError> {
		...
		if data.is_empty() {
			...
			Err(CompileError::DataOverflow { block_span: self.block_span })
		}
	}
	...
	pub(crate) fn branch_if(&mut self, cond: RegId, label_id: LabelId, span: Span) -> Result<(), CompileError> {
		self.push(Instruction::BranchIf { cond, index: label_id.0 }.into_spanned(span))
	}

	/// Add a `branch-if-empty` instruction
	pub(crate) fn branch_if_empty(&mut self, src: RegId, label_id: LabelId, span: Span) -> Result<(), CompileError> {
		self.push(Instruction::BranchIfEmpty { src, index: label_id.0 }.into_spanned(span))
	}
	...
	pub(crate) fn r#match(&mut self, pattern: Pattern, src: RegId, label_id: LabelId, span: Span) -> Result<(), CompileError> {
		self.push(
			Instruction::Match {
	...
	pub(crate) fn next_file_num(&mut self) -> Result<u32, CompileError> {
		...
		self.file_count = self
			.file_count
			.checked_add(1)
			.ok_or(CompileError::FileOverflow { block_span: self.block_span })?;
		Ok(next)
	}
	...
	pub(crate) fn push_break(&mut self, span: Span) -> Result<(), CompileError> {
		let loop_ = self.context_stack.current_loop().ok_or_else(|| CompileError::NotInALoop {
	...
	pub(crate) fn end_loop(&mut self, loop_: Loop) -> Result<(), CompileError> {
		let context_block = self.context_stack.pop().ok_or_else(|| CompileError::NotInALoop {
		...
		match context_block {
			...
			_ => Err(CompileError::IncoherentLoopState { block_span: self.block_span }),
		}
	}
	...
	pub(crate) fn finish(mut self) -> Result<IrBlock, CompileError> {
		...
		for (index, label_target) in self.labels.iter().enumerate() {
			if let Some(label_target) = label_target {
				add_comment(&mut self.comments[*label_target], format_args!("label({index})"));
			}
		}

		// Populate the actual target indices of labels into the instructions
		for ((index, instruction), span) in self.instructions.iter_mut().enumerate().zip(&self.spans) {
			if let Some(label_id) = instruction.branch_target() {
				let target_index = self
					...
					.cloned()
					.flatten()
					.ok_or(CompileError::UndefinedLabel { label_id, span: Some(*span) })?;
				// Add a comment to the target index that we come from here
				add_comment(&mut self.comments[target_index], format_args!("from({index}:)"));
				instruction
					.set_branch_target(target_index)
					.map_err(|_| CompileError::SetBranchTargetOfNonBranchInstruction {
...
impl ContextStack {
	...
	pub fn try_block_depth_from_loop(&self) -> usize {
		self.0.iter().rev().take_while(|&cb| matches!(cb, ContextBlock::Try)).count()
	}
...
fn add_comment(comment: &mut String, new_comment: impl std::fmt::Display) {
	use std::fmt::Write;
	write!(comment, "{}{}", if comment.is_empty() { "" } else { ", " }, new_comment).expect("formatting failed");
}
