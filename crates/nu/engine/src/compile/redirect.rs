use xeno_nu_protocol::ast::{Expression, RedirectionTarget};
use xeno_nu_protocol::engine::StateWorkingSet;
use xeno_nu_protocol::ir::{Instruction, RedirectMode};
use xeno_nu_protocol::{IntoSpanned, OutDest, RegId, Span, Spanned};
...
pub(crate) fn redirection_target_to_mode(
	...
	Ok(match target {
		RedirectionTarget::File {
			...
			compile_expression(working_set, builder, expr, RedirectModes::value(*redir_span), None, path_reg)?;
			builder.push(
...
pub(crate) fn redirect_modes_of_expression(working_set: &StateWorkingSet, expression: &Expression, redir_span: Span) -> Result<RedirectModes, CompileError> {
	let (out, err) = expression.expr.pipe_redirection(working_set);
	Ok(RedirectModes {
		out: out.map(|r| r.into_spanned(redir_span)).map(out_dest_to_redirect_mode).transpose()?,
		err: err.map(|r| r.into_spanned(redir_span)).map(out_dest_to_redirect_mode).transpose()?,
	})
}

/// Finish the redirection for an expression, writing to and closing files as necessary
pub(crate) fn finish_redirection(builder: &mut BlockBuilder, modes: RedirectModes, out_reg: RegId) -> Result<(), CompileError> {
	if let Some(Spanned {
	...
	}) = modes.out
	{
		...
		if !matches!(modes.err, Some(Spanned { item: RedirectMode::Pipe, .. })) {
			builder.push(Instruction::WriteFile { file_num, src: out_reg }.into_spanned(span))?;
	...
	match modes.err {
		Some(Spanned {
			...
			if !modes.out.is_some_and(|out_mode| match out_mode.item {
				RedirectMode::File { file_num: out_file_num } => file_num == out_file_num,
				_ => false,
			}) {
				builder.push(Instruction::CloseFile { file_num }.into_spanned(span))?;
			}
...
pub(crate) fn out_dest_to_redirect_mode(out_dest: Spanned<OutDest>) -> Result<Spanned<RedirectMode>, CompileError> {
