...
use xeno_nu_protocol::ir::Instruction;
use xeno_nu_protocol::{IntoSpanned, RegId, Type, VarId};
...
pub(crate) fn compile_if(
	...
	let not_condition_reg = {
		...
		compile_expression(working_set, builder, condition, RedirectModes::value(condition.span), None, condition_reg)?;

		// Negate the condition - we basically only want to jump if the condition is false
		builder.push(Instruction::Not { src_dst: condition_reg }.into_spanned(call.head))?;
	...
	compile_block(working_set, builder, true_block, redirect_modes.clone(), Some(io_reg), io_reg)?;
	...
	if let Some(else_arg) = else_arg {
		...
		match &else_expr.expr {
			Expr::Block(block_id) => {
				...
				compile_block(working_set, builder, false_block, redirect_modes, Some(io_reg), io_reg)?;
			}
			_ => {
				// The else case supports bare expressions too, not only blocks
				compile_expression(working_set, builder, else_expr, redirect_modes, Some(io_reg), io_reg)?;
			}
		}
	} else {
...
pub(crate) fn compile_match(
	...
	compile_expression(working_set, builder, match_expr, RedirectModes::value(match_expr.span), None, match_reg)?;
	...
	for (pattern, _) in match_block {
		...
		builder.r#match(pattern.pattern.clone(), match_reg, match_label, pattern.span)?;
	...
	for (index, (pattern, expr)) in match_block.iter().enumerate() {
		...
		if let Some(guard) = &pattern.guard {
			...
			compile_expression(working_set, builder, guard, RedirectModes::value(guard.span), None, guard_reg)?;
			builder.push(Instruction::CheckMatchGuard { src: guard_reg }.into_spanned(guard.span))?;
			...
			builder.branch_if(
				...
				match_block.get(index + 1).map(|b| b.0.span).unwrap_or(call.head),
			)?;
		...
		if let Expr::Block(block_id) = expr.expr {
			...
			compile_block(working_set, builder, block, redirect_modes.clone(), Some(io_reg), io_reg)?;
		} else {
			compile_expression(working_set, builder, expr, redirect_modes.clone(), Some(io_reg), io_reg)?;
		}
...
pub(crate) fn compile_let(
	...
	if has_initial_value {
		...
		compile_block(working_set, builder, block, RedirectModes::value(call.head), input_reg, io_reg)?;
		...
		if input_reg != io_reg {
			builder.push(Instruction::Move { dst: io_reg, src: input_reg }.into_spanned(call.head))?;
		}
	}
	...
	builder.push(Instruction::StoreVariable { var_id, src: io_reg }.into_spanned(call.head))?;
	...
	if has_initial_value {
		...
		builder.push(Instruction::LoadVariable { dst: io_reg, var_id }.into_spanned(call.head))?;
	}
...
pub(crate) fn compile_try(
	...
	enum CatchType<'a> {
		Block { block: &'a Block, var_id: Option<VarId> },
		Closure { closure_reg: RegId },
	}
	...
	let catch_type = catch_expr
		.map(|catch_expr| match catch_expr.as_block() {
			...
			None => {
				...
				compile_expression(working_set, builder, catch_expr, RedirectModes::value(catch_expr.span), None, closure_reg)?;
	...
	match catch_type {
		Some(CatchType::Block { block, var_id }) => {
			...
			if let Some(var_id) = var_id {
				...
				builder.push(Instruction::Clone { dst: err_reg, src: io_reg }.into_spanned(catch_span))?;
				builder.push(Instruction::StoreVariable { var_id, src: err_reg }.into_spanned(catch_span))?;
			}
		...
		Some(CatchType::Closure { closure_reg }) => {
			...
			let do_decl_id = working_set.find_decl(b"do").ok_or_else(|| CompileError::MissingRequiredDeclaration {
				decl_name: "do".into(),
				span: call.head,
			})?;
			...
			builder.push(Instruction::PushPositional { src: closure_reg }.into_spanned(catch_span))?;
	...
	if let Some(finally_part) = finally_type {
		if let Some(var_id) = finally_part.var_id {
			...
			builder.push(Instruction::Clone { dst: value_reg, src: io_reg }.into_spanned(call.head))?;
			builder.push(Instruction::StoreVariable { var_id, src: value_reg }.into_spanned(call.head))?;
		}
		compile_block(working_set, builder, finally_part.block, redirect_modes, Some(io_reg), io_reg)?;
	}
...
pub(crate) fn compile_loop(
	...
	compile_block(working_set, builder, block, RedirectModes::default(), None, io_reg)?;
...
pub(crate) fn compile_while(
	...
	compile_expression(working_set, builder, cond_arg, RedirectModes::value(call.head), None, io_reg)?;
...
pub(crate) fn compile_for(
	...
	compile_expression(working_set, builder, in_expr, RedirectModes::caller(in_expr.span), None, stream_reg)?;
	...
	builder.push(Instruction::StoreVariable { var_id, src: io_reg }.into_spanned(var_decl_arg.span))?;
...
pub(crate) fn compile_return(
	...
	if let Some(arg_expr) = call.positional_nth(0) {
		compile_expression(working_set, builder, arg_expr, RedirectModes::value(arg_expr.span), None, io_reg)?;
	} else {
		builder.load_empty(io_reg)?;
	}
