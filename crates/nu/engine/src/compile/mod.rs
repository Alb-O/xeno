use xeno_nu_protocol::ast::{Block, Expr, Pipeline, PipelineRedirection, RedirectionSource, RedirectionTarget};
use xeno_nu_protocol::engine::StateWorkingSet;
use xeno_nu_protocol::ir::{Instruction, IrBlock, RedirectMode};
use xeno_nu_protocol::{CompileError, IntoSpanned, RegId, Span};
...
pub fn compile(working_set: &StateWorkingSet, block: &Block) -> Result<IrBlock, CompileError> {
	...
	compile_block(working_set, &mut builder, block, RedirectModes::caller(span), Some(BLOCK_INPUT), BLOCK_INPUT)?;
	...
	if !block.pipelines.is_empty() {
		...
		for (index, pipeline) in block.pipelines.iter().enumerate() {
			compile_pipeline(
				...
				if index == last_index {
					redirect_modes.take().expect("should only take redirect_modes once")
				} else {
	...
	while let Some(element) = iter.next() {
		...
		let spec_redirect_modes = match &element.redirection {
			Some(PipelineRedirection::Single { source, target }) => {
				...
				match source {
					RedirectionSource::Stdout => RedirectModes { out: Some(mode), err: None },
					RedirectionSource::Stderr => RedirectModes { out: None, err: Some(mode) },
			...
			Some(PipelineRedirection::Separate { out, err }) => {
				...
				assert!(
					!matches!((out, err), (RedirectionTarget::Pipe { .. }, RedirectionTarget::Pipe { .. })),
			...
			None => RedirectModes { out: None, err: None },
		};
		...
		compile_expression(working_set, builder, &element.expr, redirect_modes.clone(), in_reg, out_reg)?;
...
fn is_block_call(expr: &Expr) -> bool {
	match expr {
		Expr::Call(inner) => inner.arguments.iter().any(|arg| matches!(arg.expr().map(|e| &e.expr), Some(Expr::Block(..)))),
		_ => false,
	}
}
