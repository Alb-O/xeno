use xeno_nu_protocol::ast::{CellPath, Expr, Expression, ListItem, RecordItem, ValueWithUnit};
use xeno_nu_protocol::engine::StateWorkingSet;
use xeno_nu_protocol::ir::{DataSlice, Instruction, Literal};
use xeno_nu_protocol::{ENV_VARIABLE_ID, IN_VARIABLE_ID, IntoSpanned, RegId, Span, Value};

use super::{BlockBuilder, CompileError, RedirectModes, compile_binary_op, compile_block, compile_call, compile_external_call, compile_load_env};
...
pub(crate) fn compile_expression(
	...
	let lit = |builder: &mut BlockBuilder, literal: Literal| {
		...
		builder
			.push(Instruction::LoadLiteral { dst: out_reg, lit: literal }.into_spanned(expr.span))
			.map(|_| ())
	};
	...
	let move_in_reg_to_out_reg = |builder: &mut BlockBuilder| {
		...
		if let Some(in_reg) = in_reg {
			if in_reg != out_reg {
				...
				builder.push(Instruction::Move { dst: out_reg, src: in_reg }.into_spanned(expr.span))?;
			}
		} else {
	...
	match &expr.expr {
		Expr::AttributeBlock(ab) => compile_expression(working_set, builder, &ab.item, redirect_modes, in_reg, out_reg),
		...
		Expr::Range(range) => {
			...
			let compile_part = |builder: &mut BlockBuilder, part_expr: Option<&Expression>| -> Result<RegId, CompileError> {
				let reg = builder.next_register()?;
				if let Some(part_expr) = part_expr {
					compile_expression(working_set, builder, part_expr, RedirectModes::value(part_expr.span), None, reg)?;
				} else {
		...
		Expr::Var(var_id) => {
				...
				&& *var_id == IN_VARIABLE_ID
			{
				...
				drop_input(builder)?;
				builder.push(Instruction::LoadVariable { dst: out_reg, var_id: *var_id }.into_spanned(expr.span))?;
			}
			Ok(())
		}
		...
		Expr::ExternalCall(head, args) => {
			...
			compile_external_call(working_set, builder, head, args, redirect_modes, in_reg, out_reg)
		}
		...
		Expr::UnaryNot(subexpr) => {
			drop_input(builder)?;
			compile_expression(working_set, builder, subexpr, RedirectModes::value(subexpr.span), None, out_reg)?;
		...
		Expr::BinaryOp(lhs, op, rhs) => {
			if let Expr::Operator(operator) = op.expr {
				...
				compile_binary_op(working_set, builder, lhs, operator.into_spanned(op.span), rhs, expr.span, in_reg, out_reg)
			} else {
		...
		Expr::List(items) => {
			...
			lit(builder, Literal::List { capacity: items.len() })?;
			for item in items {
				...
				compile_expression(working_set, builder, expr, RedirectModes::value(expr.span), None, reg)?;

				match item {
					ListItem::Item(_) => {
						// Add each item using list-push
						builder.push(Instruction::ListPush { src_dst: out_reg, item: reg }.into_spanned(expr.span))?;
					}
					ListItem::Spread(spread_span, _) => {
						// Spread the list using list-spread
						builder.push(Instruction::ListSpread { src_dst: out_reg, items: reg }.into_spanned(*spread_span))?;
					}
				}
			}
			Ok(())
		...
		Expr::Table(table) => {
			lit(builder, Literal::List { capacity: table.rows.len() })?;
			...
			let column_registers = table
				...
				.map(|column| {
					let reg = builder.next_register()?;
					compile_expression(working_set, builder, column, RedirectModes::value(column.span), None, reg)?;
					Ok(reg)
				})
			...
			for row in table.rows.iter() {
				...
				builder.load_literal(row_reg, Literal::Record { capacity: table.columns.len() }.into_spanned(expr.span))?;
				for (column_reg, item) in column_registers.iter().zip(row.iter()) {
					...
					compile_expression(working_set, builder, item, RedirectModes::value(item.span), None, item_reg)?;
					builder.push(
		...
		Expr::Record(items) => {
			lit(builder, Literal::Record { capacity: items.len() })?;
			...
			for item in items {
				match item {
					RecordItem::Pair(key, val) => {
						...
						compile_expression(working_set, builder, key, RedirectModes::value(key.span), None, key_reg)?;
						compile_expression(working_set, builder, val, RedirectModes::value(val.span), None, val_reg)?;
						builder.push(
							Instruction::RecordInsert {
								src_dst: out_reg,
								key: key_reg,
					...
					RecordItem::Spread(spread_span, expr) => {
						...
						let reg = builder.next_register()?;
						compile_expression(working_set, builder, expr, RedirectModes::value(expr.span), None, reg)?;
						builder.push(Instruction::RecordSpread { src_dst: out_reg, items: reg }.into_spanned(*spread_span))?;
					}
				}
			}
			Ok(())
		...
		Expr::Keyword(kw) => {
			...
			compile_expression(working_set, builder, &kw.expr, redirect_modes, in_reg, out_reg)
		}
		Expr::ValueWithUnit(value_with_unit) => lit(builder, literal_from_value_with_unit(value_with_unit)?),
		Expr::DateTime(dt) => lit(builder, Literal::Date(Box::new(*dt))),
		Expr::Filepath(path, no_expand) => {
			let val = builder.data(path)?;
			lit(builder, Literal::Filepath { val, no_expand: *no_expand })
		}
		Expr::Directory(path, no_expand) => {
			let val = builder.data(path)?;
			lit(builder, Literal::Directory { val, no_expand: *no_expand })
		}
		Expr::GlobPattern(path, no_expand) => {
			let val = builder.data(path)?;
			lit(builder, Literal::GlobPattern { val, no_expand: *no_expand })
		}
		Expr::String(s) => {
		...
		Expr::StringInterpolation(exprs) | Expr::GlobInterpolation(exprs, _) => {
			...
			if exprs_iter.peek().is_some_and(|e| matches!(e.expr, Expr::String(..) | Expr::RawString(..))) {
			...
			for expr in exprs_iter {
				...
				compile_expression(working_set, builder, expr, RedirectModes::value(expr.span), None, scratch_reg)?;
				builder.push(
			...
			if let Expr::GlobInterpolation(_, no_expand) = expr.expr {
				builder.push(Instruction::GlobFrom { src_dst: out_reg, no_expand }.into_spanned(expr.span))?;
			}
