use xeno_nu_protocol::ast::{Assignment, Boolean, CellPath, Expr, Expression, Math, Operator, PathMember, Pattern};
use xeno_nu_protocol::engine::StateWorkingSet;
use xeno_nu_protocol::ir::{Instruction, Literal};
use xeno_nu_protocol::{ENV_VARIABLE_ID, IntoSpanned, RegId, Span, Spanned, Value};
...
pub(crate) fn compile_binary_op(
	...
	if let Operator::Assignment(assign_op) = op.item {
		if let Some(decomposed_op) = decompose_assignment(assign_op) {
			...
			compile_binary_op(working_set, builder, lhs, decomposed_op.into_spanned(op.span), rhs, span, in_reg, out_reg)?;
		} else {
			// Compiling a plain assignment, where the current left-hand side value doesn't matter
			compile_expression(working_set, builder, rhs, RedirectModes::value(rhs.span), None, out_reg)?;
		}
		...
		let lhs_in_reg = if lhs.has_in_variable(working_set) { in_reg } else { None };

		compile_expression(working_set, builder, lhs, RedirectModes::value(lhs.span), lhs_in_reg, lhs_reg)?;
		...
		match op.item {
			...
			Operator::Boolean(bool_op @ Boolean::And) | Operator::Boolean(bool_op @ Boolean::Or) => {
				...
				builder.r#match(Pattern::Value(Value::bool(short_circuit_value, op.span)), lhs_reg, short_circuit_label, op.span)?;
				...
				let rhs_in_reg = if rhs.has_in_variable(working_set) { in_reg } else { None };

				compile_expression(working_set, builder, rhs, RedirectModes::value(rhs.span), rhs_in_reg, rhs_reg)?;
		...
		if lhs_reg != out_reg {
			builder.push(Instruction::Move { dst: out_reg, src: lhs_reg }.into_spanned(op.span))?;
		}
...
pub(crate) fn compile_assignment(
	...
	match lhs.expr {
		Expr::Var(var_id) => {
			...
			builder.push(Instruction::StoreVariable { var_id, src: rhs_reg }.into_spanned(assignment_span))?;
			Ok(())
		}
		Expr::FullCellPath(ref path) => match (&path.head, &path.tail) {
			(Expression { expr: Expr::Var(var_id), .. }, _) if *var_id == ENV_VARIABLE_ID => {
				...
				let val_reg = if path.tail.len() > 1 {
					...
					builder.push(Instruction::LoadEnvOpt { dst: head_reg, key: key_data }.into_spanned(lhs.span))?;
					...
					builder.load_literal(head_reg, Literal::Record { capacity: 0 }.into_spanned(lhs.span))?;

					// Do the upsert on the current value to incorporate rhs
					builder.set_label(upsert_label, builder.here())?;
					compile_upsert_cell_path(builder, (&path.tail[1..]).into_spanned(lhs.span), head_reg, rhs_reg, assignment_span)?;
				...
				builder.push(Instruction::StoreEnv { key: key_data, src: val_reg }.into_spanned(assignment_span))?;
				Ok(())
			}
			...
			_ => {
				...
				compile_expression(working_set, builder, &path.head, RedirectModes::value(path.head.span), None, head_reg)?;

				// Upsert the tail of the path into the old value of the head expression
				compile_upsert_cell_path(builder, path.tail.as_slice().into_spanned(lhs.span), head_reg, rhs_reg, assignment_span)?;
...
pub(crate) fn compile_load_env(builder: &mut BlockBuilder, span: Span, path: &[PathMember], out_reg: RegId) -> Result<(), CompileError> {
	match path {
		[] => builder.push(
		...
		[PathMember::String { val: key, optional, .. }, tail @ ..] => {
			...
			if !tail.is_empty() {
				let path = builder.literal(Literal::CellPath(Box::new(CellPath { members: tail.to_vec() })).into_spanned(span))?;
				builder.push(Instruction::FollowCellPath { src_dst: out_reg, path }.into_spanned(span))?;
			}
		}
	}
	Ok(())
