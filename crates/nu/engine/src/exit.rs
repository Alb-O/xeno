...
pub fn cleanup<T>(tag: T, engine_state: &EngineState) -> Option<T> {
	...
	if engine_state.is_interactive && jobs.iter().next().is_some() && !engine_state.exit_warning_given.load(Ordering::SeqCst) {
		...
		engine_state.exit_warning_given.store(true, Ordering::SeqCst);
