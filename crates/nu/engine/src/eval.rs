...
use xeno_nu_protocol::ast::{Assignment, Block, Call, Expr, Expression, ExternalArgument, PathMember};
use xeno_nu_protocol::debugger::DebugContext;
use xeno_nu_protocol::engine::{Closure, EngineState, Stack};
use xeno_nu_protocol::eval_base::Eval;
use xeno_nu_protocol::{BlockId, Config, ENV_VARIABLE_ID, IntoPipelineData, PipelineData, PipelineExecutionData, ShellError, Span, Value, VarId};
...
pub fn eval_call<D: DebugContext>(engine_state: &EngineState, caller_stack: &mut Stack, call: &Call, input: PipelineData) -> Result<PipelineData, ShellError> {
	...
	if !decl.is_known_external() && call.named_iter().any(|(flag, _, _)| flag.item == "help") {
		...
		for (param_idx, (param, required)) in decl
			...
			.iter()
			.map(|p| (p, true))
			.chain(decl.signature().optional_positional.iter().map(|p| (p, false)))
			.enumerate()
		{
		...
		if let Some(rest_positional) = decl.signature().rest_positional {
			...
			for result in call.rest_iter_flattened(
				decl.signature().required_positional.len() + decl.signature().optional_positional.len(),
...
fn eval_external(
	...
	let decl_id = engine_state.find_decl("run-external".as_bytes(), &[]).ok_or(ShellError::ExternalNotSupported {
...
pub fn eval_expression<D: DebugContext>(engine_state: &EngineState, stack: &mut Stack, expr: &Expression) -> Result<Value, ShellError> {
	...
	match &expr.expr {
		...
		Expr::FullCellPath(full_cell_path) => match &full_cell_path.head {
			...
			_ => {
				...
				stack.add_var(xeno_nu_protocol::IN_VARIABLE_ID, input_value);
...
pub fn eval_block_with_early_return<D: DebugContext>(
	...
	match eval_block::<D>(engine_state, stack, block, input) {
		Err(ShellError::Return { span: _, value }) => Ok(PipelineExecutionData::from(PipelineData::value(*value, None))),
...
pub fn eval_variable(engine_state: &EngineState, stack: &Stack, var_id: VarId, span: Span) -> Result<Value, ShellError> {
	match var_id {
		// $nu
		xeno_nu_protocol::NU_VARIABLE_ID => {
...
impl Eval for EvalRuntime {
	...
	fn eval_var(engine_state: &EngineState, stack: &mut Stack, var_id: VarId, span: Span) -> Result<Value, ShellError> {
		eval_variable(engine_state, stack, var_id, span)
	}

	fn eval_call<D: DebugContext>(engine_state: &EngineState, stack: &mut Stack, call: &Call, _: Span) -> Result<Value, ShellError> {
		// FIXME: protect this collect with ctrl-c
		eval_call::<D>(engine_state, stack, call, PipelineData::empty())?.into_value(call.head)
	}

	fn eval_external_call(engine_state: &EngineState, stack: &mut Stack, head: &Expression, args: &[ExternalArgument], _: Span) -> Result<Value, ShellError> {
		let span = head.span(&engine_state);
		// FIXME: protect this collect with ctrl-c
		eval_external(engine_state, stack, head, args, PipelineData::empty())?.into_value(span)
	}

	fn eval_collect<D: DebugContext>(engine_state: &EngineState, stack: &mut Stack, var_id: VarId, expr: &Expression) -> Result<Value, ShellError> {
		// It's a little bizarre, but the expression can still have some kind of result even with
		// nothing input
		eval_collect::<D>(engine_state, stack, var_id, expr, PipelineData::empty())?.into_value(expr.span)
	}

	fn eval_subexpression<D: DebugContext>(engine_state: &EngineState, stack: &mut Stack, block_id: BlockId, span: Span) -> Result<Value, ShellError> {
		let block = engine_state.get_block(block_id);
		// FIXME: protect this collect with ctrl-c
		eval_subexpression::<D>(engine_state, stack, block, PipelineData::empty())?.into_value(span)
	}

	fn regex_match(engine_state: &EngineState, op_span: Span, lhs: &Value, rhs: &Value, invert: bool, expr_span: Span) -> Result<Value, ShellError> {
				...
				match &cell_path.head.expr {
					Expr::Var(var_id) | Expr::VarDecl(var_id) => {
						...
						if is_env || engine_state.get_var(*var_id).mutable {
							let mut lhs = eval_expression::<D>(engine_state, stack, &cell_path.head)?;
							if is_env {
								...
								let original_key = if let Value::Record { val: record, .. } = &lhs {
									record.iter().rev().map(|(k, _)| k).find(|x| x.eq_ignore_case(&key)).cloned().unwrap_or(key)
								} else {
									key
								};
	...
	fn eval_row_condition_or_closure(engine_state: &EngineState, stack: &mut Stack, block_id: BlockId, span: Span) -> Result<Value, ShellError> {
...
pub(crate) fn is_automatic_env_var(var: &str) -> bool {
	...
	names.iter().any(|&name| if cfg!(windows) { name.eq_ignore_case(var) } else { name.eq(var) })
}
