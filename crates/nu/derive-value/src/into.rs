...
use syn::{Attribute, Data, DataEnum, DataStruct, DeriveInput, Fields, Generics, Ident, Index};
...
pub fn derive_into_value(input: TokenStream2) -> Result {
	...
	match input.data {
		Data::Struct(data_struct) => Ok(struct_into_value(input.ident, data_struct, input.generics, input.attrs)?),
		Data::Enum(data_enum) => Ok(enum_into_value(input.ident, data_enum, input.generics, input.attrs)?),
...
/// }
///
/// impl xeno_nu_protocol::IntoValue for Pet {
///     fn into_value(self, span: xeno_nu_protocol::Span) -> xeno_nu_protocol::Value {
///         xeno_nu_protocol::Value::record(xeno_nu_protocol::record! {
///             "name" => xeno_nu_protocol::IntoValue::into_value(self.name, span),
///             "age" => xeno_nu_protocol::IntoValue::into_value(self.age, span),
///             "favorite_toy" => xeno_nu_protocol::IntoValue::into_value(self.favorite_toy, span),
///         }, span)
///     }
/// }
/// ```
...
///
/// impl xeno_nu_protocol::IntoValue for Color {
///     fn into_value(self, span: xeno_nu_protocol::Span) -> xeno_nu_protocol::Value {
///         xeno_nu_protocol::Value::list(vec![
///             xeno_nu_protocol::IntoValue::into_value(self.0, span),
///             xeno_nu_protocol::IntoValue::into_value(self.1, span),
///             xeno_nu_protocol::IntoValue::into_value(self.2, span),
///         ], span)
///     }
/// }
/// ```
...
/// struct Unicorn;
///
/// impl xeno_nu_protocol::IntoValue for Unicorn {
///     fn into_value(self, span: xeno_nu_protocol::Span) -> xeno_nu_protocol::Value {
///         xeno_nu_protocol::Value::nothing(span)
///     }
/// }
/// ```
fn struct_into_value(ident: Ident, data: DataStruct, generics: Generics, attrs: Vec<Attribute>) -> Result {
	...
	let record = match &data.fields {
		...
		Fields::Unnamed(fields) => {
			let accessor = fields.unnamed.iter().enumerate().map(|(n, _)| Index::from(n)).map(|index| quote!(self.#index));
			fields_return_value(&data.fields, accessor, &container_attrs)?
		}
		Fields::Unit => quote!(xeno_nu_protocol::Value::nothing(span)),
	};
	let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
	Ok(quote! {
		#[automatically_derived]
		impl #impl_generics xeno_nu_protocol::IntoValue for #ident #ty_generics #where_clause {
			fn into_value(self, span: xeno_nu_protocol::Span) -> xeno_nu_protocol::Value {
				#record
			}
		}
...
///     fn into_value(self, span: xeno_nu_protocol::Span) -> xeno_nu_protocol::Value {
///         match self {
///             Self::Sunny => xeno_nu_protocol::Value::string("sunny", span),
///             Self::Cloudy => xeno_nu_protocol::Value::string("cloudy", span),
///             Self::Raining => xeno_nu_protocol::Value::string("rain", span),
///         }
///     }
/// }
/// ```
fn enum_into_value(ident: Ident, data: DataEnum, generics: Generics, attrs: Vec<Attribute>) -> Result {
	let container_attrs = ContainerAttributes::parse_attrs(attrs.iter())?;
	...
	let arms: Vec<TokenStream2> = data
		...
		.map(|variant| {
			...
			match &variant.fields {
				...
				Fields::Unit => Ok(quote!(Self::#ident => xeno_nu_protocol::Value::string(#ident_s, span))),
			}
		})
		.collect::<Result<_>>()?;
...
fn fields_return_value(fields: &Fields, accessor: impl Iterator<Item = impl ToTokens>, container_attrs: &ContainerAttributes) -> Result {
	match fields {
		Fields::Named(fields) => {
			let mut name_resolver = NameResolver::new();
			...
			for (field, accessor) in fields.named.iter().zip(accessor) {
				...
				items.push(quote!(#field => xeno_nu_protocol::IntoValue::into_value(#accessor, span)));
			}
			Ok(quote! {
				xeno_nu_protocol::Value::record(xeno_nu_protocol::record! {
					#(#items),*
		...
		f @ Fields::Unnamed(fields) => {
			...
			let items = fields
				...
				.iter()
				.zip(accessor)
				.map(|(_, accessor)| quote!(xeno_nu_protocol::IntoValue::into_value(#accessor, span)));
			Ok(quote!(xeno_nu_protocol::Value::list(std::vec![#(#items),*], span)))
		}
		Fields::Unit => Ok(quote!(xeno_nu_protocol::Value::nothing(span))),
	}
}
