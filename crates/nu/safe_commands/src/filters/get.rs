...
If multiple cell paths are given, this will produce a list of values."#
	...
	fn signature(&self) -> Signature {
		Signature::build("get")
			...
			.required("cell_path", SyntaxShape::CellPath, "The cell path to the data.")
			.rest("rest", SyntaxShape::CellPath, "Additional cell paths.")
			.switch("optional", "make all cell path members optional (returns `null` for missing values)", Some('o'))
			...
			.switch("sensitive", "get path in a case sensitive manner (deprecated)", Some('s'))
	...
	fn examples(&self) -> Vec<Example<'_>> {
		vec![
			...
			Example {
				...
				example: "[{A: A0}] | get A",
				result: Some(Value::list(vec![Value::test_string("A0")], Span::test_data())),
			},
		]
	}
	...
	fn run_const(&self, working_set: &StateWorkingSet, call: &Call, input: PipelineData) -> Result<PipelineData, ShellError> {
		...
		let optional = call.has_flag_const(working_set, "optional")? || call.has_flag_const(working_set, "ignore-errors")?;
	...
	fn run(&self, engine_state: &EngineState, stack: &mut Stack, call: &Call, input: PipelineData) -> Result<PipelineData, ShellError> {
		...
		let metadata = input.metadata();
		action(input, cell_path, rest, optional, ignore_case, engine_state.signals().clone(), call.head).map(|x| x.set_metadata(metadata))
	}
...
pub fn follow_cell_path_into_stream(data: PipelineData, signals: Signals, cell_path: Vec<PathMember>, head: Span) -> Result<PipelineData, ShellError> {
	let has_int_member = cell_path.iter().any(|it| matches!(it, PathMember::Int { .. }));
	match data {
