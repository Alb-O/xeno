...
use xeno_nu_protocol::engine::{CommandType, StateWorkingSet};
use xeno_nu_protocol::eval_const::{eval_const_subexpression, eval_constant, eval_constant_with_input};
...
impl Command for If {
	...
	fn signature(&self) -> xeno_nu_protocol::Signature {
		Signature::build("if")
			...
			.required("then_block", SyntaxShape::Block, "Block to run if check succeeds.")
			.optional(
				"else_expression",
				SyntaxShape::Keyword(
					b"else".to_vec(),
					Box::new(SyntaxShape::OneOf(vec![SyntaxShape::Block, SyntaxShape::Expression])),
				),
  ...
  https://www.nushell.sh/book/thinking_in_nu.html"#
	...
	fn run_const(&self, working_set: &StateWorkingSet, call: &Call, input: PipelineData) -> Result<PipelineData, ShellError> {
		...
		let then_block = then_expr.as_block().ok_or_else(|| ShellError::TypeMismatch {
	...
	fn run(&self, _engine_state: &EngineState, _stack: &mut Stack, _call: &Call, _input: PipelineData) -> Result<PipelineData, ShellError> {
