...
///
/// Paths are yielded in alphabetical order.
pub fn glob_with<I: Interruptible>(pattern: &str, options: MatchOptions, interrupt: I) -> Result<Paths<I>, PatternError> {
	#[cfg(windows)]
	...
	let root_len = normalized_pattern.to_str().expect("internal error: expected string").len() - rest.to_str().expect("internal error: expected string").len();
	let root = if root_len > 0 { Some(Path::new(&pattern[..root_len])) } else { None };

	if root_len > 0 && check_windows_verbatim(root.expect("internal error: already checked for len > 0")) {
...
pub fn glob_with_parent<I: Interruptible>(pattern: &str, options: MatchOptions, parent: &Path, interrupt: I) -> Result<Paths<I>, PatternError> {
...
///
/// ```rust
/// assert!(xeno_nu_glob::is_glob("foo/*"));
/// assert!(xeno_nu_glob::is_glob("foo/**/bar"));
/// assert!(xeno_nu_glob::is_glob("foo?"));
/// assert!(xeno_nu_glob::is_glob("foo[A]"));
///
/// assert!(!xeno_nu_glob::is_glob("foo"));
/// // xeno_nu_glob will ignore an unmatched ']'
/// assert!(!xeno_nu_glob::is_glob("foo]"));
/// // xeno_nu_glob doesn't expand {}
/// assert!(!xeno_nu_glob::is_glob("foo.{txt,png}"));
/// ```
...
impl fmt::Display for GlobError {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		write!(f, "attempting to read `{}` resulted in an error: {}", self.path.display(), self.error)
	}
}
...
impl<I: Interruptible> Iterator for Paths<I> {
	...
	fn next(&mut self) -> Option<GlobResult> {
			...
			&& !self.dir_patterns.is_empty()
		{
			...
			if self.todo.len() != 1 {
				fill_todo(&mut self.todo, &self.dir_patterns, 0, &scope, self.options, &self.interrupt);
			}
		}
		...
		loop {
			...
			let (path, mut idx) = match self.todo.pop().expect("internal error: already checked for non-empty") {
				Ok(pair) => pair,
			...
			if self.dir_patterns[idx].is_recursive {
				...
				if is_dir(&path) {
					...
					if !self.options.recursive_match_hidden_dir && path.file_name().map(|name| name.to_string_lossy().starts_with('.')).unwrap_or(false) {
						continue;
					}

					// push this directory's contents
					fill_todo(&mut self.todo, &self.dir_patterns, next, &path, self.options, &self.interrupt);
			...
			if self.dir_patterns[idx].matches_with(
				...
				if idx == self.dir_patterns.len() - 1 {
					...
					fill_todo(&mut self.todo, &self.dir_patterns, idx + 1, &path, self.options, &self.interrupt);
				}
			}
		}
	}
...
fn fill_todo(
	...
	match pattern_as_str(pattern) {
		Some(s) => {
			...
			if (special && is_dir) || (!special && (fs::metadata(&next_path).is_ok() || fs::symlink_metadata(&next_path).is_ok())) {
				add(todo, next_path);
			}
		}
		None if is_dir => {
			let dirs = fs::read_dir(path).and_then(|d| {
				d.map(|e| {
					...
					e.map(|e| {
						if curdir {
							PathBuf::from(e.path().file_name().expect("internal error: missing filename"))
						} else {
							e.path()
						}
					})
		...
		fn win() {
			...
			let root_with_device = current_dir()
				...
				.map(|p| match p.components().next().unwrap() {
					Component::Prefix(prefix_component) => Path::new(prefix_component.as_os_str()).join("*"),
