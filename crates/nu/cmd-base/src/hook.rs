...
use miette::Result;
use xeno_nu_engine::{eval_block, eval_block_with_early_return, redirect_env};
use xeno_nu_parser::parse;
use xeno_nu_protocol::debugger::WithoutDebug;
use xeno_nu_protocol::engine::{Closure, EngineState, Stack, StateWorkingSet};
use xeno_nu_protocol::report_error::{report_parse_error, report_shell_error};
use xeno_nu_protocol::{PipelineData, PositionalArg, ShellError, Span, Type, Value, VarId};

pub fn eval_env_change_hook(env_change_hook: &HashMap<String, Vec<Value>>, engine_state: &mut EngineState, stack: &mut Stack) -> Result<(), ShellError> {
...
pub fn eval_hooks(
	...
	for hook in hooks {
		eval_hook(engine_state, stack, None, arguments.clone(), hook, &format!("{hook_name} list, recursive"))?;
	}
	Ok(())
}
		...
		Value::String { val, .. } => {
			let (block, delta, vars) = {
				...
				for (name, val) in arguments {
					let var_id = working_set.add_variable(name.as_bytes().to_vec(), val.span(), Type::Any, false);
					vars.push((var_id, val));
				}

				let output = parse(&mut working_set, Some(&format!("{hook_name} hook")), val.as_bytes(), false);
			...
			let input = if let Some(input) = input { input } else { PipelineData::empty() };
		...
		Value::Record { val, .. } => {
			...
			let do_run_hook = if let Some(condition) = val.get("condition") {
				...
				if let Ok(closure) = condition.as_closure() {
					match run_hook(engine_state, stack, closure, None, arguments.clone(), other_span) {
						Ok(pipeline_data) => {
...
fn run_hook(
	...
	let mut callee_stack = stack.captures_to_stack_preserve_out_dest(closure.captures.clone()).reset_pipes();

	for (idx, PositionalArg { var_id, .. }) in block.signature.required_positional.iter().enumerate() {
	...
	let pipeline_data = eval_block_with_early_return::<WithoutDebug>(engine_state, &mut callee_stack, block, input)?.body;
