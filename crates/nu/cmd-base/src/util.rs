...
fn get_editor_commandline(value: &Value, var_name: &str) -> Result<(String, Vec<String>), ShellError> {
	match value {
...
pub fn get_editor(engine_state: &EngineState, stack: &Stack, span: Span) -> Result<(String, Vec<String>), ShellError> {
	let config = stack.get_config(engine_state);
	let env_vars = stack.get_env_vars(engine_state);

	if let Ok(buff_editor) = get_editor_commandline(&config.buffer_editor, "$env.config.buffer_editor") {
		Ok(buff_editor)
		...
		Err(ShellError::GenericError {
			...
			msg: "Please specify one via `$env.config.buffer_editor` or `$env.EDITOR`/`$env.VISUAL`".into(),
			span: Some(span),
			help: Some(HELP_MSG.into()),
			inner: vec![],
		})
