...
use windows::Wdk::System::Threading::{
	NtQueryInformationProcess, PROCESSINFOCLASS, ProcessBasicInformation, ProcessCommandLineInformation, ProcessWow64Information,
};
use windows::Win32::Foundation::{
	CloseHandle, FALSE, FILETIME, HANDLE, HLOCAL, HMODULE, LocalFree, MAX_PATH, STATUS_BUFFER_OVERFLOW, STATUS_BUFFER_TOO_SMALL, STATUS_INFO_LENGTH_MISMATCH,
	UNICODE_STRING,
};
use windows::Win32::Security::{
	AdjustTokenPrivileges, GetTokenInformation, LookupAccountSidW, LookupPrivilegeValueW, PSID, SE_DEBUG_NAME, SE_PRIVILEGE_ENABLED, SID, SID_NAME_USE,
	TOKEN_ADJUST_PRIVILEGES, TOKEN_GROUPS, TOKEN_PRIVILEGES, TOKEN_QUERY, TOKEN_USER, TokenGroups, TokenUser,
};
use windows::Win32::System::Diagnostics::Debug::ReadProcessMemory;
use windows::Win32::System::Diagnostics::ToolHelp::{CreateToolhelp32Snapshot, PROCESSENTRY32, Process32First, Process32Next, TH32CS_SNAPPROCESS};
use windows::Win32::System::Memory::{MEMORY_BASIC_INFORMATION, VirtualQueryEx};
use windows::Win32::System::ProcessStatus::{GetModuleBaseNameW, GetProcessMemoryInfo, K32EnumProcesses, PROCESS_MEMORY_COUNTERS, PROCESS_MEMORY_COUNTERS_EX};
use windows::Win32::System::SystemInformation::OSVERSIONINFOEXW;
use windows::Win32::System::Threading::{
	GetCurrentProcess, GetPriorityClass, GetProcessIoCounters, GetProcessTimes, IO_COUNTERS, OpenProcess, OpenProcessToken, PEB, PROCESS_BASIC_INFORMATION,
	...
	for (pid, prev_sys, prev_user, prev_read, prev_write, prev_time) in base_procs {
		...
		if let Some(handle) = handle {
			...
			let start_time = if let Some((start, _, _, _)) = times {
				...
				let base = NaiveDate::from_ymd_opt(1601, 1, 1).and_then(|nd| nd.and_hms_opt(0, 0, 0));
				...
				let time = NaiveDate::from_ymd_opt(1601, 1, 1).and_then(|nt| nt.and_hms_opt(0, 0, 0));
...
fn set_privilege() -> bool {
	unsafe {
		...
		if LookupPrivilegeValueW(PCWSTR::null(), SE_DEBUG_NAME, &mut tps.Privileges[0].Luid).is_err() {
			return false;
		}
...
fn get_pids() -> Vec<i32> {
	...
	unsafe {
		pids.set_len(10192);
		let result = K32EnumProcesses(pids.as_mut_ptr(), (dword_size * pids.len()) as u32, &mut cb_needed);
		if !result.as_bool() {
			return Vec::new();
		}
...
fn get_handle(pid: i32) -> Option<HANDLE> {
	...
	let handle = unsafe { OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE.into(), pid as u32) }.ok();
...
fn get_times(handle: HANDLE) -> Option<(u64, u64, u64, u64)> {
	unsafe {
		...
		if ret.is_ok() { Some((start, exit, sys, user)) } else { None }
	}
}
...
fn get_command(handle: HANDLE) -> Option<String> {
	unsafe {
		...
		if ret != 0 { Some(String::from_utf16_lossy(&exe_buf[..pos])) } else { None }
	}
}
...
unsafe fn null_terminated_wchar_to_string(slice: &[u16]) -> String {
	match slice.iter().position(|&x| x == 0) {
		Some(pos) => OsString::from_wide(&slice[..pos]).to_string_lossy().into_owned(),
		None => OsString::from_wide(slice).to_string_lossy().into_owned(),
	}
}

unsafe fn get_process_data(handle: HANDLE, ptr: *const c_void, size: usize) -> Result<Vec<u16>, &'static str> {
	let mut buffer: Vec<u16> = Vec::with_capacity(size / 2 + 1);
	let mut bytes_read = 0;

	unsafe {
		if ReadProcessMemory(handle, ptr, buffer.as_mut_ptr().cast(), size, Some(&mut bytes_read)).is_err() {
...
unsafe fn get_region_size(handle: HANDLE, ptr: *const c_void) -> Result<usize, &'static str> {
	unsafe {
		...
		if VirtualQueryEx(handle, Some(ptr), meminfo.as_mut_ptr().cast(), size_of::<MEMORY_BASIC_INFORMATION>()) == 0 {
...
unsafe fn ph_query_process_variable_size(process_handle: HANDLE, process_information_class: PROCESSINFOCLASS) -> Option<Vec<u16>> {
	unsafe {
...
unsafe fn get_process_params(handle: HANDLE) -> Result<(Vec<String>, Vec<String>, PathBuf), &'static str> {
	unsafe {
		...
		if pwow32info.is_null() {
			...
			if ReadProcessMemory(handle, pinfo.PebBaseAddress.cast(), peb.as_mut_ptr().cast(), size_of::<PEB>(), None).is_err() {
		...
		if ReadProcessMemory(handle, pwow32info, peb32.as_mut_ptr().cast(), size_of::<PEB32>(), None).is_err() {
...
static WINDOWS_8_1_OR_NEWER: LazyLock<bool> = LazyLock::new(|| unsafe {
	...
	// Windows 8.1 is 6.3
	version_info.dwMajorVersion > 6 || version_info.dwMajorVersion == 6 && version_info.dwMinorVersion >= 3
});
...
fn get_proc_env<T: RtlUserProcessParameters>(params: &T, handle: HANDLE) -> Vec<String> {
	match params.get_environ(handle) {
		Ok(buffer) => {
			let equals = "=".encode_utf16().next().expect("unable to get next utf16 value");
			let raw_env = buffer;
...
fn get_user(handle: HANDLE) -> Option<SidName> {
	unsafe {
		...
		let mut cb_needed = 0;
		let _ = GetTokenInformation(token, TokenUser, Some(ptr::null::<c_void>() as *mut c_void), 0, &mut cb_needed);

		let mut buf: Vec<u8> = Vec::with_capacity(cb_needed as usize);

		let ret = GetTokenInformation(token, TokenUser, Some(buf.as_mut_ptr() as *mut c_void), cb_needed, &mut cb_needed);
		...
		Some(SidName { sid, name, domainname })
	}
}
...
fn get_groups(handle: HANDLE) -> Option<Vec<SidName>> {
	unsafe {
		...
		let mut cb_needed = 0;
		let _ = GetTokenInformation(token, TokenGroups, Some(ptr::null::<c_void>() as *mut c_void), 0, &mut cb_needed);

		let mut buf: Vec<u8> = Vec::with_capacity(cb_needed as usize);

		let ret = GetTokenInformation(token, TokenGroups, Some(buf.as_mut_ptr() as *mut c_void), cb_needed, &mut cb_needed);
		buf.set_len(cb_needed as usize);
		...
		for i in 0..(*token_groups).GroupCount {
			...
			let sid_name = SidName { sid, name, domainname };
			ret.push(sid_name);
		}
...
thread_local!(
	pub static NAME_CACHE: RefCell<HashMap<*mut c_void, Option<(String, String)>>> = RefCell::new(HashMap::new());
);
...
fn get_name(psid: PSID) -> Option<(String, String)> {
	unsafe {
		...
		let _ = LookupAccountSidW(None, psid, None, &mut cc_name, None, &mut cc_domainname, &mut pe_use);
...
fn from_wide_ptr(ptr: *const u16) -> String {
	unsafe {
		assert!(!ptr.is_null());
		let len = (0..isize::MAX).position(|i| *ptr.offset(i) == 0).unwrap_or_default();
