...
fn get_path_info(pid: i32, mut size: size_t) -> Option<PathInfo> {
	...
	unsafe {
		let ret = libc::sysctl(mib.as_mut_ptr(), 3, ptr as *mut c_void, &mut size, ::std::ptr::null_mut(), 0);
		if ret != -1 {
			let mut n_args: c_int = 0;
			libc::memcpy((&mut n_args) as *mut c_int as *mut c_void, ptr as *const c_void, ::std::mem::size_of::<c_int>());
			...
			if cp < ptr.add(size) {
				...
				let name = exe.file_name().unwrap_or_default().to_str().unwrap_or("").to_owned();
...
impl ProcessInfo {
	...
	pub fn cpu_usage(&self) -> f64 {
		let curr_time = self.curr_task.ptinfo.pti_total_user + self.curr_task.ptinfo.pti_total_system;
	...
	pub fn cwd(&self) -> String {
		self.curr_path.as_ref().map(|cur_path| cur_path.cwd.display().to_string()).unwrap_or_default()
	}
}
