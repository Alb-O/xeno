	...
	Ok(procs_a
		...
		.map(|proc| {
			...
			let percent_cpu = if let Some(prev_proc) = prev_proc {
				let prev_rtime = prev_proc.p_rtime_sec as f64 + prev_proc.p_rtime_usec as f64 / 1_000_000.0;
...
unsafe fn sysctl_get(name: *const i32, name_len: u32, data: *mut libc::c_void, data_len: *mut usize) -> i32 {
...
fn get_procs() -> io::Result<Vec<KInfoProc>> {
	...
	unsafe {
		...
		let mut ctl_name = [CTL_KERN, TGT_KERN_PROC, KERN_PROC_ALL, 0, STRUCT_SIZE as i32, 0];

		// First, try to figure out how large a buffer we need to allocate
		// (calling with NULL just tells us that)
		let mut data_len = 0;
		check(sysctl_get(ctl_name.as_ptr(), ctl_name.len() as u32, ptr::null_mut(), &mut data_len))?;
...
fn get_proc_args(pid: i32, what: i32) -> io::Result<Vec<u8>> {
	unsafe {
		...
		let vec = {
			...
			let max_index: isize = (vec.len() / mem::size_of::<*const u8>()).try_into().expect("too big for isize");
			...
			for index in 0..max_index {
				...
				if *ptr == ptr::null() {
					...
					assert!(*ptr >= min_ptr && *ptr < max_ptr, "pointer out of bounds of the buffer returned by sysctl()");
					// Also bounds-check the C strings, to make sure we don't overrun the buffer
					new_vec.extend(
						CStr::from_bytes_until_nul(std::slice::from_raw_parts(*ptr, max_ptr.offset_from(*ptr) as usize))
