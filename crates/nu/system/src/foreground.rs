...
impl ForegroundChild {
	...
	#[cfg(unix)]
	pub fn spawn(mut command: Command, interactive: bool, background: bool, pipeline_state: &Arc<(AtomicU32, AtomicU32)>) -> io::Result<Self> {
...
impl UnfreezeHandle {
	#[cfg(unix)]
	pub fn unfreeze(self, pipeline_state: Option<Arc<(AtomicU32, AtomicU32)>>) -> io::Result<ForegroundWaitStatus> {
		// bring child's process group back into foreground and continue it

		// we only keep the guard for its drop impl
		let _guard = pipeline_state.map(|pipeline_state| ForegroundGuard::new(self.child_pid.as_raw() as u32, &pipeline_state));
...
impl ForegroundGuard {
	...
	#[cfg(unix)]
	pub fn new(pid: u32, pipeline_state: &Arc<(AtomicU32, AtomicU32)>) -> std::io::Result<ForegroundGuard> {
		...
		loop {
			...
			if pgrp.compare_exchange(0, pid, Ordering::SeqCst, Ordering::SeqCst).is_ok() {
				let _ = pcnt.fetch_add(1, Ordering::SeqCst);
...
mod child_pgroup {
	...
	pub fn set(process: &Child, existing_pgrp: u32, background: bool) {
		set_foreground_pid(Pid::from_raw(process.id() as i32), existing_pgrp, background);
	}
	...
	fn set_foreground_pid(pid: Pid, existing_pgrp: u32, background: bool) {
		...
		let pgrp = if existing_pgrp == 0 { pid } else { Pid::from_raw(existing_pgrp as i32) };
