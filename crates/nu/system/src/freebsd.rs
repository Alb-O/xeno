...
use libc::{CTL_HW, CTL_KERN, KERN_PROC, KERN_PROC_ALL, KERN_PROC_ARGS, TDF_IDLETD, c_char, kinfo_proc, sysctl};
...
fn compare_procs(interval: Duration) -> io::Result<Vec<ProcessInfo>> {
	...
	Ok(a_grouped
		.into_iter()
		.merge_join_by(b_grouped.into_iter(), |(pid_a, _), (pid_b, _)| pid_a.cmp(pid_b))
		.map(|threads| {
			...
			let mut threads = {
				...
				left.into_iter()
					...
					.merge_join_by(right.into_iter().flat_map(|(_, threads)| threads), |thread_a, thread_b| {
						thread_a.ki_tid.cmp(&thread_b.ki_tid)
					})
					.peekable()
			};
			...
			let (runtime_a, runtime_b) = threads.flat_map(|t| t.both()).fold((0., 0.), |(runtime_a, runtime_b), (a, b)| {
				let runtime_in_seconds = |proc: &kinfo_proc| proc.ki_runtime as f64 /* µsec */ / 1_000_000.0;
				(runtime_a + runtime_in_seconds(&a), runtime_b + runtime_in_seconds(&b))
			});
...
fn get_procs() -> io::Result<Vec<libc::kinfo_proc>> {
	...
	unsafe {
		...
		let mut data_len = 0;
		check(sysctl(ctl_name.as_ptr(), ctl_name.len() as u32, ptr::null_mut(), &mut data_len, ptr::null(), 0))?;
