	...
	S: AsRef<str> + 'a + ?Sized,
{
	...
	let suggestion = crate::lev_distance::find_best_match_for_name_with_substrings(&possibilities, input, None).map(|s| s.to_string());
		...
		let all_cases = [
			(
				...
				vec![
					("a", Some("a"), ""),
					("A", Some("a"), ""),
					("c", None, "Not helpful to suggest an arbitrary choice when none are close"),
					(
			...
			(
				...
				vec![
					...
					("FOO", Some("foo"), ""),
					("FOX", None, ""),
					("ccc", None, "Not helpful to suggest an arbitrary choice when none are close"),
					(
						"zzz",
						None,
