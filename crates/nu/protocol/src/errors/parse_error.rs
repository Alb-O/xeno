...
use crate::ast::RedirectionSource;
use crate::{Span, Type, did_you_mean};
...
pub enum ParseError {
	...
	#[diagnostic(code(nu::parser::extra_token_after_closing_delimiter), help("Try removing them."))]
	...
	RequiredAfterOptional(String, #[label = "required parameter {0} after optional parameter"] Span),
	...
	OutputMismatch(Type, String, #[label("expected {0}, but command outputs {1}")] Span),
	...
	#[diagnostic(code(nu::parser::shell_andand), help("use ';' instead of the shell '&&', or 'and' instead of the boolean '&&'"))]
	ShellAndAnd(#[label("instead of '&&', use ';' or 'and'")] Span),

	#[error("The '||' operator is not supported in Nushell")]
	#[diagnostic(code(nu::parser::shell_oror), help("use 'try' instead of the shell '||', or 'or' instead of the boolean '||'"))]
	...
	#[diagnostic(code(nu::parser::shell_outerr), help("Nushell redirection will write all of stdout before stderr."))]
	...
	MultipleRedirections(RedirectionSource, #[label = "first redirection"] Span, #[label = "second redirection"] Span),
	...
	#[diagnostic(code(nu::parser::unexpected_keyword), help("'{0}' keyword is allowed only in a module."))]
	UnexpectedKeyword(String, #[label("unexpected {0}")] Span),

	#[error("Can't create alias to parser keyword.")]
	#[diagnostic(code(nu::parser::cant_alias_keyword), help("Only the following keywords can be aliased: {0}."))]
	CantAliasKeyword(String, #[label("not supported in alias")] Span),

	#[error("Can't create alias to expression.")]
	#[diagnostic(code(nu::parser::cant_alias_expression), help("Only command calls can be aliased."))]
	CantAliasExpression(String, #[label("aliasing {0} is not supported")] Span),

	#[error("Unknown operator")]
	#[diagnostic(code(nu::parser::unknown_operator), help("{1}"))]
	UnknownOperator(&'static str, &'static str, #[label("Operator '{0}' not supported")] Span),

	#[error("Statement used in pipeline.")]
	#[diagnostic(
		code(nu::parser::unexpected_keyword),
		help("'{0}' keyword is not allowed in pipeline. Use '{0}' by itself, outside of a pipeline.")
	)]
	...
	#[diagnostic(
		...
		help("'{0}' is the name of a builtin Nushell variable and cannot be used as a variable name")
	)]
	...
	#[diagnostic(code(nu::parser::variable_not_valid))]
	AliasNotValid(#[label = "alias name can't be a number, a filesize, or contain a hash # or caret ^"] Span),

	#[error("Command name not supported.")]
	#[diagnostic(code(nu::parser::variable_not_valid))]
	CommandDefNotValid(#[label = "command name can't be a number, a filesize, or contain a hash # or caret ^"] Span),
	...
	ModuleMissingModNuFile(String, #[label = "module directory is missing a mod.nu file"] Span),
	...
	NamedAsModule(String, String, String, #[label = "can't export from module {1}"] Span),
	...
	ModuleDoubleMain(String, #[label = "module '{0}' already contains 'main'"] Span),
	...
	#[diagnostic(code(nu::parser::cant_remove_last_overlay), help("At least one overlay must always be active."))]
	...
	#[diagnostic(code(nu::parser::missing_positional), help("Usage: {2}. Use `--help` for more information."))]
	...
	KeywordMissingArgument(String, String, #[label("missing {0} value that follows {1}")] Span),
	...
	ParameterMismatchType(String, String, String, #[label = "parameter {0} needs to be '{1}' instead of '{2}'"] Span),
	...
	ExtraColumns(usize, #[label("expected {0} column{}", if *.0 == 1 { "" } else { "s" })] Span),

	#[error("Missing columns.")]
	#[diagnostic(code(nu::parser::missing_columns))]
	MissingColumns(usize, #[label("expected {0} column{}", if *.0 == 1 { "" } else { "s" })] Span),
	...
	#[diagnostic(code(nu::parser::sourced_file_not_found), help("sourced files need to be available before your script is run"))]
	...
	#[diagnostic()]
	RedirectingBuiltinCommand(&'static str, #[label("not allowed here")] Span, #[label("...and here")] Option<Span>),
	...
	#[diagnostic(code(nu::parser::assignment_requires_variable), help("try assigning to a variable or a cell path of a variable"))]
