...
use super::shell_error::io::IoError;
...
impl LabeledError {
	...
	///
	/// ```rust
	/// # use xeno_nu_protocol::LabeledError;
	...
	///
	/// ```rust
	/// # use xeno_nu_protocol::{LabeledError, Span};
	...
	pub fn with_label(mut self, text: impl Into<String>, span: Span) -> Self {
		self.labels.push(ErrorLabel { text: text.into(), span });
		self
	}
	...
	///
	/// ```rust
	/// # use xeno_nu_protocol::{LabeledError, ShellError};
	/// let error = LabeledError::new("An error")
	...
	///
	/// ```rust
	/// # use xeno_nu_protocol::{ShellError, LabeledError, shell_error::{self, io::IoError}, Span};
	/// #
...
impl From<ErrorLabel> for LabeledSpan {
	fn from(val: ErrorLabel) -> Self {
		LabeledSpan::new((!val.text.is_empty()).then_some(val.text), val.span.start, val.span.end - val.span.start)
	}
}
...
impl FromValue for ErrorLabel {
	...
	fn expected_type() -> crate::Type {
		Type::Record(vec![("text".into(), Type::String), ("span".into(), Type::record())].into())
	}
}
...
impl From<ErrorSource> for NamedSource<String> {
	fn from(value: ErrorSource) -> Self {
		...
		match value {
			ErrorSource {
				text: Some(text), path: None, ..
			} => NamedSource::new(name, text),
			ErrorSource {
				text: None, path: Some(path), ..
			} => {
...
impl FromValue for ErrorSource {
	fn from_value(v: Value) -> Result<Self, ShellError> {
		let record = v.clone().into_record()?;
		let name = record.get("name").and_then(|s| String::from_value(s.clone()).ok());
		...
		match (text, path) {
			...
			(text @ Some(_), _) => Ok(ErrorSource { name, text, path: None }),
	...
	fn expected_type() -> crate::Type {
		Type::Record(vec![("name".into(), Type::String), ("text".into(), Type::String), ("path".into(), Type::String)].into())
	}
}
...
impl Diagnostic for LabeledError {
	...
	fn labels(&self) -> Option<Box<dyn Iterator<Item = miette::LabeledSpan> + '_>> {
		Some(Box::new(self.labels.iter().map(|label| label.clone().into())))
	}
