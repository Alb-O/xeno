...
/// already part of the [`xeno_nu_protocol`](crate) crate and can therefore interact with
...
/// ## User Input Error
/// ```rust
/// # use xeno_nu_protocol::shell_error::io::{IoError, ErrorKind};
/// # use xeno_nu_protocol::Span;
...
/// ```rust
/// # use xeno_nu_protocol::shell_error::io::{IoError, ErrorKind};
/// # use xeno_nu_protocol::{Span, ShellError};
/// use std::path::PathBuf;
///
...
pub enum ErrorKind {
	...
	///
	/// ```rust
	/// # use xeno_nu_protocol::shell_error::io::ErrorKind;
	/// #
...
impl IoError {
	...
	/// - `new_internal` if no path is available.
	/// - `new_internal_with_path` if a path is available.
	pub fn new_with_additional_context(kind: impl Into<ErrorKind>, span: Span, path: impl Into<Option<PathBuf>>, additional_context: impl ToString) -> Self {
		let path = path.into();
	...
	///   the issue.
	///   Use the [`xeno_nu_protocol::location!`](crate::location) macro to generate the location
	///   information.
	///
	/// # Examples
	/// ```rust
	/// use xeno_nu_protocol::shell_error::{self, io::IoError};
	///
	/// let error = IoError::new_internal(
	...
	/// );
	/// ```
	pub fn new_internal(kind: impl Into<ErrorKind>, additional_context: impl ToString, location: Location) -> Self {
		Self {
			kind: kind.into(),
			span: Span::unknown(),
			path: None,
	...
	/// );
	/// ```
	pub fn new_internal_with_path(kind: impl Into<ErrorKind>, additional_context: impl ToString, location: Location, path: PathBuf) -> Self {
		Self {
			kind: kind.into(),
			span: Span::unknown(),
			path: path.into(),
...
suspicious conversion:
    ...
    into `xeno_nu_protocol::shell_error::io::ErrorKind`
...
impl Diagnostic for IoError {
	...
	fn help<'a>(&'a self) -> Option<Box<dyn std::fmt::Display + 'a>> {
		let make_msg = |path: &Path| {
			...
			match self.kind {
				...
				ErrorKind::Std(std::io::ErrorKind::NotFound, _) | ErrorKind::FileNotFound | ErrorKind::DirectoryNotFound => format!("{path} does not exist"),
				_ => format!("The error occurred at {path}"),
			}
		};

		self.path.as_ref().map(|path| make_msg(path)).map(|s| Box::new(s) as Box<dyn std::fmt::Display>)
	}
	...
	fn diagnostic_source(&self) -> Option<&dyn Diagnostic> {
		self.additional_context.as_ref().map(|ctx| ctx as &dyn Diagnostic)
	}
...
impl IoErrorExt for ErrorKind {
	fn not_found_as(self, kind: NotFound) -> ErrorKind {
		match (kind, self) {
			...
			(NotFound::Directory, Self::Std(std::io::ErrorKind::NotFound, _)) => ErrorKind::DirectoryNotFound,
			_ => self,
		}
	}
}
