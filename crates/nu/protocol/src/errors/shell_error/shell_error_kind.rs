...
pub enum ShellError {
	...
	#[diagnostic(
		...
		help(r#"The environment variable '{envvar_name}' is set automatically by Nushell and cannot be set manually."#)
	)]
	...
	#[diagnostic(code(nu::shell::cannot_replace_env), help(r#"Assigning a value to '$env' is not allowed."#))]
	CannotReplaceEnv {
 ...
 * "Tue, 1 Jul 2003 10:52:37 +0200""#
	...
	#[diagnostic(code(nu::parser::non_utf8), help("see `decode` for handling character sets other than UTF-8"))]
	NonUtf8 {
	...
	/// Check your input's encoding. Are there any funny characters/bytes?
	#[error("Non-UTF8 string")]
	#[diagnostic(code(nu::parser::non_utf8_custom), help("see `decode` for handling character sets other than UTF-8"))]
	NonUtf8Custom {
		msg: String,
		#[label("{msg}")]
		span: Span,
	...
	#[diagnostic(code(nu::shell::match_guard_not_bool), help("Match guards should evaluate to a boolean"))]
	MatchGuardNotBool {
...
This is an internal Nushell error, please file an issue https://github.com/nushell/nushell/issues."
	...
	#[diagnostic(
		...
		help("This expression is supposed to be evaluated into a constant, which means error-free.")
	)]
	...
	#[diagnostic(code(nu::shell::not_a_const_command), help("Only a subset of builtin commands can run at parse time."))]
	NotAConstCommand {
	...
	#[diagnostic(code(nu::shell::not_a_const_help), help("Help messages are currently not supported to be constants."))]
	NotAConstHelp {
		#[label("This command cannot run at parse time.")]
		span: Span,
	},
	...
	#[diagnostic(code(nu::shell::invalid_glob_pattern), help("Refer to xxx for help on nushell glob patterns."))]
	InvalidGlobPattern {
		msg: String,
		#[label("{msg}")]
		span: Span,
	...
	/// Correct unit
	#[error("Invalid unit")]
	#[diagnostic(code(nu::shell::invalid_unit), help("Supported units are: {supported_units}"))]
	InvalidUnit {
	...
	#[error("The selected range {left_flank}..{right_flank} is out of the bounds of the provided input")]
