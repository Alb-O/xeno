...
use miette::{LabeledSpan, MietteHandlerOpts, NarratableReportHandler, ReportHandler, RgbColors, Severity, SourceCode};
use serde::{Deserialize, Serialize};
use thiserror::Error;

use crate::engine::{EngineState, Stack, StateWorkingSet};
use crate::{CompileError, Config, ErrorStyle, ParseError, ParseWarning, ShellError, ShellWarning, ShortReportHandler};
	...
	R: Reportable + Hash,
{
	match reportable.report_mode() {
		...
		ReportMode::FirstUse => {
			...
			let mut report_log = engine_state.report_log.lock().expect("report log lock is poisoned");
...
pub fn format_cli_error(stack: Option<&Stack>, working_set: &StateWorkingSet, error: &dyn miette::Diagnostic, default_code: Option<&'static str>) -> String {
	format!("Error: {:?}", CliError::new(stack, error, working_set, default_code))
}
...
pub fn report_shell_warning(stack: Option<&Stack>, engine_state: &EngineState, warning: &ShellWarning) {
	if should_show_reportable(engine_state, warning) {
		report_warning(stack, &StateWorkingSet::new(engine_state), warning, "nu::shell::warning");
	}
}

pub fn report_parse_error(stack: Option<&Stack>, working_set: &StateWorkingSet, error: &ParseError) {
	report_error(stack, working_set, error, "nu::parser::error");
}

pub fn report_parse_warning(stack: Option<&Stack>, working_set: &StateWorkingSet, warning: &ParseWarning) {
	if should_show_reportable(working_set.permanent(), warning) {
		report_warning(stack, working_set, warning, "nu::parser::warning");
	}
}

pub fn report_compile_error(stack: Option<&Stack>, working_set: &StateWorkingSet, error: &CompileError) {
	report_error(stack, working_set, error, "nu::compile::error");
}

pub fn report_experimental_option_warning(stack: Option<&Stack>, working_set: &StateWorkingSet, warning: &dyn miette::Diagnostic) {
	report_warning(stack, working_set, warning, "nu::experimental_option::warning");
}

fn report_error(stack: Option<&Stack>, working_set: &StateWorkingSet, error: &dyn miette::Diagnostic, default_code: &'static str) {
	eprintln!("Error: {:?}", CliError::new(stack, error, working_set, Some(default_code)));
	// reset vt processing, aka ansi because illbehaved externals can break it
	#[cfg(windows)]
	{
		let _ = xeno_nu_utils::enable_vt_processing();
	}
}

fn report_warning(stack: Option<&Stack>, working_set: &StateWorkingSet, warning: &dyn miette::Diagnostic, default_code: &'static str) {
	eprintln!("Warning: {:?}", CliError::new(stack, warning, working_set, Some(default_code)));
	// reset vt processing, aka ansi because illbehaved externals can break it
	#[cfg(windows)]
	{
		let _ = xeno_nu_utils::enable_vt_processing();
...
fn get_config<'a>(stack: Option<&'a Stack>, engine_state: &'a EngineState) -> &'a Config {
	stack.and_then(|s| s.config.as_deref()).unwrap_or(engine_state.get_config())
}
	...
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		...
		let miette_handler: Box<dyn ReportHandler> = match error_style {
			...
			style => {
				...
				match style {
					ErrorStyle::Nested => Box::new(handler.show_related_errors_as_nested().with_cause_chain().build()),
...
impl miette::Diagnostic for CliError<'_> {
	fn code<'a>(&'a self) -> Option<Box<dyn std::fmt::Display + 'a>> {
		self.diagnostic
			.code()
			.or_else(|| self.default_code.map(|code| Box::new(code) as Box<dyn std::fmt::Display>))
	}
