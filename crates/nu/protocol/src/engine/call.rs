...
use crate::{DeclId, FromValue, ShellError, Span, Value, ir};
...
impl Call<'_> {
	...
	pub fn has_flag_const(&self, working_set: &StateWorkingSet, flag_name: &str) -> Result<bool, ShellError> {
		self.assert_ast_call()?.has_flag_const(working_set, flag_name)
	}
	...
	pub fn rest_const<T: FromValue>(&self, working_set: &StateWorkingSet, starting_pos: usize) -> Result<Vec<T>, ShellError> {
		self.assert_ast_call()?.rest_const(working_set, starting_pos)
	}
	...
	pub fn rest_iter_flattened(
		...
		stack: &mut Stack,
		eval_expression: fn(&EngineState, &mut Stack, &ast::Expression) -> Result<Value, ShellError>,
		starting_pos: usize,
		...
		fn by_ast(
			...
			call.rest_iter_flattened(starting_pos, |expr| eval_expression(engine_state, stack, expr))
		}

		fn by_ir(call: &ir::Call, stack: &Stack, starting_pos: usize) -> Result<Vec<Value>, ShellError> {
			call.rest_iter_flattened(stack, starting_pos)
		}

		match &self.inner {
			CallImpl::AstRef(call) => by_ast(call, engine_state, stack, eval_expression, starting_pos),
			CallImpl::AstBox(call) => by_ast(call, engine_state, stack, eval_expression, starting_pos),
