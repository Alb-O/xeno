...
impl EngineState {
	...
	pub fn merge_delta(&mut self, mut delta: StateDelta) -> Result<(), ShellError> {
		...
		if !delta.plugins.is_empty() {
			for plugin in std::mem::take(&mut delta.plugins) {
				...
				if let Some(existing) = self.plugins.iter_mut().find(|p| p.identity().name() == plugin.identity().name()) {
	...
	fn update_plugin_file(&self, updated_items: Vec<PluginRegistryItem>) -> Result<(), ShellError> {
		...
		let plugin_path = self.plugin_path.as_ref().ok_or_else(|| ShellError::GenericError {
		...
		let plugin_file = File::create(plugin_path.as_path())
			.map_err(|err| IoError::new_internal_with_path(err, "Failed to write plugin file", crate::location!(), PathBuf::from(plugin_path)))?;
	...
	pub fn plugin_decls(&self) -> impl Iterator<Item = &Box<dyn Command + 'static>> {
		...
		let mut plugin_decls: Vec<(&str, &Box<dyn Command>)> = unique_plugin_decls.into_iter().collect();
