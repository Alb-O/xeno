...
pub enum Argument {
	...
	Positional { span: Span, val: Value, ast: Option<Arc<Expression>> },
	/// A spread argument, e.g. `...$args`
	Spread { span: Span, vals: Value, ast: Option<Arc<Expression>> },
...
impl ArgumentStack {
	...
	pub fn get_len(&self, base: usize) -> usize {
		self.arguments.len().checked_sub(base).unwrap_or_else(|| {
			panic!("base ({}) is beyond the end of the arguments stack ({})", base, self.arguments.len());
		})
	}
