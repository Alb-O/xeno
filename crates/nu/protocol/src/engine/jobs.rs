...
impl Jobs {
	...
	pub fn kill_all(&mut self) -> shell_error::io::Result<()> {
		...
		let first_err = self.iter().map(|(_, job)| job.kill().err()).fold(None, |acc, x| acc.or(x));
...
impl Mailbox {
	...
	#[cfg(not(target_family = "wasm"))]
	pub fn recv_timeout(&mut self, filter_tag: Option<FilterTag>, timeout: Duration) -> Result<PipelineData, RecvTimeoutError> {
	...
	#[cfg(not(target_family = "wasm"))]
	pub fn try_recv(&mut self, filter_tag: Option<FilterTag>) -> Result<PipelineData, TryRecvError> {
		if let Some(value) = self.ignored_mail.pop(filter_tag) {
			Ok(value)
		} else {
			loop {
