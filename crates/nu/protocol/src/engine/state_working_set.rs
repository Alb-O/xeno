...
impl<'a> StateWorkingSet<'a> {
	...
	pub fn unique_overlay_names(&self) -> HashSet<&[u8]> {
		...
		for scope_frame in self.delta.scope.iter().rev() {
			for overlay_id in scope_frame.active_overlays.iter().rev() {
				let (overlay_name, _) = scope_frame.overlays.get(overlay_id.get()).expect("internal error: missing overlay");
	...
	pub fn add_predecl(&mut self, decl: Box<dyn Command>) -> Option<DeclId> {
		...
		self.delta.last_scope_frame_mut().predecls.insert(name, decl_id)
	}
	...
	pub fn hide_decl(&mut self, name: &[u8]) -> Option<DeclId> {
		...
		for scope_frame in self.delta.scope.iter_mut().rev() {
			for overlay_id in scope_frame.active_overlay_ids(&mut removed_overlays).iter().rev() {
		...
		for overlay_frame in self.permanent_state.active_overlays(&removed_overlays).rev() {
			visibility.append(&overlay_frame.visibility);
	...
	pub fn add_block(&mut self, block: Arc<Block>) -> BlockId {
		log::trace!("block id={} added, has IR = {:?}", self.num_blocks(), block.ir_block.is_some());
	...
	pub fn get_contents_of_file(&self, file_id: FileId) -> Option<&[u8]> {
		...
		if let Some(cached_file) = self.delta.get_file_contents().get(file_id.get() - self.permanent_state.num_files()) {
			return Some(&cached_file.content);
		}
		...
		if permanent_end <= span.start {
			for cached_file in &self.delta.files {
				if cached_file.covered_span.contains_span(span) {
					return &cached_file.content[span.start - cached_file.covered_span.start..span.end - cached_file.covered_span.start];
				}
			}
		}
	...
	pub fn find_decl_name(&self, decl_id: DeclId) -> Option<&[u8]> {
		...
		// check overlay in perma
		self.permanent_state.find_decl_name(decl_id, &removed_overlays)
	}
	...
	pub fn add_variable(&mut self, mut name: Vec<u8>, span: Span, ty: Type, mutable: bool) -> VarId {
	...
	pub fn find_commands_by_predicate(
		...
		self.traverse_commands(|name, decl_id| {
			...
			output.push((decl_id, name.to_vec(), Some(command.description().to_string()), command.command_type()));
		});
	...
	pub fn find_overlay(&self, name: &[u8]) -> Option<&OverlayFrame> {
		...
		self.permanent_state.find_overlay(name).map(|id| self.permanent_state.get_overlay(id))
	}
	...
	pub fn last_overlay_name(&self) -> &[u8] {
		...
		for scope_frame in self.delta.scope.iter().rev() {
			if let Some(last_name) = scope_frame.active_overlay_names(&mut removed_overlays).iter().rev().next_back() {
				return last_name;
			}
		}
	...
	pub fn last_overlay(&self) -> &OverlayFrame {
		...
		for scope_frame in self.delta.scope.iter().rev() {
			if let Some(last_overlay) = scope_frame.active_overlays(&mut removed_overlays).rev().next_back() {
				return last_overlay;
			}
		}
	...
	pub fn last_overlay_mut(&mut self) -> &mut OverlayFrame {
		if self.delta.last_overlay_mut().is_none() {
			...
			self.add_overlay(name, origin, ResolvedImportPattern::new(vec![], vec![], vec![], vec![]), prefixed);
		}

		self.delta.last_overlay_mut().expect("internal error: missing added overlay")
	}
	...
	pub fn add_overlay(&mut self, name: Vec<u8>, origin: ModuleId, definitions: ResolvedImportPattern, prefixed: bool) {
		let last_scope_frame = self.delta.last_scope_frame_mut();

		last_scope_frame.removed_overlays.retain(|removed_name| removed_name != &name);
		...
		let overlay_id = if let Some(overlay_id) = last_scope_frame.find_overlay(&name) {
			...
			last_scope_frame.overlays.push((name, OverlayFrame::from_origin(origin, prefixed)));
		...
		for (name, const_val) in definitions.constant_values {
			let const_var_id = self.add_variable(name.clone(), Span::unknown(), const_val.get_type(), false);
	...
	pub fn remove_overlay(&mut self, name: &[u8], keep_custom: bool) {
		...
		let maybe_module_id = if let Some(overlay_id) = last_scope_frame.find_overlay(name) {
			...
			self.permanent_state.find_overlay(name).map(|id| self.permanent_state.get_overlay(id).origin)
		};
		...
		if let Some(module_id) = maybe_module_id {
			...
			if keep_custom {
				...
				let decls = self.decls_of_overlay(name).into_iter().filter(|(n, _)| !origin_module.has_decl(n)).collect();
...
impl<'a> GetSpan for &'a StateWorkingSet<'a> {
	fn get_span(&self, span_id: SpanId) -> Span {
		...
		if span_id.get() < num_permanent_spans {
			...
			*self.delta.spans.get(span_id.get() - num_permanent_spans).expect("internal error: missing span")
		}
	}
}
...
impl miette::SourceCode for &StateWorkingSet<'_> {
	fn read_span<'b>(
		...
		for cached_file in self.files() {
			let (filename, start, end) = (&cached_file.name, cached_file.covered_span.start, cached_file.covered_span.end);
			if debugging {
			...
			if span.offset() >= start && span.offset() + span.len() <= end {
				...
				let span_contents = span_contents.read_span(&local_span, context_lines_before, context_lines_after)?;
