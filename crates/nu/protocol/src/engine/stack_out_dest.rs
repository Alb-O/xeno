...
impl<'a> StackIoGuard<'a> {
	pub(crate) fn new(stack: &'a mut Stack, stdout: Option<Redirection>, stderr: Option<Redirection>) -> Self {
		...
		let (old_pipe_stdout, old_parent_stdout) = match stdout {
			...
			Some(Redirection::File(file)) => {
				...
				(out_dest.pipe_stdout.replace(file.clone()), out_dest.push_stdout(file))
			}
		...
		let (old_pipe_stderr, old_parent_stderr) = match stderr {
			...
			Some(Redirection::File(file)) => (out_dest.pipe_stderr.take(), out_dest.push_stderr(file.into())),
