...
use crate::engine::{
	ArgumentStack, DEFAULT_OVERLAY_NAME, EngineState, ErrorHandlerStack, Redirection, StackCallArgGuard, StackCollectValueGuard, StackIoGuard, StackOutDest,
};
use crate::{Config, ENV_VARIABLE_ID, IntoValue, NU_VARIABLE_ID, OutDest, ShellError, Span, Value, VarId, report_shell_warning};
...
impl Stack {
	...
	pub fn with_changes_from_child(parent: Arc<Stack>, child: Stack) -> Stack {
		...
		unique_stack.vars.retain(|(var, _)| !child.parent_deletions.contains(var));
	...
	pub fn with_env(&mut self, env_vars: &[Arc<EnvVars>], env_hidden: &Arc<HashMap<String, HashSet<String>>>) {
	...
	pub fn get_config(&self, engine_state: &EngineState) -> Arc<Config> {
		self.config.clone().unwrap_or_else(|| engine_state.config.clone())
	}
	...
	pub fn add_env_var(&mut self, var: String, value: Value) {
		if let Some(last_overlay) = self.active_overlays.last() {
			...
			if let Some(scope) = self.env_vars.last_mut() {
				...
				self.env_vars
					.push(Arc::new([(last_overlay.into(), [(var, value)].into_iter().collect())].into_iter().collect()));
			}
		} else {
			// TODO: Remove panic
	...
	pub fn last_overlay_name(&self) -> Result<String, ShellError> {
		self.active_overlays.last().cloned().ok_or_else(|| ShellError::NushellFailed {
			msg: "No active overlay".into(),
		})
	}

	pub fn captures_to_stack(&self, captures: Vec<(VarId, Value)>) -> Stack {
		self.captures_to_stack_preserve_out_dest(captures).collect_value()
	}
		...
		for overlay_name in self.active_overlays.iter().rev() {
			...
			if let Some(env_names) = self.env_hidden.get(overlay_name) {
				for n in env_names {
					...
					// get env value.
					if let Some(Some(v)) = engine_state.env_vars.get(overlay_name).map(|env_vars| env_vars.get(n)) {
	...
	pub fn get_env_var_insensitive<'a>(&'a self, engine_state: &'a EngineState, name: &str) -> Option<(&'a String, &'a Value)> {
		for scope in self.env_vars.iter().rev() {
			for active_overlay in self.active_overlays.iter().rev() {
				if let Some(env_vars) = scope.get(active_overlay)
	...
	/// # use xeno_nu_protocol::engine::Stack;
	/// #
		...
		if !path.is_absolute() {
			if let Some(Component::Prefix(_)) = path.components().next() {
				return Err(ShellError::GenericError {
					...
					msg: "".into(),
					span: None,
					help: Some(format!("Try to use {}{MAIN_SEPARATOR} instead", path.display())),
					inner: vec![],
				});
			}
			...
			let path = xeno_nu_path::strip_trailing_slash(path);
...
mod test {
	...
	fn test_children_see_inner_values() {
		...
		assert_eq!(cloned.get_var(VarId::new(0), Span::test_data()), Ok(Value::test_string("hello")));
	}
	...
	fn test_children_dont_see_deleted_values() {
		...
		assert_eq!(
			...
			Err(crate::ShellError::VariableNotFoundAtRuntime { span: Span::test_data() })
		);
	}
	...
	fn test_children_changes_override_parent() {
		...
		assert_eq!(cloned.get_var(VarId::new(0), Span::test_data()), Ok(Value::test_string("there")));
	...
	fn test_children_changes_persist_in_offspring() {
		...
		assert_eq!(cloned.get_var(VarId::new(1), Span::test_data()), Ok(Value::test_string("there")));
	}
	...
	fn test_merging_children_back_to_parent() {
		...
		cloned.add_env_var("ADDED_IN_CHILD".to_string(), Value::test_string("New Env Var"));
		...
		assert_eq!(original.get_var(VarId::new(1), Span::test_data()), Ok(Value::test_string("there")));

		assert_eq!(
			original.get_env_var(&engine_state, "ADDED_IN_CHILD").cloned(),
