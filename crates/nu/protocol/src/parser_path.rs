...
impl ParserPath {
	...
	pub fn read_dir(&self) -> Option<Vec<ParserPath>> {
		match self {
			ParserPath::RealPath(p) => p
				.read_dir()
				.ok()
				.map(|read_dir| read_dir.flatten().map(|dir_entry| ParserPath::RealPath(dir_entry.path())).collect()),
	...
	pub fn open<'a>(&'a self, working_set: &'a StateWorkingSet) -> std::io::Result<Box<dyn std::io::Read + 'a>> {
		match self {
			ParserPath::RealPath(p) => std::fs::File::open(p).map(|f| Box::new(f) as Box<dyn std::io::Read>),
	...
	pub fn from_virtual_path(working_set: &StateWorkingSet, name: &str, virtual_path: &VirtualPath) -> Self {
		match virtual_path {
