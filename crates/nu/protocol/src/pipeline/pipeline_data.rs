...
use crate::ast::{Call, PathMember};
use crate::engine::{EngineState, Stack};
#[cfg(feature = "os")]
use crate::process::ExitStatusGuard;
use crate::shell_error::io::IoError;
use crate::shell_error::location::Location;
use crate::{
	ByteStream, ByteStreamSource, ByteStreamType, Config, ListStream, OutDest, PipelineMetadata, Range, ShellError, Signals, Span, Type, Value, location,
};
...
impl PipelineData {
	...
	pub fn metadata(&self) -> Option<PipelineMetadata> {
		match self {
			...
			PipelineData::Value(_, meta) | PipelineData::ListStream(_, meta) | PipelineData::ByteStream(_, meta) => meta.clone(),
		}
	}
	...
	pub fn set_metadata(mut self, metadata: Option<PipelineMetadata>) -> Self {
		match &mut self {
			PipelineData::Empty => {}
			PipelineData::Value(_, meta) | PipelineData::ListStream(_, meta) | PipelineData::ByteStream(_, meta) => *meta = metadata,
		}
		self
	}

	pub fn is_nothing(&self) -> bool {
		matches!(self, PipelineData::Value(Value::Nothing { .. }, ..)) || matches!(self, PipelineData::Empty)
	}
	...
	pub fn with_span(self, span: Span) -> Self {
		match self {
			...
			PipelineData::Value(value, metadata) => PipelineData::value(value.with_span(span), metadata),
			PipelineData::ListStream(stream, metadata) => PipelineData::list_stream(stream.with_span(span), metadata),
			PipelineData::ByteStream(stream, metadata) => PipelineData::byte_stream(stream.with_span(span), metadata),
		}
	}
	...
	pub fn is_subtype_of(&self, other: &Type) -> bool {
		match (self, other) {
			...
			(PipelineData::ByteStream(stream, ..), Type::String) if stream.type_().is_string_coercible() => true,
			(PipelineData::ByteStream(stream, ..), Type::Binary) if stream.type_().is_binary_coercible() => true,
	...
	pub fn try_into_stream(self, engine_state: &EngineState) -> Result<PipelineData, PipelineData> {
		...
		match self {
			...
			PipelineData::Value(Value::String { val, .. }, metadata) => Ok(PipelineData::byte_stream(
				ByteStream::read_string(val, span, engine_state.signals().clone()),
				metadata,
			)),
			PipelineData::Value(Value::Binary { val, .. }, metadata) => Ok(PipelineData::byte_stream(
		...
		match self {
			...
			PipelineData::Value(value, ..) => {
				...
				dest.write_all(&bytes)
					.map_err(|err| IoError::new_internal(err, "Could not write PipelineData to dest", crate::location!()))?;
				dest.flush()
					.map_err(|err| IoError::new_internal(err, "Could not flush PipelineData to dest", crate::location!()))?;
				Ok(())
			}
			PipelineData::ListStream(stream, ..) => {
				for value in stream {
					...
					dest.write_all(&bytes)
						.map_err(|err| IoError::new_internal(err, "Could not write PipelineData to dest", crate::location!()))?;
					dest.write_all(b"\n")
						.map_err(|err| IoError::new_internal(err, "Could not write linebreak after PipelineData to dest", crate::location!()))?;
				}
				dest.flush()
					.map_err(|err| IoError::new_internal(err, "Could not flush PipelineData to dest", crate::location!()))?;
				Ok(())
	...
	pub fn into_iter_strict(self, span: Span) -> Result<PipelineIterator, ShellError> {
		Ok(PipelineIterator(match self {
			PipelineData::Value(value, ..) => {
				...
				match value {
					Value::List { vals, .. } => PipelineIteratorInner::ListStream(ListStream::new(vals.into_iter(), val_span, Signals::empty()).into_iter()),
					Value::Binary { val, .. } => PipelineIteratorInner::ListStream(
						ListStream::new(val.into_iter().map(move |x| Value::int(x as i64, val_span)), val_span, Signals::empty()).into_iter(),
					),
					Value::Range { val, .. } => PipelineIteratorInner::ListStream(
						ListStream::new(val.into_range_iter(val_span, Signals::empty()), val_span, Signals::empty()).into_iter(),
					),
					// Handle iterable custom values by converting to base value first
					Value::Custom { ref val, .. } if val.is_iterable() => match val.to_base_value(val_span) {
						Ok(Value::List { vals, .. }) => {
							PipelineIteratorInner::ListStream(ListStream::new(vals.into_iter(), val_span, Signals::empty()).into_iter())
						}
						Ok(other) => {
			...
			PipelineData::ListStream(stream, ..) => PipelineIteratorInner::ListStream(stream.into_iter()),
			PipelineData::Empty => {
				return Err(ShellError::OnlySupportsThisInputType {
					exp_input_type: "list, binary, range, or byte stream".into(),
	...
	pub fn follow_cell_path(self, cell_path: &[PathMember], head: Span) -> Result<Value, ShellError> {
		match self {
			...
			PipelineData::ListStream(stream, ..) => Value::list(stream.into_iter().collect(), head).follow_cell_path(cell_path).map(Cow::into_owned),
		...
		match self {
			PipelineData::Value(value, metadata) => {
				...
				let pipeline = match value {
					Value::List { vals, .. } => vals.into_iter().map(f).into_pipeline_data(span, signals.clone()),
					Value::Range { val, .. } => val.into_range_iter(span, Signals::empty()).map(f).into_pipeline_data(span, signals.clone()),
					value => match f(value) {
						Value::Error { error, .. } => return Err(*error),
			...
			PipelineData::ListStream(stream, metadata) => Ok(PipelineData::list_stream(stream.map(f), metadata)),
			PipelineData::ByteStream(stream, metadata) => Ok(f(stream.into_value()?).into_pipeline_data_with_metadata(metadata)),
		}
	}
		...
		F: FnMut(Value) -> U + 'static + Send,
	{
		match self {
			...
			PipelineData::Value(value, metadata) => {
				let span = value.span();
				let pipeline = match value {
					Value::List { vals, .. } => vals.into_iter().flat_map(f).into_pipeline_data(span, signals.clone()),
					Value::Range { val, .. } => val
						.into_range_iter(span, Signals::empty())
						.flat_map(f)
						.into_pipeline_data(span, signals.clone()),
					value => f(value).into_iter().into_pipeline_data(span, signals.clone()),
				};
				Ok(pipeline.set_metadata(metadata))
			}
			PipelineData::ListStream(stream, metadata) => Ok(PipelineData::list_stream(stream.modify(|iter| iter.flat_map(f)), metadata)),
			PipelineData::ByteStream(stream, metadata) => {
				...
				Ok(iter.into_iter().into_pipeline_data_with_metadata(span, signals.clone(), metadata))
			}
		}
	}
		...
		F: FnMut(&Value) -> bool + 'static + Send,
	{
		match self {
			...
			PipelineData::Value(value, metadata) => {
				let span = value.span();
				let pipeline = match value {
					Value::List { vals, .. } => vals.into_iter().filter(f).into_pipeline_data(span, signals.clone()),
					Value::Range { val, .. } => val.into_range_iter(span, Signals::empty()).filter(f).into_pipeline_data(span, signals.clone()),
					value => {
						if f(&value) {
			...
			PipelineData::ListStream(stream, metadata) => Ok(PipelineData::list_stream(stream.modify(|iter| iter.filter(f)), metadata)),
			PipelineData::ByteStream(stream, metadata) => {
				// TODO: is this behavior desired / correct ?
				let span = stream.span();
				...
				let value = if f(&value) { value } else { Value::nothing(span) };
			...
			PipelineData::Value(v, metadata) => {
				...
				match v {
					Value::Range { val, .. } => {
						...
						let range_values: Vec<Value> = val.into_range_iter(span, Signals::empty()).collect();
	...
	pub fn print_table(self, engine_state: &EngineState, stack: &mut Stack, no_newline: bool, to_stderr: bool) -> Result<(), ShellError> {
		match self {
			// Print byte streams directly as long as they aren't binary.
			PipelineData::ByteStream(stream, ..) if stream.type_() != ByteStreamType::Binary => stream.print(to_stderr),
			_ => {
	...
	pub fn print_raw(self, engine_state: &EngineState, no_newline: bool, to_stderr: bool) -> Result<(), ShellError> {
		...
		if let PipelineData::Value(Value::Binary { val: bytes, .. }, _) = self {
			if to_stderr {
				write_all_and_flush(bytes, &mut std::io::stderr().lock(), "stderr", span, engine_state.signals())?;
			} else {
				write_all_and_flush(bytes, &mut std::io::stdout().lock(), "stdout", span, engine_state.signals())?;
			}
			Ok(())
		} else {
			self.write_all_and_flush(engine_state, no_newline, to_stderr)
	...
	fn write_all_and_flush(self, engine_state: &EngineState, no_newline: bool, to_stderr: bool) -> Result<(), ShellError> {
		let span = self.span();
		if let PipelineData::ByteStream(stream, ..) = self {
			...
			for item in self {
				...
				if to_stderr {
					write_all_and_flush(out, &mut std::io::stderr().lock(), "stderr", span, engine_state.signals())?;
				} else {
					write_all_and_flush(out, &mut std::io::stdout().lock(), "stdout", span, engine_state.signals())?;
				}
			}
	...
	T: AsRef<[u8]>,
{
	...
	for chunk in data.as_ref().chunks(OUTPUT_CHUNK_SIZE) {
		signals.check(&span)?;
		destination.write_all(chunk).map_err(|err| io_error_map(err, location!()))?;
	}
...
impl IntoIterator for PipelineData {
	...
	fn into_iter(self) -> Self::IntoIter {
		PipelineIterator(match self {
			...
			PipelineData::Value(value, ..) => {
				...
				match value {
					Value::List { vals, signals, .. } => {
						PipelineIteratorInner::ListStream(ListStream::new(vals.into_iter(), span, signals.unwrap_or_else(Signals::empty)).into_iter())
					}
					Value::Range { val, signals, .. } => PipelineIteratorInner::ListStream(
						ListStream::new(val.into_range_iter(span, signals.unwrap_or_else(Signals::empty)), span, Signals::empty()).into_iter(),
					),
					// Handle iterable custom values by converting to base value first
					Value::Custom { ref val, .. } if val.is_iterable() => match val.to_base_value(span) {
			...
			PipelineData::ListStream(stream, ..) => PipelineIteratorInner::ListStream(stream.into_iter()),
			PipelineData::ByteStream(stream, ..) => stream.chunks().map_or(PipelineIteratorInner::Empty, PipelineIteratorInner::ByteStream),
		})
	}
}
	...
	<I::IntoIter as Iterator>::Item: Into<Value>,
{
	...
	fn into_pipeline_data_with_metadata(self, span: Span, signals: Signals, metadata: impl Into<Option<PipelineMetadata>>) -> PipelineData {
		PipelineData::list_stream(ListStream::new(self.into_iter().map(Into::into), span, signals), metadata.into())
	}
}
...
impl From<PipelineData> for PipelineExecutionData {
	...
	fn from(value: PipelineData) -> Self {
		...
		let exit_status_future = value.clone_exit_status_future().map(|f| f.with_span(value_span));
		Self {
			body: value,
