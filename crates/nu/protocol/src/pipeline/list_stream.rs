...
impl ListStream {
	...
	pub fn new(iter: impl Iterator<Item = Value> + Send + 'static, span: Span, signals: Signals) -> Self {
		Self {
	...
	pub fn into_value(self) -> Result<Value, ShellError> {
		Ok(Value::list(self.stream.map(Value::unwrap_error).collect::<Result<_, _>>()?, self.span))
	}
	...
	///
	/// ```
	/// use xeno_nu_protocol::{ListStream, Signals, Span, Value};
	///
...
impl IntoIterator for ListStream {
	...
	fn into_iter(self) -> Self::IntoIter {
		IntoIter { stream: self.into_inner() }
	}
}
...
impl<I: Iterator> Iterator for InterruptIter<I> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		if self.signals.interrupted() { None } else { self.iter.next() }
	}
