...
use crate::ast::ImportPatternMember;
use crate::engine::StateWorkingSet;
use crate::parser_path::ParserPath;
use crate::{BlockId, DeclId, FileId, ModuleId, ParseError, Span, Value, VarId};
...
impl Module {
	...
	pub fn resolve_import_pattern(
		...
		let (head, rest) = if let Some((head, rest)) = members.split_first() {
			...
			for (_, id) in &self.submodules {
				...
				let (sub_results, sub_errors) = submodule.resolve_import_pattern(working_set, *id, &[], None, span, imported_modules);
			...
			return (
				ResolvedImportPattern::new(decls, vec![(final_name.clone(), self_id)], vec![], constant_values),
				errors,
			);
		};
		...
		match head {
			ImportPatternMember::Name { name, span } => {
				...
				if name == b"main" {
					if let Some(main_decl_id) = self.main {
						(ResolvedImportPattern::new(vec![(final_name, main_decl_id)], vec![], vec![], vec![]), errors)
					} else {
						(
					...
					(ResolvedImportPattern::new(vec![(name.clone(), *decl_id)], vec![], vec![], vec![]), errors)
				} else if let Some(var_id) = self.constants.get(name) {
					match working_set.get_constant(*var_id) {
						Ok(_) => (ResolvedImportPattern::new(vec![], vec![], vec![(name.clone(), *var_id)], vec![]), errors),
						Err(err) => (ResolvedImportPattern::new(vec![], vec![], vec![], vec![]), vec![err]),
					}
				} else if let Some(submodule_id) = self.submodules.get(name) {
					let submodule = working_set.get_module(*submodule_id);
					submodule.resolve_import_pattern(working_set, *submodule_id, rest, None, self.span.unwrap_or(backup_span), imported_modules)
				} else {
					(
						ResolvedImportPattern::new(vec![], vec![], vec![], vec![]),
						vec![ParseError::ExportNotFound(*span)],
			...
			ImportPatternMember::Glob { .. } => {
				...
				for (_, id) in &self.submodules {
					...
					let (sub_results, sub_errors) =
						submodule.resolve_import_pattern(working_set, *id, &[], None, self.span.unwrap_or(backup_span), imported_modules);
				...
				(ResolvedImportPattern::new(decls, submodules, constants, constant_values), errors)
			}
			ImportPatternMember::List { names } => {
				...
				(ResolvedImportPattern::new(decls, modules, constants, constant_values), errors)
			}
		}
	}
	...
	pub fn consts(&self) -> Vec<(Vec<u8>, VarId)> {
		self.constants.iter().map(|(name, id)| (name.to_vec(), *id)).collect()
	}
	...
	pub fn decls(&self) -> Vec<(Vec<u8>, DeclId)> {
		let mut result: Vec<(Vec<u8>, DeclId)> = self.decls.iter().map(|(name, id)| (name.clone(), *id)).collect();
	...
	pub fn submodules(&self) -> Vec<(Vec<u8>, ModuleId)> {
		self.submodules.iter().map(|(name, id)| (name.clone(), *id)).collect()
	}
