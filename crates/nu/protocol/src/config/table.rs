...
use super::prelude::*;
use crate::{self as xeno_nu_protocol, ConfigError, FromValue};
...
impl UpdateFromValue for FooterMode {
	fn update(&mut self, value: &Value, path: &mut ConfigPath, errors: &mut ConfigErrors) {
		match value {
			...
			_ => errors.type_mismatch(path, Type::custom("'never', 'always', 'auto', or int"), value),
		}
	}
}
...
impl Default for TrimStrategy {
	fn default() -> Self {
		Self::Wrap { try_to_keep_words: true }
	}
}
...
impl UpdateFromValue for TrimStrategy {
	fn update<'a>(&mut self, value: &'a Value, path: &mut ConfigPath<'a>, errors: &mut ConfigErrors) {
		...
		match methodology.as_str() {
			...
			Ok("truncating") => {
				let mut suffix = if let Self::Truncate { suffix } = self { suffix.take() } else { None };
				for (col, val) in record.iter() {
					let path = &mut path.push(col);
					match col.as_str() {
			...
			Ok(_) => errors.invalid_value(&path.push("methodology"), "'wrapping' or 'truncating'", methodology),
...
impl IntoValue for TableConfig {
	fn into_value(self, span: Span) -> Value {
		let abbv_count = self.abbreviated_row_count.map(|t| t as i64).into_value(span);
	...
	fn update<'a>(&mut self, value: &'a Value, path: &mut ConfigPath<'a>, errors: &mut ConfigErrors) {
		...
		for (col, val) in record.iter() {
			...
			match col.as_str() {
				...
				"batch_duration" => match Duration::from_value(val.clone()).map_err(ConfigError::from) {
