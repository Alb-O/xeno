...
pub use completions::{CompletionAlgorithm, CompletionConfig, CompletionSort, ExternalCompleterConfig};
...
use crate::{
	FromValue, {self as xeno_nu_protocol},
};
...
impl UpdateFromValue for Config {
	fn update<'a>(&mut self, value: &'a Value, path: &mut ConfigPath<'a>, errors: &mut ConfigErrors) {
		...
		for (col, val) in record.iter() {
			...
			match col.as_str() {
				...
				"buffer_editor" => match val {
					...
					Value::List { vals, .. } if vals.iter().all(|val| matches!(val, Value::String { .. })) => {
						self.buffer_editor = val.clone();
					}
					_ => errors.type_mismatch(path, Type::custom("string, list<string>, or nothing"), val),
				},
				"show_banner" => self.show_banner.update(val, path, errors),
				"display_errors" => self.display_errors.update(val, path, errors),
				"render_right_prompt_on_last_line" => self.render_right_prompt_on_last_line.update(val, path, errors),
				...
				"highlight_resolved_externals" => self.highlight_resolved_externals.update(val, path, errors),
...
impl Config {
	pub fn update_from_value(&mut self, old: &Config, value: &Value) -> Result<Option<ShellWarning>, ShellError> {
