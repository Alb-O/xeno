...
use crate::{self as xeno_nu_protocol, FromValue};
...
mod tests {
	...
	fn test_use_ansi_coloring_true() {
		...
		assert!(engine_state.get_config().use_ansi_coloring.get(&engine_state));
	...
	fn test_use_ansi_coloring_auto() {
		...
		assert_eq!(engine_state.get_config().use_ansi_coloring.get(&engine_state), is_terminal);
