...
impl Hooks {
	pub fn new() -> Self {
		Self {
			...
			display_output: Some(Value::string("if (term size).columns >= 100 { table -e } else { table }", Span::unknown())),
	...
	fn update<'a>(&mut self, value: &'a Value, path: &mut ConfigPath<'a>, errors: &mut ConfigErrors) {
		...
		for (col, val) in record.iter() {
			...
			match col.as_str() {
						...
						self.env_change = record
							...
							.map(|(key, val)| {
								...
								let new = if let Ok(hooks) = val.as_list() {
									...
									errors.type_mismatch(&path.push(key), Type::list(Type::Any), val);
									old
								};
								(key.as_str().into(), new)
							})
				...
				"display_output" => self.display_output = if val.is_nothing() { None } else { Some(val.clone()) },
				"command_not_found" => self.command_not_found = if val.is_nothing() { None } else { Some(val.clone()) },
