...
impl<'a> ConfigErrors<'a> {
	...
	pub fn invalid_value(&mut self, path: &ConfigPath, expected: impl Into<String>, actual: &Value) {
	...
	pub fn check(self) -> Result<Option<ShellWarning>, ShellError> {
		match (self.has_errors(), self.has_warnings()) {
			(true, _) => Err(ShellError::InvalidConfig { errors: self.errors }),
			(false, true) => Ok(Some(ShellWarning::InvalidConfig { warnings: self.warnings })),
			(false, false) => Ok(None),
		}
	}
}
