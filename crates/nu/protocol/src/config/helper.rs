...
pub(super) fn config_update_string_enum<T>(choice: &mut T, value: &Value, path: &mut ConfigPath, errors: &mut ConfigErrors)
where
	T: FromStr,
	T::Err: Display,
{
...
pub fn extract_value<'record>(column: &'static str, record: &'record Record, span: Span) -> Result<&'record Value, ShellError> {
	record.get(column).ok_or_else(|| ShellError::MissingRequiredColumn { column, span })
}
