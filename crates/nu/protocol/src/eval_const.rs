...
use crate::ast::{Assignment, Block, Call, Expr, Expression, ExternalArgument};
use crate::debugger::{DebugContext, WithoutDebug};
use crate::engine::{EngineState, StateWorkingSet};
use crate::eval_base::Eval;
use crate::{BlockId, Config, HistoryFileFormat, PipelineData, Record, ShellError, Span, Value, VarId, record};
...
pub(crate) fn create_nu_constant(engine_state: &EngineState, span: Span) -> Value {
	fn canonicalize_path(engine_state: &EngineState, path: &Path) -> PathBuf {
		...
		if path.exists() {
			match xeno_nu_path::canonicalize_with(path, cwd) {
	...
	let config_path = match xeno_nu_path::nu_config_dir() {
	...
	record.push(
		...
		config_path
			.as_ref()
			.map_or_else(|e| e.clone(), |path| Value::string(path.to_string_lossy(), span)),
	);
	...
	record.push(
		"home-dir",
		if let Some(path) = xeno_nu_path::home_dir() {
	...
	record.push(
		"data-dir",
		if let Some(path) = xeno_nu_path::data_dir() {
	...
	record.push(
		"cache-dir",
		if let Some(path) = xeno_nu_path::cache_dir() {
	...
	record.push("startup-time", Value::duration(engine_state.get_startup_time(), span));

	record.push("is-interactive", Value::bool(engine_state.is_interactive, span));

	record.push("is-login", Value::bool(engine_state.is_login, span));

	record.push("history-enabled", Value::bool(engine_state.history_enabled, span));
...
pub fn get_vendor_autoload_dirs(_engine_state: &EngineState) -> Vec<PathBuf> {
	...
	if let Some(data_dir) = xeno_nu_path::data_dir() {
...
pub fn get_user_autoload_dirs(_engine_state: &EngineState) -> Vec<PathBuf> {
	...
	if let Some(config_dir) = xeno_nu_path::nu_config_dir() {
...
fn eval_const_call(working_set: &StateWorkingSet, call: &Call, input: PipelineData) -> Result<PipelineData, ShellError> {
...
pub fn eval_const_subexpression(working_set: &StateWorkingSet, block: &Block, mut input: PipelineData, span: Span) -> Result<PipelineData, ShellError> {
...
pub fn eval_constant_with_input(working_set: &StateWorkingSet, expr: &Expression, input: PipelineData) -> Result<PipelineData, ShellError> {
	match &expr.expr {
...
pub fn eval_constant(working_set: &StateWorkingSet, expr: &Expression) -> Result<Value, ShellError> {
...
impl Eval for EvalConst {
	...
	fn eval_var(working_set: &StateWorkingSet, _: &mut (), var_id: VarId, span: Span) -> Result<Value, ShellError> {
	...
	fn eval_call<D: DebugContext>(working_set: &StateWorkingSet, _: &mut (), call: &Call, span: Span) -> Result<Value, ShellError> {
		// TODO: Allow debugging const eval
	...
	fn eval_external_call(_: &StateWorkingSet, _: &mut (), _: &Expression, _: &[ExternalArgument], span: Span) -> Result<Value, ShellError> {
		// TODO: It may be more helpful to give not_a_const_command error
		Err(ShellError::NotAConstant { span })
	}

	fn eval_collect<D: DebugContext>(_: &StateWorkingSet, _: &mut (), _var_id: VarId, expr: &Expression) -> Result<Value, ShellError> {
		Err(ShellError::NotAConstant { span: expr.span })
	}

	fn eval_subexpression<D: DebugContext>(working_set: &StateWorkingSet, _: &mut (), block_id: BlockId, span: Span) -> Result<Value, ShellError> {
		// If parsing errors exist in the subexpression, don't bother to evaluate it.
		if working_set.parse_errors.iter().any(|error| span.contains_span(error.span())) {
	...
	fn regex_match(_: &StateWorkingSet, _op_span: Span, _: &Value, _: &Value, _: bool, expr_span: Span) -> Result<Value, ShellError> {
	...
	fn eval_row_condition_or_closure(_: &StateWorkingSet, _: &mut (), _: BlockId, span: Span) -> Result<Value, ShellError> {
		Err(ShellError::NotAConstant { span })
	}
	...
	fn unreachable(working_set: &StateWorkingSet, expr: &Expression) -> Result<Value, ShellError> {
		Err(ShellError::NotAConstant { span: expr.span(&working_set) })
	}
}
