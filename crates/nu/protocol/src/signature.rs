...
use xeno_nu_derive_value::FromValue as DeriveFromValue;
use xeno_nu_utils::NuCow;

// Make xeno_nu_protocol available in this namespace, consumers of this crate will
// have this without such an export.
// The `FromValue` derive macro fully qualifies paths to "xeno_nu_protocol".
use crate as xeno_nu_protocol;
use crate::engine::{Call, Command, CommandType, EngineState, Stack};
use crate::{BlockId, DeclId, DeprecationEntry, Example, FromValue, IntoValue, PipelineData, ShellError, Span, SyntaxShape, Type, Value, VarId};
...
impl Completion {
	...
	pub fn to_value(&self, engine_state: &EngineState, span: Span) -> Value {
		match self {
			Completion::Command(id) => engine_state.get_decl(*id).name().to_owned().into_value(span),
			Completion::List(list) => match list {
				NuCow::Borrowed(list) => list.iter().map(|&e| e.into_value(span)).collect::<Vec<Value>>().into_value(span),
				NuCow::Owned(list) => list.iter().cloned().map(|e| e.into_value(span)).collect::<Vec<Value>>().into_value(span),
			},
		}
	}
}
...
impl Signature {
	...
	pub fn get_input_type(&self) -> Type {
		match self.input_output_types.len() {
			...
			_ => {
				...
				if self.input_output_types.iter().all(|(input, _)| input == first) {
					first.clone()
				} else {
					Type::Any
				}
	...
	pub fn get_output_type(&self) -> Type {
		match self.input_output_types.len() {
			...
			_ => {
				...
				if self.input_output_types.iter().all(|(_, output)| output == first) {
					first.clone()
				} else {
					Type::Any
				}
	...
	pub fn update_from_command(mut self, command: &dyn Command) -> Signature {
		self.search_terms = command.search_terms().into_iter().map(|term| term.to_string()).collect();
		...
		match param {
			Parameter::Flag(flag) => {
				if let Some(s) = flag.short {
					assert!(!self.get_shorts().contains(&s), "There may be duplicate short flags for '-{s}'");
				}

				let name = flag.long.as_str();
				assert!(!self.get_names().contains(&name), "There may be duplicate name flags for '--{name}'");
			...
			Parameter::Rest(positional_arg) => {
				assert!(self.rest_positional.is_none(), "Tried to set rest arguments more than once");
	...
	pub fn required(mut self, name: impl Into<String>, shape: impl Into<SyntaxShape>, desc: impl Into<String>) -> Signature {
	...
	pub fn optional(mut self, name: impl Into<String>, shape: impl Into<SyntaxShape>, desc: impl Into<String>) -> Signature {
	...
	pub fn rest(mut self, name: &str, shape: impl Into<SyntaxShape>, desc: impl Into<String>) -> Signature {
	...
	pub fn operates_on_cell_paths(&self) -> bool {
		self.required_positional.iter().chain(self.rest_positional.iter()).any(|pos| {
			matches!(
				pos,
				PositionalArg {
	...
	pub fn named(mut self, name: impl Into<String>, shape: impl Into<SyntaxShape>, desc: impl Into<String>, short: Option<char>) -> Signature {
	...
	pub fn required_named(mut self, name: impl Into<String>, shape: impl Into<SyntaxShape>, desc: impl Into<String>, short: Option<char>) -> Signature {
		let (name, s) = self.check_names(name, short);
	...
	pub fn switch(mut self, name: impl Into<String>, desc: impl Into<String>, short: Option<char>) -> Signature {
		let (name, s) = self.check_names(name, short);
	...
	fn check_names(&self, name: impl Into<String>, short: Option<char>) -> (String, Option<char>) {
		let s = short.inspect(|c| {
			assert!(!self.get_shorts().contains(c), "There may be duplicate short flags for '-{c}'");
		});

		let name = {
			let name: String = name.into();
			assert!(!self.get_names().contains(&name.as_str()), "There may be duplicate name flags for '--{name}'");
			name
		};
	...
	pub fn get_positional(&self, position: usize) -> Option<&PositionalArg> {
		if position < self.required_positional.len() {
			...
			self.optional_positional.get(position - self.required_positional.len())
		} else {
...
impl Command for Predeclaration {
	...
	fn run(&self, _engine_state: &EngineState, _stack: &mut Stack, _call: &Call, _input: PipelineData) -> Result<PipelineData, crate::ShellError> {
...
impl Command for BlockCommand {
	...
	fn run(&self, _engine_state: &EngineState, _stack: &mut Stack, _call: &Call, _input: PipelineData) -> Result<crate::PipelineData, crate::ShellError> {
	...
	fn examples(&self) -> Vec<Example<'_>> {
		self.examples.iter().map(CustomExample::to_example).collect()
	}

	fn search_terms(&self) -> Vec<&str> {
		self.signature.search_terms.iter().map(String::as_str).collect()
	}
