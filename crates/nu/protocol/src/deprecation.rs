...
use crate::{self as xeno_nu_protocol, ReportMode, Span};
use crate::{FromValue, ParseWarning, ShellError, Type, Value};
...
impl DeprecationEntry {
	...
	fn span(&self, call: &Call) -> Span {
		match &self.ty {
			...
			DeprecationType::Flag(flag) => call.get_named_arg(flag).map(|arg| arg.span).unwrap_or(Span::unknown()),
		}
	}
