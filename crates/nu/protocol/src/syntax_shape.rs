...
impl SyntaxShape {
	...
	///
	/// ```rust
	/// use xeno_nu_protocol::{SyntaxShape, Type};
	/// let non_value = SyntaxShape::ImportPattern;
	/// assert_eq!(non_value.to_type(), Type::Any);
	/// ```
	pub fn to_type(&self) -> Type {
		let mk_ty = |tys: &[(String, SyntaxShape)]| tys.iter().map(|(key, val)| (key.clone(), val.to_type())).collect();
...
impl Display for SyntaxShape {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		let mk_fmt = |tys: &[(String, SyntaxShape)]| -> String { tys.iter().map(|(x, y)| format!("{x}: {y}")).collect::<Vec<String>>().join(", ") };
		...
		match self {
			...
			SyntaxShape::RowCondition => write!(f, "oneof<condition, {}>", SyntaxShape::Closure(Some(vec![SyntaxShape::Any]))),
