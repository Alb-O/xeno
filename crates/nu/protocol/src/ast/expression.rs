...
use super::ListItem;
use crate::ast::{Argument, Block, Expr, ExternalArgument, ImportPattern, MatchPattern, RecordItem};
use crate::engine::StateWorkingSet;
use crate::{BlockId, GetSpan, IN_VARIABLE_ID, Signature, Span, SpanId, Type, VarId};
...
impl Expression {
	...
	pub fn has_in_variable(&self, working_set: &StateWorkingSet) -> bool {
		match &self.expr {
			...
			Expr::BinaryOp(left, _, right) => left.has_in_variable(working_set) || right.has_in_variable(working_set),
			Expr::UnaryNot(expr) => expr.has_in_variable(working_set),
			Expr::Block(block_id) | Expr::Closure(block_id) => {
				let block = working_set.get_block(*block_id);
				block.captures.iter().any(|(var_id, _)| var_id == &IN_VARIABLE_ID)
					|| block
						.pipelines
						.iter()
	...
	pub fn replace_span(&mut self, working_set: &mut StateWorkingSet, replaced: Span, new_span: Span) {
		...
		match &mut self.expr {
			...
			Expr::ExternalCall(head, args) => {
				...
				for ExternalArgument::Regular(expr) | ExternalArgument::Spread(expr) in args.as_mut() {
					expr.replace_span(working_set, replaced, new_span);
				}
			}
			Expr::Filepath(_, _) => {}
			...
			Expr::Float(_) => {}
			Expr::FullCellPath(full_cell_path) => {
				full_cell_path.head.replace_span(working_set, replaced, new_span);
			}
			...
			Expr::List(list) => {
				for item in list {
					item.expr_mut().replace_span(working_set, replaced, new_span);
				}
			}
			Expr::Operator(_) => {}
			Expr::Range(range) => {
	...
	pub fn replace_in_variable(&mut self, working_set: &mut StateWorkingSet, new_var_id: VarId) {
		match &mut self.expr {
			...
			Expr::Call(call) => {
				for arg in call.arguments.iter_mut() {
					match arg {
						Argument::Positional(expr) | Argument::Unknown(expr) | Argument::Named((_, _, Some(expr))) | Argument::Spread(expr) => {
			...
			Expr::ExternalCall(head, args) => {
				...
				for arg in args.iter_mut() {
					match arg {
						ExternalArgument::Regular(expr) | ExternalArgument::Spread(expr) => expr.replace_in_variable(working_set, new_var_id),
					}
				}
			}
			Expr::Operator(_) => {}
			...
			Expr::List(items) => {
				for item in items.iter_mut() {
					match item {
						ListItem::Item(expr) | ListItem::Spread(_, expr) => expr.replace_in_variable(working_set, new_var_id),
					}
				}
			}
			Expr::Table(table) => {
			...
			Expr::Record(items) => {
				for item in items.iter_mut() {
					match item {
						...
						RecordItem::Spread(_, expr) => expr.replace_in_variable(working_set, new_var_id),
					}
				}
			}
			Expr::Keyword(kw) => kw.expr.replace_in_variable(working_set, new_var_id),
			Expr::ValueWithUnit(value_with_unit) => value_with_unit.expr.replace_in_variable(working_set, new_var_id),
			Expr::DateTime(_) => {}
			Expr::Filepath(_, _) => {}
			Expr::Directory(_, _) => {}
			Expr::GlobPattern(_, _) => {}
	...
	pub fn new(working_set: &mut StateWorkingSet, expr: Expr, span: Span, ty: Type) -> Expression {
		let span_id = working_set.add_span(span);
		Expression { expr, span, span_id, ty }
	}
