...
use crate::eval_const::eval_constant;
use crate::{DeclId, FromValue, ShellError, Span, Spanned, Value};
...
impl Argument {
	...
	pub fn expr(&self) -> Option<&Expression> {
		match self {
			...
			Argument::Positional(expr) | Argument::Unknown(expr) | Argument::Spread(expr) => Some(expr),
		}
	}
}
...
/// [`Expression`].
///
/// For further utilities check the `xeno_nu_engine::CallExt` trait that extends [`Call`]
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct Call {
...
impl Call {
	...
	pub fn named_iter(&self) -> impl DoubleEndedIterator<Item = &(Spanned<String>, Option<Spanned<String>>, Option<Expression>)> {
	...
	pub fn named_iter_mut(&mut self) -> impl Iterator<Item = &mut (Spanned<String>, Option<Spanned<String>>, Option<Expression>)> {
	...
	pub fn add_named(&mut self, named: (Spanned<String>, Option<Spanned<String>>, Option<Expression>)) {
	...
	pub fn has_flag_const(&self, working_set: &StateWorkingSet, flag_name: &str) -> Result<bool, ShellError> {
	...
	pub fn get_flag_const<T: FromValue>(&self, working_set: &StateWorkingSet, name: &str) -> Result<Option<T>, ShellError> {
	...
	pub fn rest_const<T: FromValue>(&self, working_set: &StateWorkingSet, starting_pos: usize) -> Result<Vec<T>, ShellError> {
		let mut output = vec![];

		for result in self.rest_iter_flattened(starting_pos, |expr| eval_constant(working_set, expr))? {
	...
	pub fn rest_iter_flattened<F>(&self, start: usize, mut eval: F) -> Result<Vec<Value>, ShellError>
	where
	...
	pub fn req_const<T: FromValue>(&self, working_set: &StateWorkingSet, pos: usize) -> Result<T, ShellError> {
