...
pub enum RedirectionTarget {
	File { expr: Expression, append: bool, span: Span },
	Pipe { span: Span },
}
...
impl RedirectionTarget {
	...
	pub fn replace_span(&mut self, working_set: &mut StateWorkingSet, replaced: Span, new_span: Span) {
		match self {
			RedirectionTarget::File { expr, .. } => expr.replace_span(working_set, replaced, new_span),
			RedirectionTarget::Pipe { .. } => {}
		}
	}

	pub fn replace_in_variable(&mut self, working_set: &mut StateWorkingSet<'_>, new_var_id: VarId) {
		match self {
			RedirectionTarget::File { expr, .. } => expr.replace_in_variable(working_set, new_var_id),
			RedirectionTarget::Pipe { .. } => {}
		}
	}
}
...
pub enum PipelineRedirection {
	Single { source: RedirectionSource, target: RedirectionTarget },
	Separate { out: RedirectionTarget, err: RedirectionTarget },
}
impl PipelineRedirection {
	pub fn replace_in_variable(&mut self, working_set: &mut StateWorkingSet<'_>, new_var_id: VarId) {
		match self {
			PipelineRedirection::Single { source: _, target } => target.replace_in_variable(working_set, new_var_id),
...
impl PipelineElement {
	pub fn has_in_variable(&self, working_set: &StateWorkingSet) -> bool {
		self.expr.has_in_variable(working_set)
			|| self.redirection.as_ref().is_some_and(|r| match r {
				...
				PipelineRedirection::Separate { out, err } => out.has_in_variable(working_set) || err.has_in_variable(working_set),
			})
	}
	...
	pub fn replace_span(&mut self, working_set: &mut StateWorkingSet, replaced: Span, new_span: Span) {
		...
		if let Some(expr) = self.redirection.as_mut() {
			match expr {
				PipelineRedirection::Single { target, .. } => target.replace_span(working_set, replaced, new_span),
				PipelineRedirection::Separate { out, err } => {
	...
	pub fn pipe_redirection(&self, working_set: &StateWorkingSet) -> (Option<OutDest>, Option<OutDest>) {
