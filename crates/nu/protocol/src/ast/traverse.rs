use super::{Block, Expr, Expression, ListItem, MatchPattern, Pattern, PipelineRedirection, RecordItem};
		...
		F: Fn(&'a Expression) -> FindMapResult<T>,
	{
		self.pipelines.iter().find_map(|pipeline| {
			pipeline.elements.iter().find_map(|element| {
				element
					.expr
					.find_map(working_set, f)
					.or(element.redirection.as_ref().and_then(|redir| redir.find_map(working_set, f)))
			})
		})
	}
}
...
impl Traverse for PipelineRedirection {
		...
		F: Fn(&'a Expression) -> FindMapResult<T>,
	{
		...
		match self {
			...
			PipelineRedirection::Separate { out, err } => [out, err].iter().filter_map(|t| t.expr()).find_map(recur),
		}
	}
}
...
impl Traverse for Expression {
		...
		F: Fn(&'a Expression) -> Vec<T>,
	{
		...
		match &self.expr {
			Expr::RowCondition(block_id) | Expr::Subexpression(block_id) | Expr::Block(block_id) | Expr::Closure(block_id) => {
		...
		match f(self) {
			...
			FindMapResult::Continue => {
				...
				match &self.expr {
					...
					Expr::Range(range) => [&range.from, &range.next, &range.to].iter().find_map(|e| e.as_ref().and_then(recur)),
					Expr::Call(call) => call.arguments.iter().find_map(|arg| arg.expr().and_then(recur)),
					Expr::ExternalCall(head, args) => recur(head.as_ref()).or(args.iter().find_map(|arg| recur(arg.expr()))),
					Expr::UnaryNot(expr) | Expr::Collect(_, expr) => recur(expr.as_ref()),
					Expr::BinaryOp(lhs, op, rhs) => recur(lhs).or(recur(op)).or(recur(rhs)),
					Expr::MatchBlock(matches) => matches.iter().find_map(|(pattern, expr)| pattern.find_map(working_set, f).or(recur(expr))),
					...
					Expr::FullCellPath(fcp) => recur(&fcp.head),
					Expr::Keyword(kw) => recur(&kw.expr),
					Expr::StringInterpolation(vec) | Expr::GlobInterpolation(vec, _) => vec.iter().find_map(recur),
					Expr::AttributeBlock(ab) => ab.attributes.iter().find_map(|attr| recur(&attr.expr)).or_else(|| recur(&ab.item)),
...
impl Traverse for MatchPattern {
	...
	where
		F: Fn(&'a Expression) -> Vec<T>,
	{
		let mut recur_pattern = |pattern: &'a MatchPattern| pattern.flat_map(working_set, f, results);
		...
		F: Fn(&'a Expression) -> FindMapResult<T>,
	{
		...
		match &self.pattern {
			...
			Pattern::List(patterns) | Pattern::Or(patterns) => patterns.iter().find_map(recur_pattern),
