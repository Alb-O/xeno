...
use xeno_nu_utils::{escape_quote_string, needs_quoting};
...
impl PathMember {
	pub fn int(val: usize, optional: bool, span: Span) -> Self {
		PathMember::Int { val, span, optional }
	}

	pub fn string(val: String, optional: bool, casing: Casing, span: Span) -> Self {
		PathMember::String { val, span, optional, casing }
	}
...
impl PartialEq for PathMember {
	fn eq(&self, other: &Self) -> bool {
		match (self, other) {
			(
				Self::String {
					val: l_val, optional: l_opt, ..
				},
				Self::String {
					val: r_val, optional: r_opt, ..
				},
	...
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		...
		for member in self.members.iter() {
			match member {
				...
				PathMember::String { val, optional, casing, .. } => {
					let question_mark = if *optional { "?" } else { "" };
					let exclamation_mark = if *casing == Casing::Insensitive { "!" } else { "" };
					let val = if needs_quoting(val) { &escape_quote_string(val) } else { val };
...
mod test {
	...
	fn path_member_partial_ord() {
		assert_eq!(
			Some(Greater),
			PathMember::test_int(5, true).partial_cmp(&PathMember::test_string("e".into(), true, Casing::Sensitive))
		);

		assert_eq!(Some(Greater), PathMember::test_int(5, true).partial_cmp(&PathMember::test_int(5, false)));

		assert_eq!(Some(Greater), PathMember::test_int(6, true).partial_cmp(&PathMember::test_int(5, true)));

		assert_eq!(
			Some(Greater),
			PathMember::test_string("e".into(), true, Casing::Sensitive).partial_cmp(&PathMember::test_string("e".into(), false, Casing::Sensitive))
		);

		assert_eq!(
			Some(Greater),
			PathMember::test_string("f".into(), true, Casing::Sensitive).partial_cmp(&PathMember::test_string("e".into(), true, Casing::Sensitive))
		);
	}
}
