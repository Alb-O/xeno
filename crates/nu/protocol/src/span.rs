...
impl<T, E> Spanned<Result<T, E>> {
	...
	pub fn transpose(self) -> Result<Spanned<T>, E> {
		match self {
			Spanned { item: Ok(item), span } => Ok(Spanned { item, span }),
			Spanned { item: Err(err), span: _ } => Err(err),
		}
	}
}
...
pub trait IntoSpanned: Sized {
	...
	///
	/// ```
	/// # use xeno_nu_protocol::{Span, IntoSpanned};
...
impl Span {
	pub fn new(start: usize, end: usize) -> Self {
		debug_assert!(end >= start, "Can't create a Span whose end < start, start={start}, end={end}");
	...
	pub fn subspan(&self, offset_start: usize, offset_end: usize) -> Option<Self> {
		...
		if offset_start > len || offset_end > len || offset_start > offset_end {
			...
			Some(Self::new(self.start + offset_start, self.start + offset_end))
		}
	}
	...
	pub fn split_at(&self, offset: usize) -> Option<(Self, Self)> {
		if offset < self.len() {
			Some((Self::new(self.start, self.start + offset), Self::new(self.start + offset, self.end)))
		} else {
			None
		}
	}
	...
	pub fn merge_many(spans: impl IntoIterator<Item = Self>) -> Self {
		spans.into_iter().reduce(Self::merge).unwrap_or(Self::unknown())
	}
}
