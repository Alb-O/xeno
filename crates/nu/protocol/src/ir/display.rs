	...
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		...
		match self.instruction {
			...
			Instruction::LoadLiteral { dst, lit } => {
				let lit = FmtLiteral { literal: lit, data: self.data };
			...
			Instruction::OpenFile { file_num, path, append } => {
				write!(f, "{:WIDTH$} file({file_num}), {path}, append = {append:?}", "open-file")
			}
			...
			Instruction::UpsertCellPath { src_dst, path, new_value } => {
				write!(f, "{:WIDTH$} {src_dst}, {path}, {new_value}", "upsert-cell-path")
			}
			...
			Instruction::Iterate { dst, stream, end_index } => {
...
impl fmt::Display for FmtLiteral<'_> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self.literal {
			...
			Literal::Range { start, step, end, inclusion } => write!(f, "range({start}, {step}, {end}, {inclusion:?})"),
			Literal::List { capacity } => write!(f, "list(capacity = {capacity})"),
			Literal::Record { capacity } => write!(f, "record(capacity = {capacity})"),
			Literal::Filepath { val, no_expand } => write!(f, "filepath({}, no_expand = {no_expand:?})", FmtData(self.data, *val)),
			Literal::Directory { val, no_expand } => write!(f, "directory({}, no_expand = {no_expand:?})", FmtData(self.data, *val)),
			Literal::GlobPattern { val, no_expand } => write!(f, "glob-pattern({}, no_expand = {no_expand:?})", FmtData(self.data, *val)),
	...
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self.pattern {
			...
			Pattern::Expression(expr) => {
				let string = String::from_utf8_lossy(self.engine_state.get_span_contents(expr.span));
				f.write_str(&string)
			}
			Pattern::Value(value) => f.write_str(&value.to_parsable_string(", ", &self.engine_state.config)),
