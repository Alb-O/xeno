...
use crate::ast::{CellPath, Expression, Operator, Pattern, RangeInclusion};
use crate::engine::EngineState;
use crate::{BlockId, DeclId, Filesize, RegId, ShellError, Span, Value, VarId};
...
impl IrBlock {
	...
	pub fn display<'a>(&'a self, engine_state: &'a EngineState) -> FmtIrBlock<'a> {
		FmtIrBlock { engine_state, ir_block: self }
	}
}
...
pub enum Instruction {
	...
	PushParserInfo { name: DataSlice, info: Box<Expression> },
	...
	OpenFile { file_num: u32, path: RegId, append: bool },
	...
	RecordInsert { src_dst: RegId, key: RegId, val: RegId },
	...
	/// `lhs_dst`.
	BinaryOp { lhs_dst: RegId, op: Operator, rhs: RegId },
	...
	UpsertCellPath { src_dst: RegId, path: RegId, new_value: RegId },
	...
	Match { pattern: Box<Pattern>, src: RegId, index: usize },
	...
	Iterate { dst: RegId, stream: RegId, end_index: usize },
...
impl Instruction {
	...
	pub fn branch_target(&self) -> Option<usize> {
		match self {
			...
			Instruction::Match { pattern: _, src: _, index } => Some(*index),

			Instruction::Iterate { dst: _, stream: _, end_index } => Some(*end_index),
	...
	pub fn set_branch_target(&mut self, target_index: usize) -> Result<(), usize> {
		match self {
			...
			Instruction::Match { pattern: _, src: _, index } => *index = target_index,

			Instruction::Iterate { dst: _, stream: _, end_index } => *end_index = target_index,
	...
	pub fn check_interrupt(&self, engine_state: &EngineState, span: &Span) -> Result<(), ShellError> {
		match self {
			Instruction::Jump { .. } | Instruction::Return { .. } => engine_state.signals().check(span),
			_ => Ok(()),
		}
	}
}
