...
use crate::engine::{self, Argument, Stack};
...
impl Call {
	...
	pub fn named_iter<'a>(&'a self, stack: &'a Stack) -> impl Iterator<Item = (Spanned<&'a str>, Option<&'a Value>)> + 'a {
		self.arguments(stack)
			.iter()
			.filter_map(|arg: &Argument| -> Option<(Spanned<&str>, Option<&Value>)> {
				match arg {
					Argument::Flag { data, name, span, .. } => Some((
						Spanned {
					...
					Argument::Named { data, name, span, val, .. } => Some((
						Spanned {
							item: std::str::from_utf8(&data[*name]).expect("invalid arg name"),
							span: *span,
						},
	...
	pub fn get_named_arg<'a>(&self, stack: &'a Stack, flag_name: &str) -> Option<&'a Value> {
		...
		self.arguments(stack)
			...
			.find_map(|arg: &Argument| -> Option<Option<&Value>> {
				match arg {
					Argument::Flag { data, name, .. } if &data[*name] == flag_name.as_bytes() => Some(None),
					Argument::Named { data, name, val, .. } if &data[*name] == flag_name.as_bytes() => Some(Some(val)),
					_ => None,
				}
			})
			.flatten()
	...
	pub fn positional_len(&self, stack: &Stack) -> usize {
		self.arguments(stack).iter().filter(|arg| matches!(arg, Argument::Positional { .. })).count()
	}
	...
	/// required it.
	pub fn positional_ast<'a>(&self, stack: &'a Stack, index: usize) -> Option<&'a Arc<Expression>> {
		self.arguments(stack)
			.iter()
	...
	pub fn rest_iter<'a>(&self, stack: &'a Stack, start: usize) -> impl Iterator<Item = (&'a Value, bool)> + 'a {
		self.arguments(stack)
			.iter()
			.filter_map(|arg| match arg {
	...
	pub fn rest_iter_flattened(&self, stack: &Stack, start: usize) -> Result<Vec<Value>, ShellError> {
		let mut acc = vec![];
		for (rest_val, spread) in self.rest_iter(stack, start) {
			if spread {
				match rest_val {
					...
					_ => {
						return Err(ShellError::CannotSpreadAsList { span: rest_val.span() });
					}
				}
			} else {
	...
	pub fn get_parser_info<'a>(&self, stack: &'a Stack, name: &str) -> Option<&'a Expression> {
		self.arguments(stack).iter().find_map(|argument| match argument {
			Argument::ParserInfo {
				data,
				name: name_slice,
				info: expr,
...
impl CallBuilder {
	...
	pub fn add_positional(&mut self, stack: &mut Stack, span: Span, val: Value) -> &mut Self {
		self.add_argument(stack, Argument::Positional { span, val, ast: None })
	}
	...
	pub fn add_spread(&mut self, stack: &mut Stack, span: Span, vals: Value) -> &mut Self {
		self.add_argument(stack, Argument::Spread { span, vals, ast: None })
	}

	/// Add a flag (no-value named) argument to the [`Stack`] and reference it from the [`Call`].
	pub fn add_flag(&mut self, stack: &mut Stack, name: impl AsRef<str>, short: impl AsRef<str>, span: Span) -> &mut Self {
		let (data, name, short) = data_from_name_and_short(name.as_ref(), short.as_ref());
		self.add_argument(stack, Argument::Flag { data, name, short, span })
	}

	/// Add a named argument to the [`Stack`] and reference it from the [`Call`].
	pub fn add_named(&mut self, stack: &mut Stack, name: impl AsRef<str>, short: impl AsRef<str>, span: Span, val: Value) -> &mut Self {
		let (data, name, short) = data_from_name_and_short(name.as_ref(), short.as_ref());
		self.add_argument(
			stack,
			Argument::Named {
	...
	pub fn with<T>(self, stack: &mut Stack, f: impl FnOnce(&mut Stack, &engine::Call<'_>) -> T) -> T {
