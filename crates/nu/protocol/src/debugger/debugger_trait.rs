...
use crate::{PipelineData, PipelineExecutionData, ShellError, Span, Value};
...
impl DebugContext for WithDebug {
	...
	fn leave_element(engine_state: &EngineState, element: &PipelineElement, result: &Result<PipelineData, ShellError>) {
		if let Ok(mut debugger) = engine_state.debugger.lock() {
			debugger.deref_mut().leave_element(engine_state, element, result);
		}
	}

	fn enter_instruction(engine_state: &EngineState, ir_block: &IrBlock, instruction_index: usize, registers: &[PipelineExecutionData]) {
		if let Ok(mut debugger) = engine_state.debugger.lock() {
			debugger.deref_mut().enter_instruction(engine_state, ir_block, instruction_index, registers)
		}
	}
	...
	fn leave_instruction(
		...
		if let Ok(mut debugger) = engine_state.debugger.lock() {
			debugger
				.deref_mut()
				.leave_instruction(engine_state, ir_block, instruction_index, registers, error)
		}
	}
}
...
pub trait Debugger: Send + Debug {
	...
	/// Called before the IR evaluator runs an instruction
	#[allow(unused_variables)]
	fn enter_instruction(&mut self, engine_state: &EngineState, ir_block: &IrBlock, instruction_index: usize, registers: &[PipelineExecutionData]) {}
