...
use crate::{PipelineData, PipelineExecutionData, ShellError, Span, Value, record};
...
impl Profiler {
	...
	pub fn new(opts: ProfilerOptions, span: Span) -> Self {
		let first = ElementInfo {
			...
			instruction: opts.collect_instructions.then(|| (0, "<start>".to_string())),
			children: vec![],
		};
...
impl Debugger for Profiler {
	...
	fn enter_element(&mut self, engine_state: &EngineState, element: &PipelineElement) {
		...
		let expr_opt = self.opts.collect_exprs.then(|| expr_to_string(engine_state, &element.expr.expr));
	...
	fn leave_element(&mut self, _engine_state: &EngineState, element: &PipelineElement, result: &Result<PipelineData, ShellError>) {
		if self.depth > self.opts.max_depth {
			return;
		}
	...
	fn enter_instruction(&mut self, engine_state: &EngineState, ir_block: &IrBlock, instruction_index: usize, _registers: &[PipelineExecutionData]) {
		if self.depth > self.opts.max_depth {
			return;
		}
		...
		let instruction_opt = self
			...
			.then(|| (instruction_index, instruction.display(engine_state, &ir_block.data).to_string()));
	...
	fn leave_instruction(
		...
		let out_opt = self
			...
			.then(|| {
				error
					.map(Err)
					.or_else(|| instruction.output_register().map(|register| Ok(&registers[register.get() as usize])))
	...
	fn report(&self, engine_state: &EngineState, profiler_span: Span) -> Result<Value, ShellError> {
		Ok(Value::list(
			collect_data(engine_state, self, ElementId(0), ElementId(0), profiler_span)?,
			profiler_span,
		))
	}
}
...
fn format_result(result: Result<&PipelineData, impl Borrow<ShellError>>, element_span: Span) -> Value {
	match result {
		Ok(pipeline_data) => match pipeline_data {
			PipelineData::Value(val, ..) => val.clone(),
			PipelineData::ListStream(..) => Value::string("list stream", element_span),
...
fn collect_data(
	...
	if profiler.opts.collect_spans {
		let span_start = i64::try_from(element.element_span.start).map_err(|_| profiler_error("error converting span start to i64", profiler_span))?;
