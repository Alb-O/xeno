...
use xeno_nu_path::expand_path;

use crate::ast::{Assignment, Bits, Boolean, Call, Comparison, Expr, Expression, ExternalArgument, ListItem, Math, Operator, RecordItem, eval_operator};
use crate::debugger::DebugContext;
use crate::{BlockId, Config, ENV_VARIABLE_ID, GetSpan, Range, Record, ShellError, Span, Value, VarId};
	...
	fn eval<D: DebugContext>(state: Self::State<'_>, mut_state: &mut Self::MutState, expr: &Expression) -> Result<Value, ShellError> {
		...
		match &expr.expr {
            ...
            Expr::GlobPattern(pattern, quoted) => {
                ...
                // But we don't want to expand path during eval time, it's required for `xeno_nu_engine::glob_from` to run correctly
