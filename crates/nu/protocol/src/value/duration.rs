...
pub fn format_duration(duration: i64) -> String {
	...
	let text = periods.into_iter().map(|p| p.to_text().to_string().replace(' ', "")).collect::<Vec<String>>();

	format!("{}{}", if sign == -1 { "-" } else { "" }, text.join(" ").trim())
}
...
pub fn format_duration_as_timeperiod(duration: i64) -> (i32, Vec<TimePeriod>) {
	...
	let (sign, duration) = if duration >= 0 { (1, duration) } else { (-1, -duration) };
	...
	fn normalize_split(wholes: i64, wholes_duration: impl Into<Option<Duration>>, total_duration: Duration) -> (Option<i64>, Duration) {
		match wholes_duration.into() {
			Some(wholes_duration) if wholes != 0 => (Some(wholes), total_duration - wholes_duration),
