...
use crate::casing::{CaseInsensitive, CaseSensitive, CaseSensitivity, Casing, WrapCased};
...
impl<'a> DynCasedRecord<&'a mut Record> {
	...
	///
	/// ```
	/// use xeno_nu_protocol::{record, record::{Record, DynCasedRecord}, Value, casing::Casing};
	///
	/// let mut rec = record!{
...
impl Record {
	...
	pub fn memory_size(&self) -> usize {
		std::mem::size_of::<Self>() + self.inner.iter().map(|(k, v)| k.capacity() + v.memory_size()).sum::<usize>()
	}

	pub fn cased(&self, casing: Casing) -> DynCasedRecord<&Record> {
		DynCasedRecord { record: self, casing }
	}
	...
	///
	/// ```rust
	/// use xeno_nu_protocol::{record, Record, Value};
	///
	...
	pub fn columns(&self) -> Columns<'_> {
		Columns { iter: self.inner.iter() }
	}

	pub fn into_columns(self) -> IntoColumns {
		IntoColumns { iter: self.inner.into_iter() }
	}

	pub fn values(&self) -> Values<'_> {
		Values { iter: self.inner.iter() }
	}

	pub fn into_values(self) -> IntoValues {
		IntoValues { iter: self.inner.into_iter() }
	}
	...
	{
		Drain { iter: self.inner.drain(range) }
	}
...
impl<'de> Deserialize<'de> for Record {
	...
	/// use xeno_nu_protocol::{Record, Value, record};
	///
		...
		A: serde::de::MapAccess<'de>,
	{
		...
		while let Some((key, value)) = map.next_entry::<String, Value>()? {
			if record.insert(key, value).is_some() {
				return Err(serde::de::Error::custom("invalid entry, duplicate keys are not allowed for `Record`"));
			}
		}
...
impl<'a> IntoIterator for &'a mut Record {
	...
	fn into_iter(self) -> Self::IntoIter {
		IterMut { iter: self.inner.iter_mut() }
	}
}
