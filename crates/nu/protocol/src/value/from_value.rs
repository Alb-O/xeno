...
use std::num::{NonZeroI8, NonZeroI16, NonZeroI32, NonZeroI64, NonZeroIsize, NonZeroU16, NonZeroU32, NonZeroU64, NonZeroUsize};
...
use crate::{NuGlob, Range, Record, ShellError, Span, Spanned, Type, Value};
...
/// # Enum Example
/// ```
/// # use xeno_nu_protocol::{FromValue, Value, ShellError, record, Span};
/// #
	...
	T: FromValue,
{
	fn from_value(v: Value) -> Result<Self, ShellError> {
		...
		vec.try_into().map_err(|err_vec: Vec<T>| ShellError::CantConvert {
			to_type: Self::expected_type().to_string(),
			from_type: v_ty.to_string(),
			span,
			help: Some(match err_vec.len().cmp(&N) {
				Ordering::Less => format!("input list too short ({}), expected length of {N}, add missing values", err_vec.len()),
				Ordering::Equal => {
					unreachable!("conversion would have worked if the length would be the same")
				}
				Ordering::Greater => format!("input list too long ({}), expected length of {N}, remove trailing values", err_vec.len()),
			}),
		})
	}
...
impl FromValue for std::time::Duration {
	fn from_value(v: Value) -> Result<Self, ShellError> {
		match v {
			Value::Duration { val, .. } => {
				let nanos = u64::try_from(val).map_err(|_| ShellError::NeedsPositiveValue { span: v.span() })?;
		...
		impl FromValue for $type {
			fn from_value(v: Value) -> Result<Self, ShellError> {
				...
				<$type>::try_from(int).map_err(|_| match int {
					MIN..=MAX => unreachable!("int should be within the valid range for {}", stringify!($type)),
	...
	V: FromValue,
{
	fn from_value(v: Value) -> Result<Self, ShellError> {
		...
		let items: Result<Vec<(String, V)>, ShellError> = record.into_iter().map(|(k, v)| Ok((k, V::from_value(v)?))).collect();
	...
	T: FromValue,
{
	fn from_value(v: Value) -> Result<Self, ShellError> {
		match v {
			Value::List { vals, .. } => vals.into_iter().map(|v| T::from_value(v)).collect::<Result<Vec<T>, ShellError>>(),
			v => Err(ShellError::CantConvert {
				to_type: Self::expected_type().to_string(),
				from_type: v.get_type().to_string(),
				span: v.span(),
...
impl FromValue for NuGlob {
	fn from_value(v: Value) -> Result<Self, ShellError> {
		...
		match v {
			...
			Value::Glob { val, no_expand: quoted, .. } => {
				if quoted {
	...
	T: FromValue,
{
	fn from_value(v: Value) -> Result<Self, ShellError> {
		let span = v.span();
		Ok(Spanned { item: T::from_value(v)?, span })
	}
...
mod tests {
	...
	use crate::engine::Closure;
	use crate::{FromValue, IntoValue, Record, Span, Type, Value};

	#[test]
	fn expected_type_default_impl() {
		assert_eq!(Record::expected_type(), Type::Custom("Record".to_string().into_boxed_str()));

		assert_eq!(Closure::expected_type(), Type::Custom("Closure".to_string().into_boxed_str()));
	}
	...
	fn from_value_vec_u8() {
		...
		assert_eq!(Vec::<u8>::from_value(vec.clone().into_value(span)).unwrap(), vec.clone(), "Vec<u8> roundtrip");

		assert_eq!(
			Vec::<u8>::from_value(Value::test_string(string.clone())).unwrap().deref(),
			string.as_bytes(),
			"Vec<u8> from String"
		);

		assert_eq!(Vec::<u8>::from_value(Value::test_binary(vec.clone())).unwrap(), vec, "Vec<u8> from Binary");
