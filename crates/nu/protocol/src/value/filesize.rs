...
pub const SUPPORTED_FILESIZE_UNITS: [&str; 13] = ["B", "kB", "MB", "GB", "TB", "PB", "EB", "KiB", "MiB", "GiB", "TiB", "PiB", "EiB"];
...
impl Filesize {
	...
	/// # Examples
	/// ```
	/// # use xeno_nu_protocol::{Filesize, FilesizeUnit};
	///
...
impl FilesizeUnit {
	...
	/// # Examples
	/// ```
	/// # use xeno_nu_protocol::FilesizeUnit;
...
impl FilesizeUnitFormat {
	...
	/// # Examples
	/// ```
	/// # use xeno_nu_protocol::{FilesizeUnit, FilesizeUnitFormat};
...
/// # Examples
/// ```
/// # use xeno_nu_protocol::{Filesize, FilesizeFormatter, FilesizeUnit};
...
impl FilesizeFormatter {
	...
	/// # Examples
	/// ```
	/// # use xeno_nu_protocol::{Filesize, FilesizeFormatter, FilesizeUnit, FilesizeUnitFormat};
	...
	pub fn format(&self, filesize: Filesize) -> FormattedFilesize {
		FormattedFilesize { format: *self, filesize }
	}
}
...
impl fmt::Display for FormattedFilesize {
	fn fmt(&self, mut f: &mut fmt::Formatter<'_>) -> fmt::Result {
		...
		f.write_formatted(&whole, &locale).map_err(|_| std::fmt::Error)?;
...
mod tests {
	...
	fn display_unit(#[case] bytes: i64, #[case] unit: FilesizeUnit, #[case] exp: &str) {
		assert_eq!(exp, FilesizeFormatter::new().unit(unit).format(Filesize::new(bytes)).to_string());
	}
	...
	fn display_auto_binary(#[case] val: i64, #[case] exp: &str) {
		assert_eq!(
			exp,
			FilesizeFormatter::new().unit(FilesizeUnitFormat::Binary).format(Filesize::new(val)).to_string()
		);
	}
	...
	fn display_auto_metric(#[case] val: i64, #[case] exp: &str) {
		assert_eq!(
			exp,
			FilesizeFormatter::new().unit(FilesizeUnitFormat::Metric).format(Filesize::new(val)).to_string()
		);
	}
}
