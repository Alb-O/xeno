...
// The derive macro fully qualifies paths to "xeno_nu_protocol".
...
fn named_fields_struct_roundtrip() {
	let expected = NamedFieldsStruct::make();
	let actual = NamedFieldsStruct::from_value(NamedFieldsStruct::make().into_test_value()).unwrap();
	assert_eq!(expected, actual);

	let expected = NamedFieldsStruct::value();
	let actual = NamedFieldsStruct::<u32>::from_value(NamedFieldsStruct::value()).unwrap().into_test_value();
	assert_eq!(expected, actual);
}
...
fn unnamed_fields_struct_roundtrip() {
	let expected = UnnamedFieldsStruct::make();
	let actual = UnnamedFieldsStruct::from_value(UnnamedFieldsStruct::make().into_test_value()).unwrap();
	assert_eq!(expected, actual);

	let expected = UnnamedFieldsStruct::value();
	let actual = UnnamedFieldsStruct::<f64>::from_value(UnnamedFieldsStruct::value()).unwrap().into_test_value();
	assert_eq!(expected, actual);
}
...
fn unit_struct_roundtrip() {
	...
	let expected = Value::test_nothing();
	let actual = UnitStruct::from_value(Value::test_nothing()).unwrap().into_test_value();
	assert_eq!(expected, actual);
}
...
fn enum_roundtrip() {
	...
	let expected = Enum::value();
	let actual = <[Enum; 3]>::from_value(Enum::value()).unwrap().into_test_value();
	assert_eq!(expected, actual);
}
...
fn bytes_roundtrip() {
	...
	let expected = ByteContainer::value();
	let actual = ByteContainer::from_value(ByteContainer::value()).unwrap().into_test_value();
	assert_eq!(expected, actual);
}
...
fn struct_type_name_attr() {
	...
	assert_eq!(TypeNameStruct::expected_type().to_string().as_str(), "struct");
}
...
fn renamed_field_struct_roundtrip() {
	...
	let expected = RenamedFieldStruct::value();
	let actual = RenamedFieldStruct::from_value(RenamedFieldStruct::value()).unwrap().into_test_value();
	assert_eq!(expected, actual);
}
...
fn renamed_variant_enum_roundtrip() {
	...
	let expected = RenamedVariantEnum::value();
	let actual = RenamedVariantEnum::from_value(RenamedVariantEnum::value()).unwrap().into_test_value();
	assert_eq!(expected, actual);
}
