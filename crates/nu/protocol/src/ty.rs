...
impl Type {
	...
	pub fn is_subtype_of(&self, other: &Type) -> bool {
		...
		match (self, other) {
			...
			(Type::OneOf(oneof), Type::CellPath) => oneof.iter().all(|t| t.is_subtype_of(&Type::CellPath)),
			...
			(Type::Record(this), Type::Record(that)) | (Type::Table(this), Type::Table(that)) => is_subtype_collection(this, that),
			...
			(Type::OneOf(this), that @ Type::OneOf(_)) => this.iter().all(|t| t.is_subtype_of(that)),
	...
	pub fn widen(self, other: Type) -> Type {
		...
		fn flat_widen(lhs: Type, rhs: Type) -> Result<Type, (Type, Type)> {
			Ok(match (lhs, rhs) {
				...
				(Type::Int | Type::Float | Type::Number, Type::Int | Type::Float | Type::Number) => Type::Number,

				(Type::Glob, Type::String) | (Type::String, Type::Glob) => Type::String,
				(Type::Record(this), Type::Record(that)) => Type::Record(widen_collection(this, that)),
				(Type::Table(this), Type::Table(that)) => Type::Table(widen_collection(this, that)),
				(Type::List(list_item), Type::Table(table)) | (Type::Table(table), Type::List(list_item)) => {
		...
		fn widen_collection(lhs: Box<[(String, Type)]>, rhs: Box<[(String, Type)]>) -> Box<[(String, Type)]> {
	...
	pub fn to_shape(&self) -> SyntaxShape {
		let mk_shape = |tys: &[(String, Type)]| tys.iter().map(|(key, val)| (key.clone(), val.to_shape())).collect();
			...
			Type::Record(fields) => {
				if fields.is_empty() {
					...
					write!(
						f,
						"record<{}>",
						fields.iter().map(|(x, y)| format!("{x}: {y}")).collect::<Vec<String>>().join(", "),
					)
				}
			}
			Type::Table(columns) => {
				if columns.is_empty() {
					...
					write!(
						f,
						"table<{}>",
						columns.iter().map(|(x, y)| format!("{x}: {y}")).collect::<Vec<String>>().join(", ")
					)
				}
			}
