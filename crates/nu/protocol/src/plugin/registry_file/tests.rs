...
use crate::{Category, PluginExample, PluginMetadata, PluginSignature, ShellError, Signature, SyntaxShape, Type, Value};
...
fn foo_plugin() -> PluginRegistryItem {
	PluginRegistryItem {
		...
		data: PluginRegistryItemData::Valid {
			metadata: PluginMetadata { version: Some("0.1.0".into()) },
...
fn bar_plugin() -> PluginRegistryItem {
	PluginRegistryItem {
		...
		shell: None,
		data: PluginRegistryItemData::Valid {
			metadata: PluginMetadata { version: Some("0.2.0".into()) },
			commands: vec![PluginSignature {
				sig: Signature::new("bar")
					.description("overwrites files with random data")
					.switch("force", "ignore errors", Some('f'))
					.required("path", SyntaxShape::Filepath, "file to overwrite with random data")
					.category(Category::Experimental),
				examples: vec![],
			}],
		},
