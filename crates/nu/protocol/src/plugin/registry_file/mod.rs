...
impl PluginRegistryFile {
	...
	pub fn read_from(reader: impl Read, error_span: Option<Span>) -> Result<PluginRegistryFile, ShellError> {
		...
		rmp_serde::from_read(brotli_reader).map_err(|err| ShellError::GenericError {
			...
			span: error_span,
			help: Some("it may be corrupt. Try deleting it and registering your plugins again".into()),
			inner: vec![],
		})
	}
	...
	pub fn write_to(&mut self, writer: impl Write, error_span: Option<Span>) -> Result<(), ShellError> {
		// Update the Nushell version before writing
		env!("CARGO_PKG_VERSION").clone_into(&mut self.nushell_version);

		// Format is brotli compressed messagepack
		let mut brotli_writer = brotli::CompressorWriter::new(writer, BUFFER_SIZE, COMPRESSION_QUALITY, WIN_SIZE);
	...
	pub fn upsert_plugin(&mut self, item: PluginRegistryItem) {
		if let Some(existing_item) = self.plugins.iter_mut().find(|p| p.name == item.name) {
			*existing_item = item;
			...
			self.plugins.sort_by(|item1, item2| item1.name.cmp(&item2.name));
		}
	}
}
...
impl PluginRegistryItem {
	...
	pub fn new(identity: &PluginIdentity, metadata: PluginMetadata, mut commands: Vec<PluginSignature>) -> PluginRegistryItem {
...
pub enum PluginRegistryItemData {
	...
	#[serde(serialize_with = "serialize_invalid", deserialize_with = "deserialize_invalid")]
	Invalid,
}
