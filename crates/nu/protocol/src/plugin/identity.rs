...
impl From<Spanned<InvalidPluginFilename>> for ParseError {
	fn from(error: Spanned<InvalidPluginFilename>) -> ParseError {
		ParseError::LabeledError("Invalid plugin filename".into(), "must start with `nu_plugin_`".into(), error.span)
	}
}
...
impl PluginIdentity {
	...
	/// [`PluginIdentity`].
	pub fn new(filename: impl Into<PathBuf>, shell: Option<PathBuf>) -> Result<PluginIdentity, InvalidPluginFilename> {
		...
		Ok(PluginIdentity { filename, shell, name })
	}
	...
	pub fn new_fake(name: &str) -> PluginIdentity {
		PluginIdentity::new(format!(r"C:\fake\path\nu_plugin_{name}.exe"), None).expect("fake plugin identity path is invalid")
	}
	...
	#[doc(hidden)]
	pub fn new_fake(name: &str) -> PluginIdentity {
		PluginIdentity::new(format!(r"/fake/path/nu_plugin_{name}"), None).expect("fake plugin identity path is invalid")
	}

	/// A command that could be used to add the plugin, for suggesting in errors.
	pub fn add_command(&self) -> String {
		if let Some(shell) = self.shell() {
			format!("plugin add --shell '{}' '{}'", shell.display(), self.filename().display(),)
		} else {
...
fn parses_name_from_path() {
	...
	assert_eq!("foo", PluginIdentity::new(absolute_path, Some("sh".into())).expect("should be valid").name());
