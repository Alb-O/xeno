...
use syn::{Attribute, Data, DataEnum, DataStruct, DeriveInput, Fields, Generics, Ident, Type};

use crate::attributes::{self, ContainerAttributes, MemberAttributes, ParseAttrs};
use crate::case::Case;
...
pub fn derive_from_value(input: TokenStream2) -> Result {
	...
	match input.data {
		Data::Struct(data_struct) => Ok(derive_struct_from_value(input.ident, data_struct, input.generics, input.attrs)?),
		Data::Enum(data_enum) => Ok(derive_enum_from_value(input.ident, data_enum, input.generics, input.attrs)?),
...
fn derive_struct_from_value(ident: Ident, data: DataStruct, generics: Generics, attrs: Vec<Attribute>) -> Result {
	...
	let expected_type_impl = struct_expected_type(&data.fields, container_attrs.type_name.as_deref(), &container_attrs)?;
	Ok(quote! {
		#[automatically_derived]
		impl #impl_generics xeno_nu_protocol::FromValue for #ident #ty_generics #where_clause {
			#from_value_impl
			#expected_type_impl
		}
	})
...
/// }
///
/// impl xeno_nu_protocol::FromValue for Pet {
///     fn from_value(
///         v: xeno_nu_protocol::Value
///     ) -> std::result::Result<Self, xeno_nu_protocol::ShellError> {
///         let span = v.span();
///         let mut record = v.into_record()?;
///         std::result::Result::Ok(Pet {
///             name: <String as xeno_nu_protocol::FromValue>::from_value(
///                 record
///                     .remove("name")
///                     .ok_or_else(|| xeno_nu_protocol::ShellError::CantFindColumn {
///                         col_name: std::string::ToString::to_string("name"),
///                         span: std::option::Option::None,
///                         src_span: span
///                     })?,
///             )?,
///             age: <u8 as xeno_nu_protocol::FromValue>::from_value(
///                 record
///                     .remove("age")
///                     .ok_or_else(|| xeno_nu_protocol::ShellError::CantFindColumn {
...
///             favorite_toy: record
///                 .remove("favorite_toy")
///                 .map(|v| <#ty as xeno_nu_protocol::FromValue>::from_value(v))
///                 .transpose()?
///                 .flatten(),
///         })
///     }
...
///
/// impl xeno_nu_protocol::FromValue for Color {
///     fn from_value(
///         v: xeno_nu_protocol::Value
///     ) -> std::result::Result<Self, xeno_nu_protocol::ShellError> {
///         let span = v.span();
...
///             {
///                 <u8 as xeno_nu_protocol::FromValue>::from_value(
///                     deque
///                         .pop_front()
///                         .ok_or_else(|| xeno_nu_protocol::ShellError::CantFindColumn {
...
/// struct Unicorn;
///
/// impl xeno_nu_protocol::FromValue for Unicorn {
///     fn from_value(
///         v: xeno_nu_protocol::Value
///     ) -> std::result::Result<Self, xeno_nu_protocol::ShellError> {
///         match v {
///             xeno_nu_protocol::Value::Nothing {..} => Ok(Self),
///             v => std::result::Result::Err(xeno_nu_protocol::ShellError::CantConvert {
///                 to_type: std::string::ToString::to_string(&<Self as xeno_nu_protocol::FromValue>::expected_type()),
...
///
/// impl xeno_nu_protocol::FromValue for Pet {
///     fn expected_type() -> xeno_nu_protocol::Type {
///         xeno_nu_protocol::Type::Record(
///             std::vec![
///                 (
///                     std::string::ToString::to_string("name"),
///                     <String as xeno_nu_protocol::FromValue>::expected_type(),
///                 ),
///                 (
///                     std::string::ToString::to_string("age"),
///                     <u8 as xeno_nu_protocol::FromValue>::expected_type(),
///                 ),
///                 (
///                     std::string::ToString::to_string("toy"),
///                     <Option<String> as xeno_nu_protocol::FromValue>::expected_type(),
///                 )
///             ].into_boxed_slice()
///         )
///     }
...
/// struct Parrot;
///
/// impl xeno_nu_protocol::FromValue for Parrot {
///     fn expected_type() -> xeno_nu_protocol::Type {
///         xeno_nu_protocol::Type::Custom(
...
/// }
/// ```
fn struct_expected_type(fields: &Fields, attr_type_name: Option<&str>, container_attrs: &ContainerAttributes) -> Result {
	let ty = match (fields, attr_type_name) {
		(_, Some(type_name)) => {
			quote!(xeno_nu_protocol::Type::Custom(
		...
		(Fields::Named(fields), _) => {
			...
			for field in fields.named.iter() {
				...
				fields_ts.push(quote! {(
					...
					<#ty as xeno_nu_protocol::FromValue>::expected_type(),
				)});
			}
			quote!(xeno_nu_protocol::Type::Record(std::vec![#(#fields_ts),*].into_boxed_slice()))
		}
		(f @ Fields::Unnamed(fields), _) => {
			...
			let fields = fields.unnamed.iter().map(|field| {
				let ty = &field.ty;
				quote!(<#ty as xeno_nu_protocol::FromValue>::expected_type())
			});
		...
		(Fields::Unit, _) => quote!(xeno_nu_protocol::Type::Nothing),
	};
...
fn derive_enum_from_value(ident: Ident, data: DataEnum, generics: Generics, attrs: Vec<Attribute>) -> Result {
	let container_attrs = ContainerAttributes::parse_attrs(attrs.iter())?;
	let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
...
/// }
///
/// impl xeno_nu_protocol::IntoValue for Weather {
///     fn into_value(self, span: xeno_nu_protocol::Span) -> xeno_nu_protocol::Value {
///         let span = v.span();
///         let ty = v.get_type();
///
///         let s = v.into_string()?;
...
///             _ => std::result::Result::Err(xeno_nu_protocol::ShellError::CantConvert {
///                 to_type: std::string::ToString::to_string(
///                     &<Self as xeno_nu_protocol::FromValue>::expected_type()
///                 ),
	...
	let arms: Vec<TokenStream2> = data
		...
		.map(|variant| {
			...
			match &variant.fields {
				Fields::Named(fields) => Err(DeriveError::UnsupportedEnums { fields_span: fields.span() }),
				Fields::Unnamed(fields) => Err(DeriveError::UnsupportedEnums { fields_span: fields.span() }),
...
/// }
///
/// impl xeno_nu_protocol::FromValue for Weather {
///     fn expected_type() -> xeno_nu_protocol::Type {
///         xeno_nu_protocol::Type::Custom(
...
/// generation.
fn parse_value_via_fields(fields: &Fields, self_ident: impl ToTokens, container_attrs: &ContainerAttributes) -> Result {
	match fields {
		Fields::Named(fields) => {
			let mut name_resolver = NameResolver::new();
			...
			for field in fields.named.iter() {
				...
				fields_ts.push(match (type_is_option(ty), member_attrs.default) {
					...
					(false, false) => quote! {
						#ident: <#ty as xeno_nu_protocol::FromValue>::from_value(
							record
								.remove(#ident_s)
								.ok_or_else(|| xeno_nu_protocol::ShellError::CantFindColumn {
		...
		f @ Fields::Unnamed(fields) => {
			...
			let fields = fields.unnamed.iter().enumerate().map(|(i, field)| {
				let ty = &field.ty;
				quote! {{
					<#ty as xeno_nu_protocol::FromValue>::from_value(
						deque
							.pop_front()
							.ok_or_else(|| xeno_nu_protocol::ShellError::CantFindColumn {
		...
		Fields::Unit => Ok(quote! {
			match v {
				xeno_nu_protocol::Value::Nothing {..} => Ok(#self_ident),
				v => std::result::Result::Err(xeno_nu_protocol::ShellError::CantConvert {
					to_type: std::string::ToString::to_string(&<Self as xeno_nu_protocol::FromValue>::expected_type()),
					from_type: std::string::ToString::to_string(&v.get_type()),
					span: v.span(),
...
fn type_is_option(ty: &Type) -> bool {
	...
	s.starts_with(PRELUDE_OPTION) || s.starts_with(PARTIALLY_QUALIFIED_OPTION) || s.starts_with(FULLY_QUALIFIED_OPTION)
}
