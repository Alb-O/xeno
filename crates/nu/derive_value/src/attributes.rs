...
use crate::HELPER_ATTRIBUTE;
use crate::case::Case;
use crate::error::DeriveError;

pub trait ParseAttrs: Default {
	fn parse_attrs<'a, M>(iter: impl IntoIterator<Item = &'a Attribute>) -> Result<Self, DeriveError<M>> {
...
impl ParseAttrs for ContainerAttributes {
	fn parse_attr<M>(&mut self, attr_meta: ParseNestedMeta<'_>) -> Result<(), DeriveError<M>> {
		...
		match ident.to_string().as_str() {
			...
			ident => {
				return Err(DeriveError::UnexpectedAttribute { meta_span: ident.span() });
			}
		}
...
pub fn filter<'a>(iter: impl Iterator<Item = &'a Attribute>) -> impl Iterator<Item = &'a Attribute> {
...
pub fn deny<M>(attrs: &[Attribute]) -> Result<(), DeriveError<M>> {
	match filter(attrs.iter()).next() {
		Some(attr) => Err(DeriveError::InvalidAttributePosition { attribute_span: attr.span() }),
		None => Ok(()),
	}
}
