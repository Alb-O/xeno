...
pub enum DeriveError<M> {
	...
	InvalidAttributeValue { value_span: Span, value: Box<dyn Debug> },

	/// Two keys or variants are called the same name breaking bidirectionality.
	NonUniqueName { name: String, first: Span, second: Span },
}
...
impl<M> From<DeriveError<M>> for Diagnostic {
	fn from(value: DeriveError<M>) -> Self {
		...
		match value {
			...
			DeriveError::UnsupportedUnions => Diagnostic::new(Level::Error, format!("`{derive_name}` cannot be derived from unions"))
			...
			DeriveError::UnsupportedEnums { fields_span } => {
				Diagnostic::spanned(fields_span, Level::Error, format!("`{derive_name}` can only be derived from plain enums"))
					.help("consider refactoring your data type to a struct with a plain enum as a field".to_string())
					.note("more complex enums could be implemented in the future".to_string())
			}

			DeriveError::InvalidAttributePosition { attribute_span } => {
				Diagnostic::spanned(attribute_span, Level::Error, "invalid attribute position".to_string())
					.help(format!("check documentation for `{derive_name}` for valid placements"))
			}

			DeriveError::UnexpectedAttribute { meta_span } => Diagnostic::spanned(meta_span, Level::Error, "unknown attribute".to_string())
				.help(format!("check documentation for `{derive_name}` for valid attributes")),

			DeriveError::InvalidAttributeValue { value_span, value } => Diagnostic::spanned(value_span, Level::Error, format!("invalid value {value:?}"))
				.help(format!("check documentation for `{derive_name}` for valid attribute values")),

			DeriveError::NonUniqueName { name, first, second } => Diagnostic::new(Level::Error, format!("non-unique name {name:?} found"))
