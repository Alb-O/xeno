...
pub mod users {
	...
	pub fn get_user_groups(username: &str, gid: Gid) -> Option<Vec<Gid>> {
		...
		#[cfg(target_os = "macos")]
		let res = unsafe { nix::libc::getgrouplist(name.as_ptr(), gid.as_raw() as i32, buff.as_mut_ptr(), &mut count) };

		#[cfg(not(target_os = "macos"))]
		let res = unsafe { nix::libc::getgrouplist(name.as_ptr(), gid.as_raw(), buff.as_mut_ptr(), &mut count) };
