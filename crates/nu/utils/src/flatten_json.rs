...
/// use xeno_nu_utils;
///
/// let flattener = xeno_nu_utils::JsonFlattener { ..Default::default() };
/// ```
pub struct JsonFlattener<'a> {
	...
	/// ```
	/// use xeno_nu_utils;
	/// let flattener = xeno_nu_utils::JsonFlattener { separator: "_", ..Default::default()};
	/// ```
	pub separator: &'a str,
	...
	/// ```
	/// use xeno_nu_utils;
	/// let flattener = xeno_nu_utils::JsonFlattener { alt_array_flattening: true, ..Default::default()};
	/// ```
	...
	/// ```
	/// use xeno_nu_utils;
	/// let flattener = xeno_nu_utils::JsonFlattener { preserve_arrays: true, ..Default::default()};
	/// ```
...
/// use serde_json::json;
///
/// let flattener = xeno_nu_utils::JsonFlattener::new();
/// let example = json!({
///     "a": {
///         "b": "c"
///     }
...
impl JsonFlattener<'_> {
	...
	pub fn flatten(&self, json: &SerdeValue) -> SerdeValue {
		...
		match json {
			SerdeValue::Array(obj_arr) => self.flatten_array(&mut flattened_val, &"".to_string(), obj_arr),
			SerdeValue::Object(obj_val) => self.flatten_object(&mut flattened_val, None, obj_val, false),
			_ => self.flatten_value(&mut flattened_val, &"".to_string(), json, false),
		}
		SerdeValue::Object(flattened_val)
	}

	fn flatten_object(&self, builder: &mut Map<String, SerdeValue>, identifier: Option<&String>, obj: &Map<String, SerdeValue>, arr: bool) {
		for (k, v) in obj {
			let expanded_identifier = identifier.map_or_else(|| k.clone(), |identifier| format!("{identifier}{}{k}", self.separator));

			if expanded_identifier.contains("span.start") || expanded_identifier.contains("span.end") {
				continue;
			}
			...
			match v {
				SerdeValue::Object(obj_val) => self.flatten_object(builder, Some(&expanded_identifier), obj_val, arr),
				SerdeValue::Array(obj_arr) => self.flatten_array(builder, &expanded_identifier, obj_arr),
				_ => self.flatten_value(builder, &expanded_identifier, v, arr),
			}
		}
	}

	fn flatten_array(&self, builder: &mut Map<String, SerdeValue>, identifier: &String, obj: &[SerdeValue]) {
		for (k, v) in obj.iter().enumerate() {
			...
			match v {
				SerdeValue::Object(obj_val) => self.flatten_object(
					builder,
					Some(if self.preserve_arrays { &with_key } else { identifier }),
					obj_val,
					self.alt_array_flattening,
				),
				SerdeValue::Array(obj_arr) => self.flatten_array(builder, if self.preserve_arrays { &with_key } else { identifier }, obj_arr),
				_ => self.flatten_value(builder, if self.preserve_arrays { &with_key } else { identifier }, v, self.alt_array_flattening),
			}
		}
	}

	fn flatten_value(&self, builder: &mut Map<String, SerdeValue>, identifier: &String, obj: &SerdeValue, arr: bool) {
		if let Some(v) = builder.get_mut(identifier) {
			...
			builder.insert(identifier.to_string(), if arr { json!(vec![obj.clone()]) } else { obj.clone() });
		}
	}
	...
	fn filter_known_keys(&self, key: &str) -> String {
		...
		if filtered_key.contains(".Wrap.try_to_keep_words") {
			filtered_key = filtered_key.replace(".Wrap.try_to_keep_words", ".wrapping_try_keep_words");
		}
