...
use xeno_nu_engine::eval_expression;
...
impl Command for Let {
	...
	fn signature(&self) -> xeno_nu_protocol::Signature {
		Signature::build("let")
			...
			.required("var_name", SyntaxShape::VarWithOptType, "The variable name to create.")
			.optional(
				"initial_value",
  ...
  https://www.nushell.sh/book/thinking_in_nu.html"#
	...
	fn run(&self, engine_state: &EngineState, stack: &mut Stack, call: &Call, input: PipelineData) -> Result<PipelineData, ShellError> {
		let expr = call.positional_nth(stack, 0).ok_or(ShellError::NushellFailed {
		...
		let rhs = if let Some(ref initial_value_expr) = initial_value {
			...
			if let Expr::Block(block_id) | Expr::Subexpression(block_id) = &initial_value_expr.expr {
				...
				if block.pipelines.iter().any(|pipeline| pipeline.elements.len() > 1) {
