...
use xeno_nu_engine::{find_in_dirs_env, get_dirs_var_from_call, get_eval_block, redirect_env};
use xeno_nu_protocol::ast::{Expr, Expression};
use xeno_nu_protocol::engine::CommandType;
...
impl Command for ExportUse {
	...
	fn signature(&self) -> xeno_nu_protocol::Signature {
		Signature::build("export use")
			.input_output_types(vec![(Type::Nothing, Type::Nothing)])
			.required("module", SyntaxShape::String, "Module or module file.")
			.rest("members", SyntaxShape::Any, "Which members of the module to import.")
	...
	fn run(&self, engine_state: &EngineState, caller_stack: &mut Stack, call: &Call, input: PipelineData) -> Result<PipelineData, ShellError> {
		...
		if let Some(module_id) = import_pattern.head.id {
			...
			if let Some(block_id) = module.env_block {
				...
				let module_arg_str = String::from_utf8_lossy(engine_state.get_span_contents(import_pattern.head.span));

				let maybe_file_path_or_dir = find_in_dirs_env(&module_arg_str, engine_state, caller_stack, get_dirs_var_from_call(caller_stack, call))?;
				...
				let mut callee_stack = caller_stack.gather_captures(engine_state, &block.captures).reset_pipes();
			...
			return Err(ShellError::GenericError {
				error: format!("Could not import from '{}'", String::from_utf8_lossy(&import_pattern.head.name)),
