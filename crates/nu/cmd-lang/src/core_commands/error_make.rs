...
use xeno_nu_protocol::{ErrorLabel, ErrorSource, FromValue, IntoValue, LabeledError};
...
the most recent `error make`."
	...
	fn run(&self, engine_state: &EngineState, stack: &mut Stack, call: &Call, input: PipelineData) -> Result<PipelineData, ShellError> {
		...
		Err(match (inners, value) {
			...
			(inner, Value::Record { val, internal_span, .. }) => {
...
impl ErrorInfo {
	...
	pub fn labeled(self, span: Span, show_labels: bool) -> ShellError {
		...
		match self {
			...
			// Normal error
			ei @ ErrorInfo { src: None, raw: None, .. } => LabeledError {
				labels: match (show_labels, labels.as_slice()) {
					(true, []) => vec![ErrorLabel { text: "".into(), span }],
