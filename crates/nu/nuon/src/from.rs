...
use xeno_nu_protocol::ast::{Expr, Expression, ListItem, RecordItem};
use xeno_nu_protocol::engine::{EngineState, StateWorkingSet};
use xeno_nu_protocol::{Filesize, IntoValue, Range, Record, ShellError, Span, Type, Unit, Value};
...
pub fn from_nuon(input: &str, span: Option<Span>) -> Result<Value, ShellError> {
	...
	let mut block = xeno_nu_parser::parse(&mut working_set, None, input.as_bytes(), false);
	...
	let expr = if block.pipelines.is_empty() {
		Expression::new(&mut working_set, Expr::Nothing, span.unwrap_or(Span::unknown()), Type::Nothing)
	} else {
...
fn convert_to_value(expr: Expression, span: Span, original_text: &str) -> Result<Value, ShellError> {
	match expr.expr {
		...
		Expr::Keyword(kw) => Err(ShellError::OutsideSpannedLabeledError {
			src: original_text.to_string(),
			error: "Error when loading".into(),
			msg: format!("{} not supported in nuon", String::from_utf8_lossy(&kw.keyword)),
			span: expr.span,
		}),
		Expr::List(vals) => {
			let mut output = vec![];
		...
		Expr::Range(range) => {
			...
			Ok(Value::range(Range::new(from, next, to, range.operator.inclusion, expr.span)?, expr.span))
		}
		...
		Expr::Table(mut table) => {
			...
			for row in table.rows.into_vec() {
				...
				let record = cols
					.iter()
					.zip(row.into_vec())
					.map(|(col, cell)| convert_to_value(cell, span, original_text).map(|val| (col.clone(), val)))
					.collect::<Result<_, _>>()?;
