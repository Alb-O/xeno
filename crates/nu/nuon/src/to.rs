...
use xeno_nu_engine::get_columns;
use xeno_nu_protocol::engine::EngineState;
use xeno_nu_protocol::{Range, ShellError, Span, Value};
use xeno_nu_utils::{ObviousFloat, as_raw_string, escape_quote_string, needs_quoting};
...
/// # Example
/// ```ignore
/// use xeno_nu_nuon::{to_nuon, ToNuonConfig, ToStyle};
///
/// let config = ToNuonConfig::default()
///     .style(ToStyle::Spaces(2))
///     .raw_strings(true);
...
/// ```
pub fn to_nuon(engine_state: &EngineState, input: &Value, config: ToNuonConfig) -> Result<String, ShellError> {
	...
	let res = value_to_string(engine_state, input, span, 0, indentation.as_deref(), config.serialize_types, config.raw_strings)?;
...
fn value_to_string(
	...
	match v {
		...
		Value::Closure { val, .. } => {
			if serialize_types {
				Ok(quote_string(&val.coerce_into_string(engine_state, span)?, raw_strings))
			} else {
		...
		Value::List { vals, .. } => {
			...
			if !headers.is_empty() && vals.iter().all(|x| x.columns().eq(headers.iter())) {
				...
				let headers: Vec<String> = headers
					.iter()
					.map(|string| {
						let string = if needs_quoting(string) { &escape_quote_string(string) } else { string };
						format!("{idt}{string}")
					})
					.collect();
				...
				for val in vals {
					collection.push(format!(
						"{idt_po}{}",
						value_to_string_without_quotes(engine_state, val, span, depth + 1, indent, serialize_types, raw_strings,)?
					));
				}
				Ok(format!("[{nl}{}{nl}{idt}]", collection.join(&format!(",{sep}{nl}"))))
			}
		}
		...
		Value::Record { val, .. } => {
			let mut collection = vec![];
			for (col, val) in &**val {
				let col = if needs_quoting(col) { &escape_quote_string(col) } else { col };
				collection.push(format!(
					"{idt_po}{col}:{kv_sep}{}",
					value_to_string_without_quotes(engine_state, val, span, depth + 1, indent, serialize_types, raw_strings,)?
				));
...
fn value_to_string_without_quotes(
	...
	match v {
		Value::String { val, .. } => Ok({ if needs_quoting(val) { quote_string(val, raw_strings) } else { val.clone() } }),
		_ => value_to_string(engine_state, v, span, depth, indent, serialize_types, raw_strings),
	}
}
