...
pub use to::{ToNuonConfig, ToStyle, to_nuon};

#[cfg(test)]
mod tests {
	use chrono::DateTime;
	use xeno_nu_protocol::ast::{CellPath, PathMember, RangeInclusion};
	use xeno_nu_protocol::casing::Casing;
	use xeno_nu_protocol::engine::{Closure, EngineState};
	use xeno_nu_protocol::{BlockId, IntRange, Range, Span, Value, record};
	...
	fn nuon_end_to_end(input: &str, middle: Option<Value>) {
		...
		assert_eq!(to_nuon(&engine_state, &val, ToNuonConfig::default()).unwrap(), input);
	}
	...
	fn list_of_numbers() {
		nuon_end_to_end(
			"[1, 2, 3]",
			Some(Value::test_list(vec![Value::test_int(1), Value::test_int(2), Value::test_int(3)])),
		);
	}
	...
	#[test]
	fn escaping3() {
		nuon_end_to_end(r#"[hello\\world]"#, Some(Value::test_list(vec![Value::test_string(r#"hello\\world"#)])));
	}
	...
	#[test]
	fn to_nuon_datetime() {
		nuon_end_to_end("1970-01-01T00:00:00+00:00", Some(Value::test_date(DateTime::UNIX_EPOCH.into())));
	}
	...
	#[test]
	fn binary() {
		nuon_end_to_end("0x[ABCDEF]", Some(Value::test_binary(vec![0xab, 0xcd, 0xef])));
	}
	...
	fn binary_roundtrip() {
		...
		assert_eq!(
			to_nuon(&engine_state, &from_nuon("0x[1f ff]", None).unwrap(), ToNuonConfig::default(),).unwrap(),
			"0x[1FFF]"
		);
	}
	...
	fn read_sample_data() {
		assert_eq!(
			from_nuon(include_str!("../../../tests/fixtures/formats/sample.nuon"), None,).unwrap(),
			Value::test_list(vec![
				Value::test_list(vec![
					Value::test_record(record!(
						"a" => Value::test_int(1),
	...
	fn float_doesnt_become_int() {
		...
		assert_eq!(to_nuon(&engine_state, &Value::test_float(1.0), ToNuonConfig::default(),).unwrap(), "1.0");
	}
	...
	fn float_inf_parsed_properly() {
		...
		assert_eq!(
			to_nuon(&engine_state, &Value::test_float(f64::INFINITY), ToNuonConfig::default(),).unwrap(),
			"inf"
		);
	}
	...
	fn float_neg_inf_parsed_properly() {
		...
		assert_eq!(
			to_nuon(&engine_state, &Value::test_float(f64::NEG_INFINITY), ToNuonConfig::default(),).unwrap(),
			"-inf"
		);
	}
	...
	fn float_nan_parsed_properly() {
		...
		assert_eq!(to_nuon(&engine_state, &Value::test_float(-f64::NAN), ToNuonConfig::default(),).unwrap(), "NaN");
	}
	...
	fn to_nuon_quotes_empty_string() {
		...
		let res = to_nuon(&engine_state, &Value::test_string(""), ToNuonConfig::default());
		assert!(res.is_ok());
	...
	fn to_nuon_quotes_empty_string_in_list() {
		nuon_end_to_end(r#"[""]"#, Some(Value::test_list(vec![Value::test_string("")])));
	}
	...
	fn cell_path() {
		nuon_end_to_end(
			...
			Some(Value::test_cell_path(CellPath {
				members: vec![
					PathMember::string("foo".to_string(), false, Casing::Sensitive, Span::new(2, 5)),
					PathMember::string("bar".to_string(), false, Casing::Sensitive, Span::new(6, 9)),
	...
	fn read_code_should_fail_rather_than_panic() {
		assert!(from_nuon(include_str!("../../../tests/fixtures/formats/code.nu"), None,).is_err());
	}
	...
	fn raw_strings_option_generates_raw() {
		...
		let result = to_nuon(&engine_state, &val, ToNuonConfig::default().raw_strings(true)).unwrap();
	...
	fn raw_strings_combined_with_raw_style() {
		...
		let result = to_nuon(&engine_state, &val, ToNuonConfig::default().style(ToStyle::Raw).raw_strings(true)).unwrap();
	...
	fn raw_strings_roundtrip_with_raw_strings_option() {
		...
		let nuon = to_nuon(&engine_state, &original, ToNuonConfig::default().raw_strings(true)).unwrap();
