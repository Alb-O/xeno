...
impl TextPredicate {
	...
	pub fn satisfied<I: Input>(&self, input: &mut I, matched_nodes: &[MatchedNode], query: &Query) -> bool {
		let mut capture_nodes = matched_nodes.iter().filter(|matched_node| matched_node.capture == self.capture);
		match self.kind {
			...
			TextPredicateKind::EqCapture(other_capture) => {
				let mut other_nodes = matched_nodes.iter().filter(|matched_node| matched_node.capture == other_capture);

				let res = self.satisfied_helper(zip(&mut capture_nodes, &mut other_nodes).map(|(node1, node2)| {
			...
			TextPredicateKind::MatchString(ref regex) => self.satisfied_helper(capture_nodes.map(|node| {
				let range = node.node.byte_range();
		...
		for predicate in predicates {
			...
			match predicate.name() {
				...
				"any-of?" | "not-any-of?" => {
					...
					let values: Result<_, InvalidPredicateError> = (1..predicate.num_args()).map(|i| predicate.query_str_arg(i)).collect();
					self.text_predicates.push(TextPredicate {
						capture,
...
impl<'a> Predicate<'a> {
	unsafe fn new(query: &'a Query, predicate: &'a [PredicateStep]) -> Result<Predicate<'a>, InvalidPredicateError> {
		ensure!(
	...
	pub fn check_arg_count(&self, n: usize) -> Result<(), InvalidPredicateError> {
		ensure!(self.args.len() == n, "expected {n} arguments for #{}, got {}", self.name(), self.args.len());
		Ok(())
	}
	...
	pub fn capture_arg(&self, i: usize) -> Result<Capture, InvalidPredicateError> {
		match self.arg(i) {
			...
			PredicateArg::String(str) => bail!("{i}. argument to #{} expected a capture, got literal {:?}", self.name(), str.get(self.query)),
		}
	}
...
impl InvalidPredicateError {
	pub fn unknown(predicate: UserPredicate) -> Self {
		match predicate {
			UserPredicate::IsPropertySet { key, .. } => Self::UnknownProperty { property: key.into() },
			UserPredicate::SetProperty { key, .. } => Self::UnknownProperty { property: key.into() },
			UserPredicate::Other(predicate) => Self::UnknownPredicate { name: predicate.name().into() },
		}
	}
}
...
impl From<String> for InvalidPredicateError {
	fn from(value: String) -> Self {
		InvalidPredicateError::Other { msg: value.into_boxed_str() }
	}
}
...
unsafe extern "C" {
	...
	fn ts_query_predicates_for_pattern(query: NonNull<QueryData>, pattern_index: u32, step_count: &mut u32) -> *const PredicateStep;
