...
unsafe fn with_cache<T>(f: impl FnOnce(&mut Vec<InactiveQueryCursor>) -> T) -> T {
	CURSOR_CACHE.with(|cache| unsafe { f(&mut *cache.get()) })
}
...
impl<'tree, I: Input> QueryCursor<'_, 'tree, I> {
	pub fn next_match(&mut self) -> Option<QueryMatch<'_, 'tree>> {
		...
		loop {
			let success = unsafe { ts_query_cursor_next_match(self.ptr.as_ptr(), &mut query_match) };
			if !success {
				return None;
			}
			let matched_nodes = unsafe { slice::from_raw_parts(query_match.captures.cast(), query_match.capture_count as usize) };
	...
	pub fn next_matched_node(&mut self) -> Option<(QueryMatch<'_, 'tree>, MatchedNodeIdx)> {
		...
		loop {
			let success = unsafe { ts_query_cursor_next_capture(self.ptr.as_ptr(), &mut query_match, &mut capture_idx) };
			if !success {
				return None;
			}
			let matched_nodes = unsafe { slice::from_raw_parts(query_match.captures.cast(), query_match.capture_count as usize) };
...
impl<'tree> QueryMatch<'_, 'tree> {
	...
	pub fn nodes_for_capture(&self, capture: Capture) -> impl Iterator<Item = &Node<'tree>> {
		self.matched_nodes.iter().filter(move |mat| mat.capture == capture).map(|mat| &mat.node)
	}
...
unsafe extern "C" {
	...
	fn ts_query_cursor_next_capture(self_: *mut QueryCursorData, match_: &mut TSQueryMatch, capture_index: &mut u32) -> bool;
