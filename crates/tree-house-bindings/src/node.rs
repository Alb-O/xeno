...
impl<'tree> Node<'tree> {
	...
	pub fn kind(&self) -> &'tree str {
		unsafe { CStr::from_ptr(ts_node_type(self.as_raw())) }.to_str().unwrap()
	}
	...
	pub fn is_contained_within(&self, range: Range<u32>) -> bool {
		(self.start_byte() > range.start && self.end_byte() <= range.end) || (self.start_byte() >= range.start && self.end_byte() < range.end)
	}
	...
	pub fn named_descendant_for_byte_range(&self, start: u32, end: u32) -> Option<Self> {
		unsafe { Self::from_raw(ts_node_named_descendant_for_byte_range(self.as_raw(), start, end)) }
	}
