...
impl Parser {
	...
	pub fn set_included_ranges(&mut self, ranges: &[Range]) -> Result<(), InvalidRangesError> {
		...
		// tree sitter range
		let success = unsafe { ts_parser_set_included_ranges(self.ptr, ranges.as_ptr(), ranges.len() as u32) };
		if success { Ok(()) } else { Err(InvalidRangesError) }
	}
	...
	pub fn parse<I: Input>(&mut self, input: impl IntoInput<Input = I>, old_tree: Option<&Tree>) -> Option<Tree> {
		...
		unsafe extern "C" fn read<C: Input>(payload: NonNull<c_void>, byte_index: u32, _position: Point, bytes_read: *mut u32) -> *const u8 {
			let cursor = catch_unwind(AssertUnwindSafe(move || {
				let input: &mut C = unsafe { payload.cast().as_mut() };
			...
			match cursor {
				Ok((chunk_offset, ptr, len)) if chunk_offset < len => unsafe {
...
type TreeSitterReadFn = unsafe extern "C" fn(payload: NonNull<c_void>, byte_index: u32, position: Point, bytes_read: *mut u32) -> *const u8;

/// A function that reads one code point from the given string, returning the number of bytes
/// consumed.
type DecodeInputFn = unsafe extern "C" fn(string: *const u8, length: u32, code_point: *const i32) -> u32;
...
unsafe extern "C" {
	...
	/// returns `true`
	fn ts_parser_set_included_ranges(parser: NonNull<ParserData>, ranges: *const Range, count: u32) -> bool;

	fn ts_parser_parse(parser: NonNull<ParserData>, old_tree: Option<NonNull<SyntaxTreeData>>, input: ParserInputRaw) -> Option<NonNull<SyntaxTreeData>>;
