...
impl Grammar {
	...
	pub unsafe fn new(name: &str, library_path: &Path) -> Result<Grammar, Error> {
		...
		let language_fn: Symbol<unsafe extern "C" fn() -> NonNull<GrammarData>> = unsafe {
			library
				.get(language_fn_name.as_bytes())
				.map_err(|err| Error::DlSym { err, symbol: name.to_owned() })?
		};
		let grammar = Grammar::from_grammar_data(unsafe { language_fn() })?;
...
impl fmt::Display for IncompatibleGrammarError {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		write!(f, "Tried to load grammar with incompatible ABI version {}.", self.abi_version,)
	}
}
