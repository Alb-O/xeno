...
pub enum UserPredicate<'a> {
	IsPropertySet { negate: bool, key: &'a str, val: Option<&'a str> },
	SetProperty { key: &'a str, val: Option<&'a str> },
	Other(Predicate<'a>),
}
...
impl Query {
	...
	pub fn new(
		...
		// Compile the query.
		let ptr = unsafe { ts_query_new(grammar, bytes.as_ptr(), bytes.len() as u32, &mut error_offset, &mut error_kind) };
		...
		let Some(raw) = ptr else {
			...
			let err = match error_kind {
				...
				RawQueryError::Syntax => ParseError::SyntaxError(ParserErrorLocation::new(source, offset, 0)),
				RawQueryError::Structure => ParseError::ImpossiblePattern(ParserErrorLocation::new(source, offset, 0)),
				RawQueryError::None => {
		...
		let patterns: Result<_, ParseError> = (0..num_patterns)
			.map(|pattern| {
				query.parse_pattern_predicates(Pattern(pattern), &mut custom_predicate).map_err(|err| {
					let pattern_start = unsafe { ts_query_start_byte_for_pattern(query.raw, pattern) as usize };
					match err {
	...
	pub fn new(source: &str, start: usize, len: usize) -> ParserErrorLocation {
		...
		for (this_line_no, this_line) in source.split('\n').enumerate() {
			...
			if line_start <= start && start <= line_end {
				line = this_line_no;
				line_content = this_line.strip_suffix('\r').unwrap_or(this_line).to_string();
				column = source[line_start..start].chars().count();
				line_before = source[..line_start].lines().next_back().filter(|s| !s.is_empty()).map(ToOwned::to_owned);
				line_after = source
					.get(line_end + 1..)
...
impl Display for ParserErrorLocation {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		...
		let max_line_number = if self.line_after.is_some() { self.line + 2 } else { self.line + 1 };
		...
		writeln!(f, "{prefix}{:width$} {:^<len$}", "", "^", width = self.column as usize, len = self.len as usize)?;
...
pub enum ParseError {
	...
	InvalidNodeType { node: String, location: ParserErrorLocation },
	#[error("invalid field name {field:?}\n{location}")]
	InvalidFieldName { field: String, location: ParserErrorLocation },
	#[error("invalid capture name {capture:?}\n{location}")]
	InvalidCaptureName { capture: String, location: ParserErrorLocation },
	#[error("{message}\n{location}")]
	InvalidPredicate { message: String, location: ParserErrorLocation },
...
unsafe extern "C" {
	...
	fn ts_query_new(grammar: Grammar, source: *const u8, source_len: u32, error_offset: &mut u32, error_type: &mut RawQueryError)
	...
	fn ts_query_capture_name_for_id(query: NonNull<QueryData>, index: u32, length: &mut u32) -> *const u8;

	fn ts_query_string_value_for_id(self_: NonNull<QueryData>, index: u32, length: &mut u32) -> *const u8;
