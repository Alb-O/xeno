...
mod tests {
	...
	fn test_utf16_with_emoji() {
		// Emoji like 😀 is a single Unicode codepoint but 2 UTF-16 code units
		...
		let char_idx = lsp_position_to_char(&text, pos, encoding).unwrap();
		assert_eq!(char_idx, 2); // In rope chars: a=0, 😀=1, b=2
	...
	fn test_utf8_with_multibyte() {
		...
		// Position at 'é' (char index 3, but é is at byte offset 4 and takes 2 bytes)
		// Actually "café" = c(1) + a(1) + f(1) + é(2) = 5 bytes
		let char_idx = 3; // 'é' position
		let back = char_to_lsp_position(&text, char_idx, encoding).unwrap();
		assert_eq!(back.character, 3); // byte offset of 'é' is 3
	}
