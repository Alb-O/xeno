...
fn command_completion_shows_menu() {
	...
	run_with_timeout(TEST_TIMEOUT, || {
		with_kitty_capture(&workspace_dir(), &xeno_cmd(), |kitty| {
			...
			kitty_send_keys!(kitty, KeyCode::Char(':'), KeyCode::Char('w'), KeyCode::Char('r'),);

			// Should show 'write' in completion menu with its kind icon/label
			let (_raw, clean) = wait_for_screen_text_clean(kitty, Duration::from_secs(3), |_r, clean| clean.contains("write") && clean.contains("Cmd"));

			assert!(clean.contains("write"), "Completion menu should show 'write'. Clean: {clean:?}");
			assert!(clean.contains("Cmd"), "Completion menu should show 'Cmd' kind. Clean: {clean:?}");
			...
			assert!(clean.contains(":write"), "Command line should be filled with 'write'. Clean: {clean:?}");
		});
	});
}
...
fn theme_completion_appends_to_command() {
	...
	run_with_timeout(TEST_TIMEOUT, || {
		with_kitty_capture(&workspace_dir(), &xeno_cmd(), |kitty| {
			...
			let (_raw, clean) = wait_for_screen_text_clean(kitty, Duration::from_secs(3), |_r, clean| clean.contains("gruvbox") && clean.contains("Theme"));

			assert!(clean.contains("gruvbox"), "Completion menu should show 'gruvbox'. Clean: {clean:?}");
			assert!(clean.contains("Theme"), "Completion menu should show 'Theme' kind. Clean: {clean:?}");
			...
			assert!(clean.contains(":theme "), "Command line should preserve 'theme ' prefix. Clean: {clean:?}");
		});
	});
}
