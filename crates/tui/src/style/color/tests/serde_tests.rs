...
fn deserialize() -> Result<(), de::value::Error> {
	assert_eq!(Color::Black, Color::deserialize("Black".into_deserializer())?);
	assert_eq!(Color::Magenta, Color::deserialize("magenta".into_deserializer())?);
	assert_eq!(Color::LightGreen, Color::deserialize("LightGreen".into_deserializer())?);
	assert_eq!(Color::White, Color::deserialize("bright-white".into_deserializer())?);
	assert_eq!(Color::Indexed(42), Color::deserialize("42".into_deserializer())?);
	assert_eq!(Color::Rgb(0, 255, 0), Color::deserialize("#00ff00".into_deserializer())?);
	Ok(())
}
...
fn serialize_then_deserialize() -> Result<(), serde_json::Error> {
	...
	assert_eq!(serde_json::from_str::<Color>(&json_rgb)?, Color::Rgb(255, 0, 255));
	...
	assert_eq!(serde_json::from_str::<Color>(&json_indexed)?, Color::Indexed(10));
