...
/// # Initialization
///
/// For most applications, consider using the convenience functions `tome_tui::run()`,
/// `tome_tui::init()`, and `tome_tui::restore()` (available since version 0.28.1) along with the
	...
	B: Backend,
{
	...
	///
	/// Note that unlike `tome_tui::init`, this does not install a panic hook, so it is recommended
	...
	/// use std::io::stdout;
	///
	/// use crate::{backend::CrosstermBackend, layout::Rect, Terminal, TerminalOptions, Viewport};
	///
	/// let backend = CrosstermBackend::new(stdout());
	...
	///
	/// ```rust,ignore
	/// # let backend = tome_tui::backend::TestBackend::new(10, 10);
	/// # let mut terminal = tome_tui::Terminal::new(backend)?;
	/// use crate::{layout::Position, widgets::Paragraph};
	///
	/// // with a closure
	/// terminal.draw(|frame| {
	///     let area = frame.area();
	...
	///
	/// fn render(frame: &mut tome_tui::Frame) {
	...
	///
	/// fn render(frame: &mut tome_tui::Frame) -> io::Result<()> {
	///     let value: u8 = "not a number".parse().map_err(io::Error::other)?;
	///     frame.render_widget(Paragraph::new("Hello World!"), frame.area());
	///     Ok(())
	/// }
