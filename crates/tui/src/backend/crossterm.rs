...
//! This module provides [`CrosstermBackend`], an implementation of the [`Backend`] trait for the
//! [Ratatui] library. It uses the [Crossterm] library for all terminal manipulation.
//!
//! ## Crossterm Version and Re-export
//!
//! `tome-tui` requires you to specify a version of the [Crossterm] library to be used.
...
//! (0.x) releases of `tome-tui`, and we may add support for newer versions in minor
//! (0.x.y) releases.
//!
...
//! ensuring compatibility with the version used by `tome-tui`. By using
...
//! `tome-tui`, then any code using `crate::backend::crossterm::crossterm` will refer to
...
//! # Crate Organization
//!
//! `tome-tui` is part of the Ratatui workspace that was modularized in version 0.30.0.
//! This crate provides the [Crossterm] backend implementation for Ratatui.
//!
//! **When to use `tome-tui`:**
//!
...
//!
//! **When to use the main [`tome_tui`] crate:**
//!
...
cfg_if::cfg_if! {
	...
	// to make it possible to easily enable all features when compiling `tome-tui`.
...
pub trait IntoCrossterm<C> {
	/// Converts the tome_tui type to a crossterm type.
...
pub trait FromCrossterm<C> {
	/// Converts the crossterm type to a tome_tui type.
...
mod tests {
	...
	mod modifier {
		...
		fn from_crossterm_attribute(
			...
			#[case] tome_tui_modifier: Modifier,
		) {
			assert_eq!(
