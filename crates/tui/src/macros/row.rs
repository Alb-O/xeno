...
/// use xeno_tui::{row, line, text, span};
///
/// let row = row![
...
macro_rules! row {
    () => {
        $crate::widgets::table::Row::default()
    };
    ($cell:expr; $n:expr) => {
        $crate::widgets::table::Row::new($crate::vec![$crate::widgets::table::Cell::from($cell); $n])
    };
    ($($cell:expr),+ $(,)?) => {{
        $crate::widgets::table::Row::new($crate::vec![
        $(
            $crate::widgets::table::Cell::from($cell),
        )+
        ])
    }};
}
...
mod tests {
	...
	use crate::text::Text;
	use crate::widgets::table::{Cell, Row};
