...
pub(super) fn stateful_widget(widget: List<'_>, state: &mut ListState, width: u16, height: u16) -> Buffer {
	let mut buffer = Buffer::empty(Rect::new(0, 0, width, height));
...
fn combinations() {
	...
	#[track_caller]
	fn test_case_render_stateful<'line, Lines>(items: &[ListItem], selected: Option<usize>, expected: Lines)
	where
		Lines: IntoIterator,
		Lines::Item: Into<Line<'line>>,
	{
	...
	// empty list
	test_case_render(&empty_items, ["          ", "          ", "          ", "          ", "          "]);
	test_case_render_stateful(&empty_items, None, ["          ", "          ", "          ", "          ", "          "]);
	test_case_render_stateful(&empty_items, Some(0), ["          ", "          ", "          ", "          ", "          "]);

	// single item
	test_case_render(&single_item, ["Item 0    ", "          ", "          ", "          ", "          "]);
	test_case_render_stateful(&single_item, None, ["Item 0    ", "          ", "          ", "          ", "          "]);
	test_case_render_stateful(&single_item, Some(0), [">>Item 0  ", "          ", "          ", "          ", "          "]);
	test_case_render_stateful(&single_item, Some(1), [">>Item 0  ", "          ", "          ", "          ", "          "]);

	// multiple items
	test_case_render(&multiple_items, ["Item 0    ", "Item 1    ", "Item 2    ", "          ", "          "]);
	test_case_render_stateful(&multiple_items, None, ["Item 0    ", "Item 1    ", "Item 2    ", "          ", "          "]);
	test_case_render_stateful(&multiple_items, Some(0), [">>Item 0  ", "  Item 1  ", "  Item 2  ", "          ", "          "]);
	test_case_render_stateful(&multiple_items, Some(1), ["  Item 0  ", ">>Item 1  ", "  Item 2  ", "          ", "          "]);
	test_case_render_stateful(&multiple_items, Some(3), ["  Item 0  ", "  Item 1  ", ">>Item 2  ", "          ", "          "]);

	// multi line items
	test_case_render(&multi_line_items, ["Item 0    ", "Line 2    ", "Item 1    ", "Item 2    ", "          "]);
	test_case_render_stateful(&multi_line_items, None, ["Item 0    ", "Line 2    ", "Item 1    ", "Item 2    ", "          "]);
	test_case_render_stateful(
		&multi_line_items,
		Some(0),
		[">>Item 0  ", "  Line 2  ", "  Item 1  ", "  Item 2  ", "          "],
	);
	test_case_render_stateful(
		&multi_line_items,
		Some(1),
