...
use super::layout::{STACK_SPACING, anchor_position, apply_animation, calculate_toast_size, calculate_x, calculate_y};
...
pub(super) fn render_toast(state: &ToastState, rect: Rect, buf: &mut Buffer) {
	...
	let bg_colors = if opacity < 1.0 { Some(sample_background(rect, buf)) } else { None };
	...
	let content_area = if let Some(ref icon) = state.toast.icon {
		...
		if inner.width > icon_width {
			...
			Rect::new(inner.x + icon_width, inner.y, inner.width - icon_width, inner.height)
		} else {
			inner
		}
	} else {
...
pub(super) fn sample_background(rect: Rect, buf: &Buffer) -> Vec<Color> {
	...
	for y in rect.y..rect.bottom() {
		for x in rect.x..rect.right() {
			let color = buf.cell(Position::new(x, y)).map(|c| c.bg).unwrap_or(Color::Reset);
			colors.push(color);
		}
	}
	colors
...
impl super::ToastManager {
	...
	pub(super) fn render_anchor_group(&mut self, anchor: Anchor, ids: &[u64], area: Rect, buf: &mut Buffer) {
		let mut sorted_ids: Vec<u64> = ids.to_vec();
		sorted_ids.sort_by_key(|id| self.states.get(id).map(|s| s.created_at).unwrap_or(Instant::now()));

		let stacks_up = matches!(anchor, Anchor::BottomLeft | Anchor::BottomCenter | Anchor::BottomRight);
		let anchor_pos = anchor_position(anchor, area);

		let ordered: Vec<u64> = if stacks_up { sorted_ids.into_iter().rev().collect() } else { sorted_ids };
