...
async fn nu_macro_recursion_depth_guard_trips() {
	...
	std::fs::write(temp.path().join("xeno.nu"), "export def recur [] { xeno call recur }").expect("xeno.nu should be writable");
...
async fn nu_macro_ctx_is_injected() {
	...
	std::fs::write(
		...
		"export def go [] { let ctx = (xeno ctx); if $ctx.kind == \"macro\" { (xeno emit action invocation_test_action) } else { (xeno emit action does-not-exist) } }",
	)
...
async fn nu_hook_ctx_is_injected() {
	...
	std::fs::write(
		temp.path().join("xeno.nu"),
		"export def on_action_post [name result] { if $env.XENO_CTX.kind == \"hook\" { (xeno emit action invocation_test_action) } else { (xeno emit action does-not-exist) } }",
	)
	.expect("xeno.nu should be writable");
...
async fn nu_runtime_reload_swaps_executor_and_disables_old_runtime_hooks() {
	...
	std::fs::write(
		...
		"export def on_action_post [name result] {\n\
			if $name == \"invocation_edit_action\" and $result == \"ok\" {\n\
				(xeno emit action invocation_test_action)\n\
			}\n\
		}",
	)
	.expect("xeno.nu should be writable");
	...
	std::fs::write(
		...
		"export def on_action_post [name result] {\n\
			if $name == \"invocation_edit_action\" and $result == \"ok\" {\n\
				(xeno emit action invocation_test_action_alt)\n\
			}\n\
		}",
	)
	.expect("xeno.nu should be writable");
...
async fn nu_macro_respects_configured_decode_limits() {
	...
	std::fs::write(
		...
		"export def go [] { [(xeno emit action invocation_test_action), (xeno emit action invocation_test_action)] }",
	)
	.expect("xeno.nu should be writable");
...
async fn nu_stats_reflect_hook_pipeline_state() {
	...
	std::fs::write(
		temp.path().join("xeno.nu"),
		"export def on_action_post [name result] { xeno emit action invocation_test_action }",
	)
	.expect("xeno.nu should be writable");
