...
pub(super) fn spawn_plan(
	...
	// Stage-A (highest priority)
	if let Some(req) = plan.stage_a {
		let vp_key = req.spec.viewport_key;
		let injections = req.spec.opts.injections;
		let parse_timeout = req.spec.opts.parse_timeout;
		if let Some(task_id) = collector.spawn(Arc::clone(permits), Arc::clone(engine), req.spec, mgr_cfg.viewport_reserve, req.privileged) {
			entry.sched.lanes.viewport_urgent.active = Some(task_id);
			entry.sched.lanes.viewport_urgent.requested_doc_version = ctx.doc_version;
			kicked_any = true;
			tracing::trace!(
				target: "xeno_undo_trace",
				doc_id = ?ctx.doc_id,
	...
	if let Some(req) = plan.stage_b {
		...
		if let Some(task_id) = collector.spawn(Arc::clone(permits), Arc::clone(engine), req.spec, mgr_cfg.viewport_reserve, req.privileged) {
			...
			entry.sched.lanes.viewport_enrich.requested_doc_version = ctx.doc_version;
			debug_assert!(
	...
	if let Some(req) = plan.bg {
		let class = req.spec.kind.class();
		let injections = req.spec.opts.injections;
		let parse_timeout = req.spec.opts.parse_timeout;
		if let Some(task_id) = collector.spawn(Arc::clone(permits), Arc::clone(engine), req.spec, mgr_cfg.viewport_reserve, req.privileged) {
			entry.sched.lanes.bg.active = Some(task_id);
			entry.sched.lanes.bg.requested_doc_version = ctx.doc_version;
