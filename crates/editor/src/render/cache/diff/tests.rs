...
fn get_or_build_reuses_existing_entry_for_same_version() {
	...
	let first: Arc<Vec<DiffLineNumbers>> = Arc::clone(&cache.get_or_build(doc_id, 7, || vec![DiffLineNumbers::default()]).line_numbers);
	let second: Arc<Vec<DiffLineNumbers>> = Arc::clone(&cache.get_or_build(doc_id, 7, || panic!("should not rebuild")).line_numbers);
...
fn get_or_build_rebuilds_for_new_version() {
	...
	let first: Arc<Vec<DiffLineNumbers>> = Arc::clone(&cache.get_or_build(doc_id, 1, || vec![DiffLineNumbers::default()]).line_numbers);
	let second: Arc<Vec<DiffLineNumbers>> = Arc::clone(&cache.get_or_build(doc_id, 2, || vec![DiffLineNumbers { old: Some(1), new: None }]).line_numbers);
...
fn invalidate_document_clears_all_versions_for_doc() {
	...
	let rebuilt: Arc<Vec<DiffLineNumbers>> = Arc::clone(
		&cache
			.get_or_build(doc1, 1, || vec![DiffLineNumbers { old: Some(9), new: Some(9) }])
			.line_numbers,
	);
	assert_eq!(rebuilt[0].old, Some(9));

	let existing: Arc<Vec<DiffLineNumbers>> = Arc::clone(&cache.get_or_build(doc2, 1, || panic!("doc2 entry should remain")).line_numbers);
