...
impl ViewManager {
	...
	/// Replaces the buffer stored at `id` while preserving the view identity.
	///
	/// Updates the reverse document-to-view index for both old and new documents.
	pub(crate) fn replace_buffer(&mut self, id: ViewId, buffer: Buffer) -> Option<Buffer> {
		debug_assert_eq!(id, buffer.id, "replacement buffer must keep target view id");
		let new_doc_id = buffer.document_id();
		let old = self.buffers.insert(id, buffer);
		if let Some(old_buffer) = old.as_ref() {
			let old_doc_id = old_buffer.document_id();
			if old_doc_id != new_doc_id {
				self.index_remove(old_doc_id, id);
				self.index_add(new_doc_id, id);
			}
		} else {
			self.index_add(new_doc_id, id);
		}
