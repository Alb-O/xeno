...
macro_rules! notification_type {
	($static_name:ident, $name:literal, $level:expr, $icon:expr, $style:expr, $auto_dismiss:expr) => {
		#[::linkme::distributed_slice($crate::notifications::NOTIFICATION_TYPES)]
		static $static_name: crate::notifications::NotificationTypeDef =
			crate::notifications::NotificationTypeDef {
				id: $name,
				name: $name,
				level: $level,
				icon: $icon,
