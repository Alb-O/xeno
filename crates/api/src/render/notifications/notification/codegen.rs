//! Code generation for notification debugging.
//!
...
pub fn generate_code(notification: &Notification) -> String {
	...
	if notification.border_kind != defaults.border_kind {
		lines.push(format!(
			"    .border_kind(BorderKind::{:?})",
			notification.border_kind
		));
	}
