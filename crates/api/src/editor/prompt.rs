...
impl Editor {
	pub fn open_rename_prompt(&mut self) -> bool {
		...
		if !word.is_empty() {
			...
			buffer.doc_mut().content = Rope::from(word.as_str());
...
fn word_at_cursor(buffer: &crate::buffer::Buffer) -> String {
	let doc = buffer.doc();
	let mut start = buffer.cursor.min(doc.content.len_chars());
	let mut end = start;
	while start > 0 {
		let ch = doc.content.char(start - 1);
		if is_word_char(ch) {
	...
	while end < doc.content.len_chars() {
		let ch = doc.content.char(end);
		if is_word_char(ch) {
	...
	doc.content.slice(start..end).to_string()
}
