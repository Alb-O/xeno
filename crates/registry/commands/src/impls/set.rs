...
mod tests {
	...
	fn test_parse_equals_syntax() {
		let args = vec!["tab-width=4"];
		let (key, value) = parse_set_args(&args).unwrap();
		assert_eq!(key, "tab-width");
		assert_eq!(value, "4");
	}

	#[test]
	fn test_parse_space_syntax() {
		let args = vec!["tab-width", "4"];
		let (key, value) = parse_set_args(&args).unwrap();
		assert_eq!(key, "tab-width");
		assert_eq!(value, "4");
	...
	fn test_parse_bool_true() {
		let args = vec!["cursorline"];
		let (key, value) = parse_set_args(&args).unwrap();
		assert_eq!(key, "cursorline");
		assert_eq!(value, "true");
	}

	#[test]
	fn test_parse_bool_false() {
		let args = vec!["nocursorline"];
		let (key, value) = parse_set_args(&args).unwrap();
		assert_eq!(key, "cursorline");
	...
	fn test_parse_string_with_spaces() {
		let args = vec!["theme", "gruvbox", "dark"];
		let (key, value) = parse_set_args(&args).unwrap();
		assert_eq!(key, "theme");
		assert_eq!(value, "gruvbox dark");
	}
}
