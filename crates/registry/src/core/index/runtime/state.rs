use super::*;
use crate::core::index::{build, cmp_party, lookup, u32_index};
	...
	T: RuntimeEntry,
{
	...
	fn register_internal(&self, def: &dyn build::BuildEntry<T>) -> Result<RegistryRef<T, Id>, RegisterError<T, Id>> {
		loop {
			...
			let mut runtime_ctx = build::RuntimeBuildCtx { interner: &interner };
			...
			let new_entry = {
				...
				let mut ctx = build::DebugBuildCtx {
			...
			if let Some(idx) = existing_idx {
				...
				let is_better = match self.policy {
					...
					DuplicatePolicy::ByPriority => cmp_party(&new_party, &existing_party) == Ordering::Greater,
				...
				if is_better {
					...
					let existing_id_id = Id::from_u32(u32_index(idx, self.label));
				...
				new_by_id.insert(id_sym, Id::from_u32(u32_index(new_idx, self.label)));
			}
			...
			let (by_name, by_key, mut key_collisions) = if let Some((idx, _, _)) = replaced_info {
				lookup::update_stage_maps_replace(self.label, &new_table, &parties, &key_pool, idx, &old, &new_by_id)
			} else {
				// Incremental append
				lookup::update_stage_maps_append(
					self.label,
					&new_table,
					&parties,
					&key_pool,
			...
			if Arc::ptr_eq(&prev, &old) {
				...
				let result_id = replaced_info
					.map(|(i, _, _)| Id::from_u32(u32_index(i, self.label)))
					.unwrap_or_else(|| Id::from_u32(u32_index(new_idx, self.label)));
