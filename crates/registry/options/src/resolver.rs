...
mod tests {
	...
	fn test_resolve_default() {
		...
		let value = resolver.resolve(keys::tab_width);
	...
	fn test_resolve_global() {
		let mut global = OptionStore::new();
		global.set(keys::tab_width, OptionValue::Int(8));

		let resolver = OptionResolver::new().with_global(&global);

		assert_eq!(resolver.resolve_int(keys::tab_width), 8);
	}
	...
	fn test_resolve_language_overrides_global() {
		...
		language.set(keys::tab_width, OptionValue::Int(2));
	...
	fn test_resolve_buffer_overrides_all() {
		...
		let mut buffer = OptionStore::new();
		buffer.set(keys::tab_width, OptionValue::Int(8));
	...
	fn test_resolve_fallthrough() {
		...
		let mut buffer = OptionStore::new();
		buffer.set(keys::theme, OptionValue::String("monokai".to_string()));
		...
		assert_eq!(resolver.resolve_int(keys::tab_width), 4);
		// theme from buffer
		assert_eq!(resolver.resolve_string(keys::theme), "monokai");
		// scroll_margin from default (nobody has it)
		assert_eq!(resolver.resolve_int(keys::scroll_margin), 3); // Default is 3
	}

	#[test]
	fn test_resolve_bool() {
		let mut global = OptionStore::new();
		global.set(keys::cursorline, OptionValue::Bool(false));

		let resolver = OptionResolver::new().with_global(&global);

		assert!(!resolver.resolve_bool(keys::cursorline));
	}

	#[test]
	fn test_resolve_string() {
		let mut global = OptionStore::new();
		global.set(keys::theme, OptionValue::String("nord".to_string()));

		let resolver = OptionResolver::new().with_global(&global);

		assert_eq!(resolver.resolve_string(keys::theme), "nord");
	}
	...
	fn test_type_mismatch_falls_back_to_default() {
		...
		global.set(keys::tab_width, OptionValue::String("bad".to_string()));
