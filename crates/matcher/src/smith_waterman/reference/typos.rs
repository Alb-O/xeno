...
mod tests {
	...
	fn analyze_contract(needle: &str, haystack: &str, scoring: &Scoring) -> Option<(u16, u16, u16)> {
		...
		let (_score, score_matrix, _exact) = smith_waterman(needle, haystack, scoring);
	...
	///
	/// Finds a case where `old_det > new_min` and verifies `match_list` now accepts at the
	/// tighter budget where the old traceback would have rejected.
	#[test]
	fn tie_aware_traceback_improves_over_deterministic() {
		...
		let (needle, haystack, end_score, new_typos, old_typos) =
		...
		// The new tie-aware traceback accepts at the tighter budget.
		let cfg = Config {
			prefilter: false,
			max_typos: Some(new_typos),
			sort: false,
			scoring: scoring.clone(),
		};
		let hay = [haystack.as_str()];
		let accepted = match_list(needle.as_str(), &hay, &cfg);
		assert!(
			!accepted.is_empty(),
			"tie-aware traceback should accept at budget={new_typos}: needle={needle:?} \
			 haystack={haystack:?} end_score={end_score} new_typos={new_typos} old_typos={old_typos}"
		);
	}
