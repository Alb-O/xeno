...
impl<'a> EditorContext<'a> {
	...
	pub fn require_search(&mut self) -> Result<&mut dyn SearchAccess, crate::registry::CommandError> {
		self.inner
			.search()
			.ok_or(crate::registry::CommandError::MissingCapability(
				crate::registry::Capability::Search,
			))
	}
	...
	pub fn require_undo(&mut self) -> Result<&mut dyn UndoAccess, crate::registry::CommandError> {
		self.inner
			.undo()
			.ok_or(crate::registry::CommandError::MissingCapability(
				crate::registry::Capability::Undo,
			))
	}
	...
	pub fn require_edit(&mut self) -> Result<&mut dyn EditAccess, crate::registry::CommandError> {
		self.inner
			.edit()
			.ok_or(crate::registry::CommandError::MissingCapability(
				crate::registry::Capability::Edit,
			))
	}
	...
	pub fn require_selection_ops(
		&mut self,
	) -> Result<&mut dyn SelectionOpsAccess, crate::registry::CommandError> {
		self.inner
			.selection_ops()
			.ok_or(crate::registry::CommandError::MissingCapability(
				crate::registry::Capability::SelectionOps,
			))
	}

	pub fn check_capability(&mut self, cap: crate::registry::Capability) -> bool {
	...
	pub fn check_all_capabilities(
		&mut self,
		caps: &[crate::registry::Capability],
	) -> Result<(), crate::registry::CommandError> {
		for &cap in caps {
			if !self.check_capability(cap) {
				return Err(crate::registry::CommandError::MissingCapability(cap));
			}
		}
		Ok(())
	}
