...
fn cmd_registry_diag(ctx: &mut CommandContext) -> Result<CommandOutcome, CommandError> {
	let mut out = Vec::new();

	out.push("--- Registry Inventory ---".to_string());
	out.push(format!("Commands: {}", crate::registry::all_commands().count()));
	out.push(format!("Actions:  {}", crate::registry::all_actions().count()));
	out.push(format!("Motions:  {}", crate::registry::all_motions().count()));
	out.push(format!(
		"Objects:  {}",
...
fn cmd_registry_doctor(ctx: &mut CommandContext) -> Result<CommandOutcome, CommandError> {
	let diag = diagnostics();
	if diag.collisions.is_empty() {
