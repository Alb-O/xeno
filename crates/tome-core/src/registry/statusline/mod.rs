...
macro_rules! statusline_segment {
	($static_name:ident, $name:expr, $position:expr, $priority:expr, $enabled:expr, $render:expr) => {
		#[::linkme::distributed_slice($crate::registry::statusline::STATUSLINE_SEGMENTS)]
		static $static_name: $crate::registry::statusline::StatuslineSegmentDef =
			$crate::registry::statusline::StatuslineSegmentDef {
				id: $name,
				name: $name,
				position: $position,
				priority: $priority,
