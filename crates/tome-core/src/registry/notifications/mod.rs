...
pub struct NotificationTypeDef {
	...
	pub priority: i16,
	pub source: crate::registry::RegistrySource,
}
...
macro_rules! notification_type {
	($static_name:ident, $name:literal, $level:expr, $icon:expr, $style:expr, $auto_dismiss:expr) => {
		#[::linkme::distributed_slice($crate::registry::notifications::NOTIFICATION_TYPES)]
		static $static_name: $crate::registry::notifications::NotificationTypeDef =
			$crate::registry::notifications::NotificationTypeDef {
				id: $name,
				name: $name,
				level: $level,
				icon: $icon,
