...
use crate::registry::notifications::notification::Notification;
use crate::registry::notifications::render::render_notifications;
use crate::registry::notifications::state::{ManagerDefaults, NotificationState};
use crate::registry::notifications::types::{Anchor, NotificationError, Overflow};
...
impl Notifications {
	...
	pub fn tick(&mut self, delta: Duration) {
		...
		let finished: Vec<u64> = self
			...
			.iter()
			.filter_map(|(id, state)| {
				if state.current_phase == crate::registry::notifications::types::AnimationPhase::Finished
				{
					Some(*id)
				} else {
					None
