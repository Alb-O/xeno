...
impl TextObjectQuery {
	...
	) -> Option<Box<dyn Iterator<Item = CapturedNode<'a>> + 'a>> {
		let capture_names = [capture_name];
		self.capture_nodes_any(&capture_names, node, slice, cursor)
	}
	...
	pub fn capture_nodes_any<'a>(
		...
		let capture = capture_names.iter().find_map(|cap| self.query.get_capture(cap))?;
