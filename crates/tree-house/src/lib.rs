...
pub use crate::sealed_source::SealedSource;
...
impl Syntax {
	pub fn new(source: RopeSlice, language: Language, timeout: Duration, loader: &impl LanguageLoader) -> Result<Self, Error> {
	...
	pub fn named_descendant_for_byte_range(&self, start: u32, end: u32) -> Option<Node<'_>> {
		self.tree_for_byte_range(start, end).root_node().named_descendant_for_byte_range(start, end)
	}

	#[inline]
	pub fn descendant_for_byte_range(&self, start: u32, end: u32) -> Option<Node<'_>> {
		self.tree_for_byte_range(start, end).root_node().descendant_for_byte_range(start, end)
	}
	...
	pub fn layer_for_byte_range(&self, start: u32, end: u32) -> Layer {
		self.layers_for_byte_range(start, end).last().expect("always includes the root layer")
	}
...
impl PartialEq for LayerData {
	fn eq(&self, other: &Self) -> bool {
		self.parent == other.parent && self.language == other.language && self.ranges == other.ranges
	}
}
...
impl LayerData {
	...
	pub fn injection_at_byte_idx(&self, idx: u32) -> Option<&Injection> {
		self.injections_at_byte_idx(idx).next().filter(|injection| injection.range.start <= idx)
	}
	...
	pub fn injections_at_byte_idx(&self, idx: u32) -> impl Iterator<Item = &Injection> {
		let i = self.injections.partition_point(|range| range.range.end < idx);
...
impl fmt::Display for Error {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::NoRootConfig => f.write_str("`LanguageLoader::get_config` for the root layer language returned `None`"),
			Self::IncompatibleGrammar(language, IncompatibleGrammarError { abi_version }) => {
				write!(f, "failed to load grammar for language {language:?} with ABI version {abi_version}")
			}
		}
	}
}
