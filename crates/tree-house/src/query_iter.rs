...
use crate::locals::{Scope, ScopeCursor};
...
impl<'a, 'tree> LayerQueryIter<'a, 'tree> {
	fn peek<Loader: QueryLoader<'a>>(&mut self, source: RopeSlice<'_>, loader: &Loader) -> Option<&MatchedNode<'tree>> {
		if self.peeked.is_none() {
			loop {
				...
				if !loader.are_predicates_satisfied(self.language, &query_match, source, &self.scope_cursor) {
					query_match.remove();
	...
	S: Default,
{
	fn init_layer(&mut self, injection: Injection) -> Box<ActiveLayer<'a, 'tree, S>> {
		self.active_layers.remove(&injection.layer).unwrap_or_else(|| {
			let layer = self.syntax.layer(injection.layer);
			let start_point = injection.range.start.max(self.range.start);
			let injection_start = layer.injections.partition_point(|child| child.range.end < start_point);
			let cursor = if self.finished_layers.contains(&injection.layer) {
				...
				self.loader
					...
					.map(|(query, node)| {
						InactiveQueryCursor::new(self.range.clone(), TREE_SITTER_MATCH_LIMIT).execute_query(query, &node, RopeInput::new(self.src))
					})
			};
			Box::new(ActiveLayer {
				state: S::default(),
...
where
	...
	LayerState: Default,
{
	pub fn new(syntax: &'tree Syntax, src: RopeSlice<'a>, loader: Loader, range: impl RangeBounds<u32>) -> Self {
	...
	pub fn current_injection(&mut self) -> (Injection, &mut LayerState) {
		(self.current_injection.clone(), &mut self.current_layer.state)
	}

	#[inline]
	pub fn current_language(&self) -> Language {
		self.layer_manager.syntax.layer(self.current_injection.layer).language
	}
	...
	pub fn layer_state(&mut self, layer: Layer) -> &mut LayerState {
		if layer == self.current_injection.layer {
			...
			&mut self.layer_manager.active_layers.get_mut(&layer).unwrap().state
		}
	}
	...
	fn enter_injection(&mut self, injection: Injection) {
		...
		self.layer_manager.active_layers.insert(old_injection.layer, old_layer);
		self.layer_manager.active_injections.push(old_injection);
	}

	fn exit_injection(&mut self) -> Option<(Injection, Option<LayerState>)> {
		let injection = replace(&mut self.current_injection, self.layer_manager.active_injections.pop()?);
		let mut layer = replace(&mut self.current_layer, self.layer_manager.active_layers.remove(&self.current_injection.layer)?);
		let layer_unfinished = layer.query_iter.peeked.is_some() || layer.injections.peek().is_some();
		if layer_unfinished {
			self.layer_manager.active_layers.insert(injection.layer, layer);
			Some((injection, None))
		} else {
	...
	fn next(&mut self) -> Option<Self::Item> {
		loop {
			...
			let next_match = self
				...
				.filter(|matched_node| matched_node.node.start_byte() <= self.current_injection.range.end);

			match (next_match, next_injection) {
				(None, None) => {
					return self
						.exit_injection()
						.map(|(injection, state)| QueryIterEvent::ExitInjection { injection, state });
				}
				...
				(Some(matched_node), Some(injection)) if matched_node.node.start_byte() < injection.range.end => {
					// consume match
					let matched_node = self.current_layer.query_iter.consume();
					// ignore nodes that are overlapped by the injection
					if matched_node.node.start_byte() <= injection.range.start || injection.range.end < matched_node.node.end_byte() {
						return Some(QueryIterEvent::Match(matched_node));
					}
				}
...
pub enum QueryIterEvent<'tree, State = ()> {
	...
	ExitInjection { injection: Injection, state: Option<State> },
}
