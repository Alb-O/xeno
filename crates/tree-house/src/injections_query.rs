...
use tree_sitter::{Capture, Grammar, InactiveQueryCursor, MatchedNodeIdx, Node, Pattern, Query, QueryMatch};
...
impl InjectionsQuery {
	pub fn new(grammar: Grammar, injection_query_text: &str, local_query_text: &str) -> Result<Self, query::ParseError> {
	...
	pub(crate) fn configure(&self, f: &mut impl FnMut(&str) -> Option<Highlight>) {
		let local_definition_captures = self
			...
			.collect();
		self.local_definition_captures.store(Arc::new(local_definition_captures));
	}
	...
	fn process_match<'a, 'tree>(
		...
		for (i, matched_node) in query_match.matched_nodes().enumerate() {
			...
			if capture == self.injection_language_capture {
				...
				marker = Some(InjectionLanguageMarker::Match(source.byte_slice(range.start as usize..range.end as usize)));
			} else if capture == self.injection_filename_capture {
				let range = matched_node.node.byte_range();
				marker = Some(InjectionLanguageMarker::Filename(source.byte_slice(range.start as usize..range.end as usize)));
		...
		let marker = marker.or(properties.and_then(|p| p.language.as_deref()).map(InjectionLanguageMarker::Name))?;
	...
	fn execute<'a>(&'a self, node: &Node<'a>, source: RopeSlice<'a>, loader: &'a impl LanguageLoader) -> impl Iterator<Item = InjectionQueryMatch<'a>> + 'a {
		let mut cursor = InactiveQueryCursor::new(0..u32::MAX, TREE_SITTER_MATCH_LIMIT).execute_query(&self.injection_query, node, source);
...
impl Syntax {
	pub(crate) fn run_injection_query(
		...
		for mat in injection_query {
			...
			if let Some(last_injection) = injections.last().filter(|injection| ranges_intersect(&injection.range, &matched_node_range)) {
				// this condition is not needed but serves as fast path
				// for common cases
				if last_injection.range.start <= matched_node_range.start {
					continue;
				} else {
					insert_position = injections.partition_point(|injection| injection.range.end <= matched_node_range.start);
			...
			if layer_data.flags.reused {
				layer_data.flags.modified |= reused_injection
					.as_ref()
					.is_none_or(|injection| injection.matched_node_range != matched_node_range || injection.layer != layer);
	...
	fn map_injections(
		...
		let first_relevant_injection = layer_data.injections.partition_point(|injection| injection.range.end < edits[0].start_byte);
		if first_relevant_injection == layer_data.injections.len() {
			return;
		}
		let mut offset = if let Some(offset) = offset {
			let first_relevant_edit = edits.partition_point(|edit| (edit.old_end_byte as i32) < (layer_data.ranges[0].end_byte as i32 - offset));
	...
	fn reuse_injection(&mut self, language: Language, new_range: Range, injections: &mut Peekable<impl Iterator<Item = Injection>>) -> Option<Injection> {
		...
		injections
			.next_if(|injection| {
				injection.range.start < new_range.end && self.layer(injection.layer).language == language && !self.layer(injection.layer).flags.reused
			})
			.clone()
	}
}

fn intersect_ranges(include_children: IncludedChildren, node: Node, parent_ranges: &[tree_sitter::Range], push_range: impl FnMut(Range)) {
	...
	match include_children {
		IncludedChildren::None => intersect_ranges_impl(range, node.children().map(|node| node.byte_range()), parent_ranges, push_range),
		...
		IncludedChildren::Unnamed => intersect_ranges_impl(
			range,
			node.children().filter(|node| node.is_named()).map(|node| node.byte_range()),
			parent_ranges,
			push_range,
		),
	}
