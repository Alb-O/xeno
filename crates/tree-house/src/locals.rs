...
impl Locals {
	fn push(&mut self, scope: ScopeData) -> Scope {
		...
		let parent = scope.parent.expect("push cannot be used for the root layer");
	...
	pub fn scope_cursor(&self, pos: u32) -> ScopeCursor<'_> {
		...
		loop {
			let scope_data = &self[scope];
			let child_idx = scope_data.children.partition_point(|&child| self[child].range.end < pos);
		...
		ScopeCursor { locals: self, scope_stack }
	}
}
...
impl Syntax {
	pub(crate) fn run_local_query(&mut self, layer: Layer, source: RopeSlice<'_>, loader: &impl LanguageLoader) {
		let layer_data = &mut self.layer_mut(layer);
		let Some(LanguageConfig { injection_query, .. }) = loader.get_config(layer_data.language) else {
			return;
		};
		...
		let mut cursor = InactiveQueryCursor::new(0..u32::MAX, TREE_SITTER_MATCH_LIMIT).execute_query(&injection_query.local_query, &root, source);
