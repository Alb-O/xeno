...
impl Syntax {
	pub fn update(&mut self, source: RopeSlice, timeout: Duration, edits: &[tree_sitter::InputEdit], loader: &impl LanguageLoader) -> Result<(), Error> {
	...
	fn prune_dead_layers(&mut self) {
		self.layers.retain(|_, layer| take(&mut layer.flags).touched);
	}
}

impl LayerData {
	fn parse(&mut self, parser: &mut Parser, source: RopeSlice, loader: &impl LanguageLoader) -> Result<(), Error> {
		...
		parser.set_included_ranges(&self.ranges).map_err(|_| Error::InvalidRanges)?;
		...
		let tree = self.parse_tree.as_ref().filter(|tree| {
			let included_ranges_range = self.ranges.first().map(|r| r.start_byte).unwrap_or(0)..self.ranges.last().map(|r| r.end_byte).unwrap_or(u32::MAX);
			...
			tree_range.start <= included_ranges_range.start && tree_range.end >= included_ranges_range.end
		});
