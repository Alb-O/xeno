...
impl HighlightQuery {
	pub(crate) fn new(grammar: Grammar, highlight_query_text: &str, local_query_text: &str) -> Result<Self, query::ParseError> {
		// Concatenate the highlights and locals queries.
		let mut query_source = String::with_capacity(highlight_query_text.len() + local_query_text.len());
	...
	pub(crate) fn configure(&self, f: &mut impl FnMut(&str) -> Option<Highlight>) {
		let highlight_indices = self.query.captures().map(|(_, capture_name)| f(capture_name)).collect();
...
impl<'a, 'tree: 'a, Loader: LanguageLoader> Highlighter<'a, 'tree, Loader> {
	...
	pub fn advance(&mut self) -> (HighlightEvent, HighlightList<'_>) {
		...
		while self.next_highlight_start == pos {
			...
			match query_event {
				...
				QueryIterEvent::ExitInjection { injection, state } => {
					...
					// removing it.
					let layer_is_finished = state.is_some() && self.current_layer_highlights().iter().all(|h| h.end <= injection.range.end);
		...
		self.next_highlight_end = self.active_highlights.last().map_or(u32::MAX, |node| node.end);

		if refresh {
			(HighlightEvent::Refresh, HighlightList(self.active_highlights.iter()))
		} else {
			(HighlightEvent::Push, HighlightList(self.active_highlights[prev_stack_size..].iter()))
		}
	}
	...
	fn advance_query_iter(&mut self) -> Option<QueryIterEvent<'tree, ()>> {
		...
		self.next_highlight_start = self.next_query_event.as_ref().map_or(u32::MAX, |event| event.start_byte());
		event
	}

	fn process_highlight_end(&mut self, pos: u32) {
		let i = self.active_highlights.iter().rposition(|highlight| highlight.end != pos).map_or(0, |i| i + 1);
	...
	fn deactivate_layer(&mut self, injection: Injection) {
		...
		layer_data.dormant_highlights.extend(self.active_highlights.drain(parent_highlights..));
	...
	fn start_highlight(&mut self, node: MatchedNode, first_highlight: &mut bool) {
		...
		let highlight = if Some(node.capture) == config.highlight_query.local_reference_capture {
			...
			let text: Cow<str> = self.query.source().byte_slice(range.start as usize..range.end as usize).into();
			...
			config.injection_query.local_definition_captures.load().get(&definition.capture).copied()
		} else {
			config.highlight_query.highlight_indices.load()[node.capture.idx()]
		};

		let highlight = highlight.map(|highlight| HighlightedNode { end: range.end, highlight });
		...
		if !*first_highlight {
			...
			let insert_position = self.active_highlights.iter().rposition(|h| h.end <= range.end);
...
impl<'a, T: LanguageLoader> QueryLoader<'a> for HighlightQueryLoader<&'a T> {
	...
	fn are_predicates_satisfied(&self, lang: Language, mat: &QueryMatch<'_, '_>, source: RopeSlice<'_>, locals_cursor: &ScopeCursor<'_>) -> bool {
		let highlight_query = &self.0.get_config(lang).expect("must have a config to emit matches").highlight_query;
		...
		if highlight_query.local_reference_capture.is_some() && highlight_query.non_local_patterns.contains(&mat.pattern()) {
