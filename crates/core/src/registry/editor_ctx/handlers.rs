...
macro_rules! result_handler {
	($slice:ident, $static_name:ident, $name:literal, $body:expr) => {
		#[::linkme::distributed_slice($crate::registry::editor_ctx::$slice)]
		static $static_name: $crate::registry::editor_ctx::ResultHandler =
			$crate::registry::editor_ctx::ResultHandler {
				name: $name,
				handle: $body,
			};
	};
