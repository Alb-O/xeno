...
pub(super) fn sync_bootstrap(entry: &mut DocEntry, ctx: &EnsureLang<'_>, engine: &dyn SyntaxEngine) -> Flow<()> {
	let cfg = ctx.base.cfg;
	let is_bootstrap = !entry.slot.has_any_tree();
	let is_visible = matches!(ctx.base.hotness, SyntaxHotness::Visible);
	...
	match engine.parse(ctx.base.content.slice(..), lang_id, ctx.base.loader, sync_opts) {
		Ok(syntax) => {
			let is_bootstrap = !entry.slot.has_any_tree();
			let is_visible = matches!(ctx.base.hotness, SyntaxHotness::Visible);
