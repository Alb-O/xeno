...
impl<'arena> ImmutablePropertiesMap<'arena> {
	...
	pub fn new_from_try<Error>(
		...
		let key_length_layout = alloc::Layout::array::<usize>(len).expect(
			"LayoutError for key_length_layout: arithmetic overflow or total size exceeds isize::MAX",
		);
		let key_datas_layout = alloc::Layout::array::<*const u8>(len).expect(
			"LayoutError for key_datas_layout: arithmetic overflow or total size exceeds isize::MAX",
		);
...
impl<'de, 'arena> serde::de::DeserializeSeed<'de> for ImmutablePropertiesMapDeSeed<'arena> {
		...
		D: serde::de::Deserializer<'de>,
	{
		...
		impl<'de, 'arena> serde::de::Visitor<'de> for ImmutablePropertiesMapVisitor<'arena> {
			...
			{
				let num_items = map.size_hint().expect(
					"You shouldn't have updated bincode. In v1.3.3 a size_hint was always passed for maps",
				);
