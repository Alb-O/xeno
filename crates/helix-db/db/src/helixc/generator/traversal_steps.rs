...
use super::bool_ops::{BoExp, BoolOp};
use super::source_steps::SourceStep;
use super::utils::{GenRef, GeneratedValue, Order, Separator};
use crate::helixc::analyzer::types::Type;
use crate::helixc::generator::utils::{VecData, write_properties_slice};
	...
	pub fn has_graph_steps(&self) -> bool {
		...
		self.steps.iter().any(|sep| {
			...
			matches!(
				step,
				Step::Out(_)
					| Step::In(_) | Step::OutE(_)
					| Step::InE(_) | Step::FromN
					| Step::ToN | Step::FromV(_)
					| Step::ToV(_) | Step::Count
					| Step::SearchVector(_)
					| Step::ShortestPath(_)
