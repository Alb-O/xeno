...
1. [Critical Improvements Needed](#critical-improvements-needed)
1. [Feature Priorities](#feature-priorities)
1. [WorkspaceEdit Applier Design](#workspaceedit-applier-design)
1. [Undo/Redo Strategy](#undoredo-strategy)
1. [LSP Sync Architecture](#lsp-sync-architecture)
1. [Completion UI Design](#completion-ui-design)
...
01. [Patterns to Keep](#appendix-patterns-to-keep)
01. [Anti-Patterns to Fix](#appendix-anti-patterns-to-fix)
...
1. **Apply atomically** (mutate): push undo state, apply edits, bump versions, trigger reparse, enqueue LSP sync
...
1. Push one `EditorUndoEntry::Group { buffers }`

**On undo:**

1. Pop group entry
1. Call `doc.undo(...)` for each buffer in reverse order
...
1. Wait for next `tick()` to flush
1. **One tick of latency** before server knows about changes
...
1. P1-T1 → P1-T2 (sync control)
1. P1-T6 (flush-now)
1. P2-T1 (undo grouping)
1. P2-T2 → P2-T7 (WorkspaceEdit chain)
1. P5-T2 / P5-T3 (code actions + rename)
...
1. **Central conversion utilities with tests**
